...
class Common
{
	...
	{
		if(empty($GLOBALS['ProfileInfo'] ))
		{
			$pu = Common::GetCurrentUsername();
			// $GLOBALS['ProfileInfo'] = Profile::GetProfileInfo(null, $pu);
		}

		return $GLOBALS['ProfileInfo'];
	}
	...
	{
		if(empty($GLOBALS['GroupInfo'] ))
		{
			$gid = Common::GetCurrentGroupID();
			$gcode = Common::GetCurrentGroupcode();
//			$GLOBALS['GroupInfo'] = Group::GetGroupInfo($gid, $gcode);
		}

		return $GLOBALS['GroupInfo'];
	}
