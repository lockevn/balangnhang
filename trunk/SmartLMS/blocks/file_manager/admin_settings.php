...
*/
	...
	$strtitle = strip_tags(get_string('msgadminsetinstruct', 'block_file_manager'));
	
    //* FUNCTION DEPRECATED @ romain */
    if (!function_exists('build_navigation')){
		// before 1.9
        print_header($strtitle, "$course->fullname",
            "$courselink $stradminsettings", "", "", true, '',
            navmenu($course, null, 1),'','');
    } else {
	    // new method path level 2 @ romain
    	
        $navigation = build_navigation(array(array('name'=>$strtitle, "$courselink $stradminsettings", 'type'=>'misc')));
