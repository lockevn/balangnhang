...
/// Ensures the user is able to view the fmanager
		...
		switch ($groupmode){
			...
			case SEPARATEGROUPS : 
				if ($isadmin || $editingteacher || $coursecreator){
					// Displays everything because of super rights
			...
			case VISIBLEGROUPS : 
				if ($isadmin || $editingteacher || $coursecreator){
					// Displays everything because of super rights
				} elseif (! groups_is_member($groupid)){ // It is ok, no check needed for read-only access
					$readonlyaccess = true;
				}
				break;
		}
    ...
    if (!function_exists('build_navigation')){
        ...
        print_header(strip_tags(get_string('filemanager', 'block_file_manager')), "$course->fullname",
            " $courselink <a href='view.php?id=$id&groupid={$groupid}&amp;rootdir={$tmplink}'>".get_string('filemanager', 'block_file_manager')."</a>$rootlink", 
			'', $meta, true, '',
        ...
        print_header($strtitle, $course->fullname, $navigation, '', $meta,  true, "&nbsp;", "&nbsp;"); 
    }
