...
class yui_menu_plugin_outline extends yui_menu_plugin {
    ...
    function sections($config) {
        global $COURSE, $CFG, $USER, $THEME, $PAGE;
 		
        // probably inefficient, but it works
        get_all_mods($COURSE->id, $mods, $modnames, $modnamesplural, $modnamesused);
        
//        /*danhut added, only display structure of current lesson*/
//        
//        $id = optional_param("id");
//        if(!empty($id)) {
//        	$cm = get_coursemodule_from_id($PAGE->activityname, $id);
//        	$sections = get_records("course_sections", "id", $cm->section, "",
...
//            } else {
            ...
            if ($THEME->custompix) $iconpath .= "/theme/{$CFG->theme}";
            $iconpath .= '/pix';
...
//            }
            ...
            /*to store most recent parent node*/
            $recentParentNode = $item;
            foreach ($sectionmods as $modnumber) {
                if (empty($mods[$modnumber])) continue;
                $mod = $mods[$modnumber];                
                // danhut modified: only show label, resource, quiz, assignment, survey, wiki
                if (!$mod->visible ||  
                	($mod->modname != 'label'
                	&& $mod->modname != 'resource'
                	&& $mod->modname != 'assignment'
                	&& $mod->modname != 'survey'
                	&& $mod->modname != 'wiki'
                	&& $mod->modname != 'quiz')) {
                		continue;
                ...
                /*danhut added: nếu indent = 0, add to Lesson node, nếu indent = 1, add to node có indent = 0 ngay trước đó*/
                ...
                if($mod->indent == 0) {
                	...
                	$recentParentNode = $child;
                } 
                else if($mod->indent == 1) {
                	$recentParentNode->children[$modnumber] = $child;
                }
