...
class PayAccount_Check extends PayAccount {
	
	/**
	 * class constructor
	 */
	function PayAccount_Check() {
	...
	function getFormDetails() {
		...
		$lang =& DoceboLanguage::createInstance('payaccount_check', 'ecom');
	...
	function saveDetails($data_source) {
		...
		foreach ($data_source as $key => $value) {
			if($key!='save' && $key!='account_name'){
				...
				WHERE account_name='check' and  param_name ='".$key."'";
