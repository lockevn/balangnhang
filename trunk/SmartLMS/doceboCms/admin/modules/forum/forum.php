...
/************************************************************************/
/* DOCEBO CMS - Learning Managment System                               */
/* ============================================                         */
/*                                                                      */
/* Copyright (c) 2004                                                   */
/* http://www.docebo.com                                                */
...
$GLOBALS["page"]->addEnd('<link href="'.getPathTemplate("cms").'style/style-admin.css" rel="stylesheet" type="text/css" />', "page_head");

function loadUnreaded() {

	if(!isset($_SESSION['unreaded_forum'])) {
		...
		while(list($id_thread, $id_forum, $is_generator, $how_much_mess) = mysql_fetch_row($reUnreaded)) {
			...
			$_SESSION['unreaded_forum'] = $unreaded;
		}
...
function forum() {
	...
	if($mod_perm) {
		...
		$query_num_view = "
		SELECT COUNT(*) FROM ".$GLOBALS['prefix_cms']."_forum AS f";
	} else {
	...
	if($GLOBALS['cms']['forum_as_table'] == 'on') {
		...
		if($mod_perm) {
			$type_h[] = 'image'; $type_h[] = 'image'; $type_h[] = 'image'; $type_h[] = 'image'; $type_h[] = 'image';
		}
		...
		while(list($idF, $title, $descr, $num_thread, $num_post, $locked, $emoticons) = mysql_fetch_row($re_forum) ) {
			...
			elseif( isset($_SESSION['unreaded_forum'][$idF])) {

				if(isset($_SESSION['unreaded_forum'][$idF]) && is_array($_SESSION['unreaded_forum'][$idF])) {
					foreach($_SESSION['unreaded_forum'][$idF] as $k => $n_mess)
			...
			if(isset($last_post[$idF])) {
				...
				$content[] = $last_post[$idF]['info'].' ( '.$lang->def('_BY').': <span class="mess_author">'
							.( $authors_names[$author][ACL_INFO_LASTNAME].$authors_names[$author][ACL_INFO_FIRSTNAME] == '' ?
								$authors_names[$author][ACL_INFO_USERID] :
								$authors_names[$author][ACL_INFO_LASTNAME].' '.$authors_names[$author][ACL_INFO_FIRSTNAME] ).'</span> )';
			} else {
			...
			if($mod_perm) {
				...
				$content[] = '<a href="index.php?modname=forum&amp;op=delforum&amp;idForum='.$idF.'&amp;confirm=1" title="'.$lang->def('_DEL').' : '.$title.'">
					<img src="'.getPathImage().'standard/rem.gif" title="'.$lang->def('_DELFORUM').'" alt="'.$lang->def('_DEL').'" /></a>';
			}
			$tb->addBody( $content );
			++$i;
		}
		if($mod_perm) {
			...
			$tb->addActionAdd('<a href="index.php?modname=forum&amp;op=addforum">
		...
		while( list($idF, $title, $descr, $num_thread, $num_post, $locked, $emoticons) = mysql_fetch_row( $re_forum ) ) {
			...
			if($mod_perm) {
				...
				$GLOBALS['page']->add(
					'<li><a href="index.php?modname=forum&amp;op=modforumaccess&amp;idF='.$idF.'">
					...
					.'<li><a href="index.php?modname=forum&amp;op=delforum&amp;idForum='.$idF.'">
						<img src="'.getPathImage().'standard/rem.gif" title="'.$lang->def('_DELFORUM').'" alt="'.$lang->def('_DEL').'" /></a></li>'
				, 'content');
...
function insforum() {
	...
	if(isset($_POST['undo'])) jumpTo('index.php?modname=forum&op=forum');
	if($_POST['title'] == '') {
	...
	$ins_query = "
	INSERT INTO ".$GLOBALS['prefix_cms']."_forum
	( title, description, sequence, emoticons ) VALUES
	( '".$_POST['title']."',
		'".$_POST['description']."',
		'$seq',
		'".$_POST['emoticons']."' )";
	if(!mysql_query( $ins_query )) jumpTo('index.php?modname=forum&op=forum&result=err');


	require_once($GLOBALS['where_framework'].'/lib/lib.eventmanager.php');

	$acl_manager=$GLOBALS["current_user"]->getAclManager();
	$recipients=$acl_manager->getGroupAllUser($acl_manager->getGroupRegisteredId());

	$msg_composer = new EventMessageComposer('forum', 'cms');

	$msg_composer->setSubjectLangText('email', '_NEW_FORUM', false);
	$msg_composer->setBodyLangText('email', '_NEW_FORUM_BODY', array(	'[url]' => $GLOBALS['cms']['url'],
																		'[title]' => $_POST['title'],
																		'[text]' => $_POST['description'] ) );

	$msg_composer->setSubjectLangText('sms', '_NEW_FORUM_SMS', false);
	$msg_composer->setBodyLangText('sms', '_NEW_FORUM_BODY_SMS', array(	'[url]' => $GLOBALS['cms']['url'],
																		'[title]' => $_POST['title'],
																		'[text]' => $_POST['description'] ) );

	createNewAlert('CmsForumNewCategory',
						'forum',
						'addforum',
						1,
						"new forum category",
	...
	jumpTo('index.php?modname=forum&op=forum&result=ok');
}
...
function upforum() {
	...
	WHERE idForum = '".(int)$_POST['idForum']."'";
	if(!mysql_query( $ins_query )) jumpTo('index.php?modname=forum&op=forum&result=err');
	jumpTo('index.php?modname=forum&op=forum&result=ok');
}
...
function moveforum($idForum, $direction) {
	...
	jumpTo('index.php?modname=forum&op=forum');
}
...
function changestatus() {
	...
	WHERE idForum = '".(int)$_GET['idForum']."'");
	jumpTo('index.php?modname=forum&op=thread&idForum='.(int)$_GET['idForum']);
}
...
function delforum() {
	...
	if(isset($_GET['confirm'])) {
		...
		while(list($idT) = mysql_fetch_row($re_thread)) {
			...
			WHERE idThread = '$idT'")) jumpTo('index.php?modname=forum&op=forum&result=err_del');
			unsetNotify('thread', $idT);
		}
		if(!mysql_query("
		DELETE FROM ".$GLOBALS['prefix_cms']."_forumthread
		WHERE idForum= '".$id_forum."'")) jumpTo('index.php?modname=forum&op=forum&result=err_del');
		if(!mysql_query("
		DELETE FROM ".$GLOBALS['prefix_cms']."_forum_access
		WHERE idForum='".$id_forum."'")) jumpTo('index.php?modname=forum&op=forum&result=err_del');
		if(!mysql_query("
		DELETE FROM ".$GLOBALS['prefix_cms']."_forum
		WHERE idForum='".$id_forum."'")) jumpTo('index.php?modname=forum&op=forum&result=err_del');
...
function modforumaccess() {
	...
	$url="index.php?modname=forum&amp;op=modforumaccess&amp;idForum=".$idForum;
	$back_url="index.php?modname=forum&amp;op=forum";
	...
	if (!empty($page_body)) {
		...
		$ta_array=array();
		$ta_array["index.php?modname=forum&amp;op=forum"]=$lang->def("_FORUM");
...
function modforumaccess_ok() {
	...
	$mdir->sel_extend->setExtraSel($perm);
	if (isset($_POST[DIRECTORY_ID]))
		$mdir->sel_extend->setPrintedItems($mdir->getPrintedItems($_POST));

	$idForum=(int)importVar("idForum");
	//$this->docman->requireFolderPerm($tree_id, "addfolder");
	...
	if( isset($_POST['okselector']) ) {  // Saving permissions..

		$selected_items=$mdir->sel_extend->getSelectedItems(); //print_r($selected_items); die();
		$database_items=$mdir->sel_extend->getDatabaseItemsFromVar();

		saveForumPerm($idForum, $selected_items, $database_items);

		jumpTo(str_replace("&amp;", "&", $back_url));
	}
	else if( isset($_POST['cancelselector']) ) { // Undo..
		jumpTo(str_replace("&amp;", "&", $back_url));
	}
	else { // Show selector


		$url="index.php?modname=forum&amp;op=modforumaccess&amp;idForum=".$idForum."&amp;stayon=1";

		if( !isset($_GET['stayon']) ) {
			$saved_perm=loadForumSavedPerm($idForum);
			$mdir->sel_extend->grabSelectedItems($saved_perm);
			$mdir->sel_extend->setDatabaseItems($saved_perm);
		}
		else {
			$mdir->sel_extend->grabSelectedItems($_POST);
		}


		$mdir->loadSelector($url,
		$lang->def( '_FORUM_PERM' ), "", TRUE);
	}
...
function modforumaccess_old() {
	...
	if(isset($_POST['cancelselector'])) jumpTo('index.php?modname=forum&amp;op=forum');
	if(isset($_POST['okselector'])) {
		...
		$query_reader = "
		SELECT idMember
		FROM ".$GLOBALS['prefix_cms']."_forum_access
		WHERE idForum = '".$id_forum."'";
		...
		if(is_array($add_reader)) {
			...
			while(list(, $idst) = each($add_reader)) {
				...
				$query_insert = "
				INSERT INTO ".$GLOBALS['prefix_cms']."_forum_access
	...
	$user_select->setPageTitle(getTitleArea(
		array('index.php?modname=forum&amp;op=forum' => $lang->def('_FORUM'),
		$lang->def('_FORUM_ACCESS')), 'forum'));
...
function thread() {
	...
	$query_thread = "
	SELECT t.idThread, t.author AS thread_author, t.posted, t.title, t.num_post, t.num_view, t.locked, t.erased
	...
	$tb->setColsStyle($type_h);
	$tb->addHead($cont_h);
	while(list($idT, $t_author, $posted, $title, $num_post, $num_view, $locked, $erased) = mysql_fetch_row($re_thread)) {

		$c_css = '';
		// thread author
		$t_author = ( $authors_names[$t_author][ACL_INFO_LASTNAME].$authors_names[$t_author][ACL_INFO_FIRSTNAME] == '' ?
				$authors_names[$t_author][ACL_INFO_USERID] :
		...
		} elseif(isset($_SESSION['unreaded_forum'][$id_forum][$idT])) {
		...
		$content[] = ( $erased && !$mod_perm ?
					...
					'<a'.$c_css.' href="index.php?modname=forum&amp;op=message&amp;idThread='.$idT.'">'.$title.'</a>');
		$content[] = $num_post
			.( isset($_SESSION['unreaded_forum'][$id_forum][$idT]) && $_SESSION['unreaded_forum'][$id_forum][$idT] != 'new_thread'
				? '<br />(<span class="forum_notread">'.$_SESSION['unreaded_forum'][$id_forum][$idT].' '.$lang->def('_NEW').')</span>'
				: ( isset($_SESSION['unreaded_forum'][$id_forum][$idT]) && $_SESSION['unreaded_forum'][$id_forum][$idT] == 'new_thread'
	...
	if(!$locked_f || $mod_perm) {
		$tb->addActionAdd('<a href="index.php?modname=forum&amp;op=addthread&amp;idForum='.$id_forum.'">'
	...
	if($can_notify) {
		...
		$text_inner .= '<li><a href="index.php?modname=forum&amp;op=thread&amp;notify=1&amp;idForum='.$id_forum.'&amp;ini='.$ini_page.'" '
		.( !$is_notify ?
	...
	if($moderate) {
		$text_inner .= '<li><a href="index.php?modname=forum&amp;op=modstatus&amp;idForum='.$id_forum.'">'
			.( $locked_f
...
function insthread() {
	...
	$lang =& DoceboLanguage::createInstance('forum', 'cms');
	$id_forum = importVar('idForum', true , 0);
	if(isset($_POST['undo'])) jumpTo('index.php?modname=forum&op=thread&idForum='.$id_forum);

	list($forum_title) = mysql_fetch_row(mysql_query("
	SELECT title
	FROM ".$GLOBALS['prefix_cms']."_forum
	WHERE AND idForum = '".$id_forum."'"));
	...
	if(!$continue) jumpTo('index.php?modname=forum&op=thread&idForum='.$id_forum.'&amp;result=err_cannotsee');
	if($locked) jumpTo('index.php?modname=forum&op=thread&idForum='.$id_forum.'&amp;result=err_lock');
	if($_POST['title'] == '') $_POST['title'] = substr(strip_tags($_POST['textof']), 0, 50).'...';

	$now = date("Y-m-d H:i:s");
	$ins_query = "
	INSERT INTO ".$GLOBALS['prefix_cms']."_forumthread
	( idForum, title, author, num_post, last_post, posted )
	VALUES (
		'".$id_forum."',
		'".$_POST['title']."',
		'".getLogUserId()."',
		 ...
		 0,
		 '".$now ."' )";
	if(!mysql_query($ins_query)) jumpTo('index.php?modname=forum&op=thread&idForum='.$id_forum.'&amp;result=err_ins');
	...
	if(!mysql_query( $ins_mess_query )) {
		...
		jumpTo('index.php?modname=forum&op=thread&idForum='.$id_forum.'&amp;result=err_ins2');
	}
	...
	launchNotify('forum', $id_forum, $lang->def('_NEW_THREAD'),
		$lang->def('_SUBJECT_NOTIFY_THREAD'),
		$lang->def('_NEW_THREAD_INSERT_IN_FORUM')."\n".$GLOBALS['cms']['url'].' : '.$course_name.' > '.$forum_title.' > '.$_POST['title']
	);

	jumpTo('index.php?modname=forum&op=message&idThread='.$id_thread);
}
...
function upthread() {
	...
	if(isset($_POST['undo'])) {
		...
		if($_POST['search'] == 1) jumpTo('index.php?modname=forum&op=search&amp;ini='.$ini);
		else jumpTo('index.php?modname=forum&op=message&idThread='.$id_thread.'&amp;ini='.$ini);
	}
	...
	if($locked_t ||$erased_t && (!$mod_perm && !$moderate)) {
		if($_POST['search'] == 1) jumpTo('index.php?modname=forum&op=search&amp;ini='.$ini);
		else jumpTo('index.php?modname=forum&op=message&idThread='.$id_thread.'&amp;ini='.$ini.'&amp;result=err_lock');
	}
	if($_POST['title'] == '') $_POST['title'] = substr(strip_tags($_POST['textof']), 0, 50).'...';
	...
	if(!mysql_query($upd_mess_query)) {
		...
		delete_file($name_file);
		if($_POST['search'] == 1) jumpTo('index.php?modname=forum&op=search&amp;ini='.$ini);
		else jumpTo('index.php?modname=forum&op=thread&idForum='.$id_forum.'&amp;result=err_ins');
	}
	...
	WHERE idThread = '".$id_thread."'");
	if($_POST['search'] == 1) jumpTo('index.php?modname=forum&op=search&amp;ini='.$ini);
	else jumpTo('index.php?modname=forum&op=thread&idForum='.$id_forum.'&amp;result=ok');
}
...
function delthread() {
	...
	if(isset($_POST['undo'])) {
		if($_POST['search'] == 1) jumpTo('index.php?modname=forum&op=search&amp;ini='.$ini);
		else jumpTo('index.php?modname=forum&op=thread&idForum='.$id_forum);
	}
	if(isset($_POST['confirm'])) {
		...
		WHERE idThread = '".$id_thread."'"))
			if($_POST['search'] == 1) jumpTo('index.php?modname=forum&op=search&amp;ini='.$ini);
			else jumpTo('index.php?modname=forum&op=thread&idForum='.$id_forum.'&amp;result=err_del');

		if(!mysql_query("
		UPDATE ".$GLOBALS['prefix_cms']."_forum
		SET num_thread = num_thread - 1,
			num_post = num_post - ".$post_deleted
		.( $last_post_forum == $last_post ? " , last_post = '' " : " " )
		." WHERE idForum = '".$id_forum."'"))
			if($_POST['search'] == 1) jumpTo('index.php?modname=forum&op=search&amp;ini='.$ini);
			else jumpTo('index.php?modname=forum&op=thread&idForum='.$id_forum.'&amp;result=err_del');
		...
		if($_POST['search'] == 1) jumpTo('index.php?modname=forum&op=search');
		else jumpTo('index.php?modname=forum&op=thread&idForum='.$id_forum.'&amp;result=ok');
	} else {
...
function message() {
	...
	if(isset($_SESSION['unreaded_forum'][$id_forum][$id_thread])) unset($_SESSION['unreaded_forum'][$id_forum][$id_thread]);
	...
	FROM ".$GLOBALS['prefix_cms']."_forum_sema");
	...
	if(!empty($authors)) {
		...
		FROM ".$GLOBALS['prefix_cms']."_forummessage AS m, ".$GLOBALS['prefix_cms']."_courseuser AS u
		WHERE m.author = u.idUser AND m.author IN ( ".implode($authors, ',')." )
	...
	$path = $GLOBALS['cms']['url'].$GLOBALS['where_files_relative'].'/doceboCms/'.$GLOBALS['cms']['pathphoto'];
	while(list($id_message, $message_info) = each($messages)) {

		// sender info
		$m_author = $message_info['author'];
		
		if($authors_names[$m_author][ACL_INFO_AVATAR] != '') $img_size = @getimagesize($path.$authors_names[$m_author][ACL_INFO_AVATAR]);
		
		$sender = '<div class="forum_author">'
			.( $authors_names[$m_author][ACL_INFO_LASTNAME].$authors_names[$m_author][ACL_INFO_FIRSTNAME] == '' ?
				$authors_names[$m_author][ACL_INFO_USERID] :
				$authors_names[$m_author][ACL_INFO_LASTNAME].' '.$authors_names[$m_author][ACL_INFO_FIRSTNAME] )
			.'</div>'
			...
			.'<a href="index.php?modname=forum&amp;op=viewprofile&amp;idMessage='.$id_message.'&amp;ini='.$ini_page.'">'.$lang->def('_VIEWPROFILE').'</a>';
			...
			if($message_info['modified_by'] != 0) {
				...
				$msgtext .= '<div class="forum_post_modified_by">'
						...
						.( $authors_names[$modify_by][ACL_INFO_LASTNAME].$authors_names[$modify_by][ACL_INFO_FIRSTNAME] == '' ?
							$authors_names[$modify_by][ACL_INFO_USERID] :
							$authors_names[$modify_by][ACL_INFO_LASTNAME].' '.$authors_names[$modify_by][ACL_INFO_FIRSTNAME] )
	...
	if( (!$locked_t && !$locked_f) || $mod_perm || $moderate ) {
		...
		$tb->addActionAdd(
			'<a href="index.php?modname=forum&amp;op=addmessage&amp;idThread='.$id_thread.'&amp;ini='.$ini_page.'" title="'.$lang->def('_ADDMESSAGET').'">'
	...
	if($can_notify) {
		...
		$text_inner .= '<li><a href="index.php?modname=forum&amp;op=message&amp;notify=1&amp;idThread='.$id_thread.'&amp;ini='.$ini_page.'" '
		.( !$is_notify ?
	...
	if($moderate) {
		...
		$text_inner .= '<li><a href="index.php?modname=forum&amp;op=modstatusthread&amp;idThread='.$id_thread.'&amp;ini='.$ini_page.'">'
			.( $locked_t
	...
	if($mod_perm) {
		...
		$text_inner .= '<li><a href="index.php?modname=forum&amp;op=changeerased&amp;idThread='.$id_thread.'&amp;ini='.$ini_page.'">'
			.( $erased_t
...
function moderatemessage() {
	...
	jumpTo('index.php?modname=forum&op=message&idThread='.$id_thread.'&ini='.$_GET['ini']);
}
...
function showMessageForAdd($id_thread, $how_much) {
	...
	while(list($id_message, $message_info) = each($messages)) {
		...
		$sender = '<div class="forum_author">'
			...
			.'</div>'
			.'<img src="'.getPathImage().'standard/user.gif" alt="&gt;" />&nbsp;'
			.'<a href="index.php?modname=forum&amp;op=viewprofile&amp;idMessage='.$id_message.'">'.$lang->def('_VIEWPROFILE').'</a>';
		...
		if($message_info['locked']) {
			...
			if($message_info['modified_by'] != 0) {
				...
				$msgtext .= '<div class="forum_post_modified_by">'
						...
						.( $authors_names[$modify_by][ACL_INFO_LASTNAME].$authors_names[$modify_by][ACL_INFO_FIRSTNAME] == '' ?
							$authors_names[$modify_by][ACL_INFO_USERID] :
							$authors_names[$modify_by][ACL_INFO_LASTNAME].' '.$authors_names[$modify_by][ACL_INFO_FIRSTNAME] ).'</div>';
			}
			if($authors_names[$m_author][ACL_INFO_SIGNATURE] != '') {
				$msgtext .= '<div class="forum_post_sign_separator"></div>'
					.'<div class="forum_post_sign">'
...
function addmessage() {
	...
	if($id_message != 0) {
		...
		FROM ".$GLOBALS['prefix_cms']."_forummessage
		WHERE idMessage = '".$id_message."'";
		list($m_title, $m_textof) = mysql_fetch_row(mysql_query($message_query));
	}
	$GLOBALS['page']->add(
		getTitleArea($page_title, 'forum')
		.'<div class="std_block">'
		...
		.Form::getTextarea($lang->def('_TEXTOF'), 'textof', 'textof', ($id_message != '' ? '[quote]'.$m_textof.'[/quote]' : '' ))
	, 'content');
	if(checkPerm('upload', true)) {
...
function insmessage() {
	...
	if(isset($_POST['undo'])) jumpTo('index.php?modname=forum&op=message&idThread='.$id_thread.'&amp;ini='.$ini);
	...
	if(!$continue) jumpTo('index.php?modname=forum&op=message&idThread='.$id_thread.'&amp;ini='.$ini.'&amp;result=err_cannotsee');
	if($locked_f || $locked_t ||$erased_t && (!$mod_perm && !$moderate)) {
		jumpTo('index.php?modname=forum&op=message&idThread='.$id_thread.'&amp;ini='.$ini.'&amp;result=err_lock');
	}
	if($_POST['title'] == '') $_POST['title'] = substr(strip_tags($_POST['textof']), 0, 50).'...';
	...
	if(!mysql_query($ins_mess_query)) {
		...
		delete_file($name_file);
		jumpTo('index.php?modname=forum&op=message&idThread='.$id_thread.'&amp;ini='.$ini.'&amp;result=err_ins');
	}
	...
	launchNotify('thread', $id_thread, $lang->def('_NEW_MESSAGE'),
		$lang->def('_SUBJECT_NOTIFY_MESSAGE'),
		$lang->def('_NEW_MESSAGE_INSERT_IN_THREAD')."\n".$GLOBALS['cms']['url']." : ".$course_name.' > '.$forum_title.' > '.$thread_title
	);
	jumpTo('index.php?modname=forum&op=message&idThread='.$id_thread.'&amp;ini='.$ini.'&amp;result=ok');
}
...
function delmessage() {
	...
	$thread_query = "
	SELECT idForum, title, num_post
	FROM ".$GLOBALS['prefix_cms']."_forumthread
	WHERE idThread = '".$id_thread."'";
	list($id_forum, $thread_title, $num_post) = mysql_fetch_row(mysql_query($thread_query));
	...
	if(isset($_POST['confirm'])) {
		...
		WHERE answer_tree LIKE '".$answer_tree."/%'"))
			jumpTo('index.php?modname=forum&op=message&idThread='.$id_thread.'&amp;result=err_del');
		...
		jumpTo('index.php?modname=forum&op=message&idThread='.$id_thread.'&amp;result=ok');
	} else {
...
function viewprofile() {
	...
	require_once($GLOBALS['where_cms'].'/lib/lib.profile.php');
	$lang =& DoceboLanguage::createInstance('forum', 'cms');
	...
	$page_title = array(
		'index.php?modname=forum&amp;op=forum' => $lang->def('_FORUM'),
		$lang->def('_VIEW_PROFILE')
	);
	$GLOBALS['page']->add(
		getTitleArea($page_title, 'forum')
		.'<div class="std_block">'
		.getBackUi('index.php?modname=forum&amp;op=message&amp;idThread='.$id_thread.'&amp;ini='.$ini, $lang->def('_BACK'))
		.getprofile($idst_user)
		.getBackUi('index.php?modname=forum&amp;op=message&amp;idThread='.$id_thread.'&amp;ini='.$ini, $lang->def('_BACK'))
		.'</div>', 'content');
}
...
function editsema() {
	...
	$query_sema = "
	SELECT *
	FROM ".$GLOBALS['prefix_cms']."_pagel_sema
	ORDER BY title";
	...
	$GLOBALS['page']->add(
		...
		.Form::openForm('editsema', 'index.php?modname=forum&amp;op=savesema')
		.Form::openElementSpace()
		.Form::getHidden('idMessage', 'idMessage', $id_message)
		.Form::getHidden('ini', 'ini', $ini)
	, 'content');
	if(mysql_num_rows($re_sema) > 0) {
		...
		while($pagel_sema = mysql_fetch_assoc($re_sema)) {
			...
			$query_item = "
			SELECT id, title
			FROM ".$GLOBALS['prefix_cms']."_pagel_sema_items
...
function forumsearch() {
	...
	while(list($idT, $id_forum, $t_author, $posted, $title, $num_post, $num_view, $locked, $erased) = mysql_fetch_row($re_thread)) {
		...
		$content[] = ( $erased && !$mod_perm ?
					...
					.eregi_replace($search_arg, '<span class="filter_evidence">'.$search_arg.'</span>', $title).'</a>');
		$content[] = $num_post
			.( isset($_SESSION['unreaded_forum'][$id_forum][$idT]) && $_SESSION['unreaded_forum'][$id_forum][$idT] != 'new_thread'
				? '<br />(<span class="forum_notread">'.$_SESSION['unreaded_forum'][$id_forum][$idT].' '.$lang->def('_NEW').')</span>'
				: '' );
...
function launchNotify($notify_is_a, $id_notify, $description, $subject, $body) {
...
function forumDispatch($op) {
	...
	switch($op) {
		...
		case "download" : {
			...
			$path = $GLOBALS['where_files_relative'].'/doceboCms/'.$GLOBALS['cms']['pathforum'];
			//send file
