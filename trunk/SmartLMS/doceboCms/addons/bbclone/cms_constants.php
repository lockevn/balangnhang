 ...
 *
 * $Header: /cvs/bbclone/constants.php,v 1.108 2006/12/27 17:01:41 christoph Exp $
 *
...
$BBC_IMAGES_PATH = $GLOBALS["where_cms_relative"]."/addons/bbclone/images/";
...
// Added for DoceboCMS:
require_once($BBC_ROOT_PATH."cms_set_globals.php");
...
function bbEchoInDocebo($text) {
	$GLOBALS["page"]->add($text, "content");
}


// MAIN [edited for the CMS Admin]

function getPageViewArray() {
  global $_, $access;

  $stat['last_year'] 	= (!empty($access['time']['month']) ? bbc_histcalc($access['time']['month']) : "0");
  $stat['last_month'] 	= ((!empty($access['time']['day'])) ? bbc_histcalc($access['time']['day']) : "0");
  $stat['last_week'] 	= (!empty($access['time']['wday']) ? bbc_histcalc($access['time']['wday']) : "0");
  $stat['last_day'] 	= (!empty($access['time']['wday']) ? bbc_histcalc($access['time']['hour']) : "0");
  $stat['totalvisit'] 	= (!empty($access['stat']['totalvisits']) ? $access['stat']['totalvisits'] : "0");
  $stat['totalunique'] 	= (!empty($access['stat']['totalcount']) ? $access['stat']['totalcount'] : "0");
  return $stat;
