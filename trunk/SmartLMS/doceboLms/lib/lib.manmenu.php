...
function cerateCourseMenuFromCustom($id_custom, $id_course, $group_idst) {
	...
	$main_values = array();
	$query_ins_main = "
	INSERT INTO ".$GLOBALS['prefix_lms']."_menucourse_main (idCourse, sequence, name, image ) VALUES";
	while(list($id_main, $seq, $name, $image) = mysql_fetch_row($re_main)) {
		
		if(!mysql_query($query_ins_main." ( '".$id_course."','".$seq."', '".$name."', '".$image."')") ) {
	...
	$re = true;
	$query_ins_module = "
	INSERT INTO ".$GLOBALS['prefix_lms']."_menucourse_under ( idCourse, idModule, idMain, sequence, my_name ) VALUES";
	while(list($id_module, $id_main, $seq, $my_name) = mysql_fetch_row($re_module)) {
		
		if(isset($map_main_id[$id_main]) && ($map_main_id[$id_main] !== false)) {
			
			$re &= mysql_query($query_ins_module
...
function getAllCustom() {
	...
	$customs = array();
	while(list($id, $name) = mysql_fetch_row($re_custom)) {
		$customs[$id] = $name;
	}
	return $customs;
}
...
function getIdCustomFromMain($id_main) {
	...
	list($id_custom) = mysql_fetch_row(mysql_query($query_custom));
	return $id_custom;
}
