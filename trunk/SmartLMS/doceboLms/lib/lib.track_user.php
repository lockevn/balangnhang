...
class TrackUser {
	...
	function createSessionCourseTrack() {
		...
		list($last_course_access) = mysql_fetch_row(mysql_query("
	...
	function setActionTrack($id_user, $id_course, $mod_name, $mode) {
		...
		if($GLOBALS['lms']['tracking'] == 'on' && $_SESSION['levelCourse'] != '2') {
	...
	function closeSessionCourseTrack() {
		...
		TrackUser::setActionTrack(getLogUserId(), $_SESSION['idCourse'], '_ELECOURSE', 'view');
	}
	...
	function getUserTotalCourseTime($idst_user, $id_course){
		...
		$re = mysql_query($query_time);
		if($re && mysql_num_rows($re)) list($tot_time) = mysql_fetch_row(mysql_query($query_time));
	...
	static function getUserPreviousSessionCourseTime($idst_user, $id_course){
		...
		$re = mysql_query($query_time);
		if($re && mysql_num_rows($re)) list($tot_time) = mysql_fetch_row($re);
	...
	static function getUserCurrentSessionCourseTime($id_course) {
		...
		if(isset($_SESSION['id_enter_course'])) {
			...
			WHERE idCourse = '".$id_course."' AND idUser = '".getLogUserId()."' 
				AND idEnter = '".$_SESSION['id_enter_course']."'";
			list($partial_time) = mysql_fetch_row(mysql_query($query_time));
	...
	static function getWhoIsOnline($id_course, $gap_minute = 5) {
		...
		list($who_is_online) = mysql_fetch_row(mysql_query($query_time));
	...
	function getLastAccessToCourse($id_user) {
		...
		while(list($id_c, $access) = mysql_fetch_row($re_time)) {
	...
	static function checkSession($id_user) {
		...
		if(isset($_SESSION['id_enter_course'])) {
			...
			WHERE idUser = '".$id_user."' AND idEnter = '".$_SESSION['id_enter_course']."' "
					." AND active = 1";
			list($num_active) = mysql_fetch_row(mysql_query($query_time));
