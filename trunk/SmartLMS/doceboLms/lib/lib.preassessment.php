...
class AssessmentList {
	...
	function fetch_row($re) 		{ return mysql_fetch_row($re); }
	...
	function saveAssessment($id_assessment, $assessment_data) {
		...
		if($id_assessment == 0) {
			...
			$id_main = $this->man_course->addMainToCourse($id_course, def('_PREASSESSMENT_MENU', 'preassessment', 'framework'));
...
class AssessmentRule {
	...
	function fetch_row($re) 		{ if($re === false) return false; return mysql_fetch_row($re); }
