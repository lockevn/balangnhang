...
class Course_Manager {
	...
	function loadCourseSelector() {
		...
		$this->course_selector->loadCourseSelector();
	}
	...
	function loadCoursePathSelector() {
		...
		$this->coursepath_selector->loadCoursePathSelector();
	}
	...
	function loadCatalogueSelector() {
		...
		$this->catalogue_selector->loadCatalogueSelector();
	}
	...
	function loadSelector($show_tabs = true) {
		...
		$GLOBALS['page']->addStart($this->tab->printTabView_Begin('', false), 'content');
		$GLOBALS['page']->addEnd($this->tab->printTabView_End(), 'content');
		...
		switch($active_tab) {
			...
			case "tab_coursepath": {
				...
				$this->loadCoursePathSelector();
			};break;
			case "tab_catalogue": {
				$this->_loadCatalogueStatus($active_tab, $old_tab);
				$this->loadCatalogueSelector();
			};break;
		}
		$GLOBALS['page']->add(
			$this->_saveCourseStatus($active_tab, $old_tab)
			.$this->_saveCoursePathStatus($active_tab, $old_tab)
			.$this->_saveCatalogueStatus($active_tab, $old_tab)
			.Form::getHidden('old_tab', 'old_tab', $active_tab), 'content');
	}
}
...
class AdminCourseManagment {
	...
	function &getUserPathCourses($id_user) {
		...
		while(list($id) = mysql_fetch_row($re_coursepath)) {
	...
	function &getUserCatalogues($id_user) {
		...
		while(list($id) = mysql_fetch_row($re_catalogue)) {
	...
	function getCoursesStats() {
		...
		if($GLOBALS['current_user']->getUserLevelId() != ADMIN_GROUP_GODADMIN) {
			...
			if(empty($course_array)) {
				...
				if($GLOBALS['lms']['on_catalogue_empty'] == 'on') return $course_stats;
			} else {
				$where .= " AND idCourse IN ( ".implode(',', $course_array)." )";
			}
		}
		
		list($course_stats['total']) = mysql_fetch_row(mysql_query($query_course));
		list($course_stats['active']) = mysql_fetch_row(mysql_query($query_course
			.$where." AND status = '1'"));
		
		list($course_stats['active_seven']) = mysql_fetch_row(mysql_query($query_course
			.$where." AND date_begin > '".date("Y-m-d H:i:s")."' AND date_begin < '".date("Y-m-d", time() + 7*24*3600)." 23:59:59'"));
		list($course_stats['deactive_seven']) = mysql_fetch_row(mysql_query($query_course
			.$where." AND date_end > '".date("Y-m-d H:i:s")."' AND date_end < '".date("Y-m-d", time() + 7*24*3600)." 23:59:59'"));
		
		$query_subscribe = "
		SELECT `waiting`, COUNT(*)
		FROM ".$GLOBALS['prefix_lms']."_courseuser ";
		$re_query = mysql_query($query_subscribe
			.$where
			." GROUP BY `waiting`");
		
		while(list($wait_stat, $number) = mysql_fetch_row($re_query)) {;
