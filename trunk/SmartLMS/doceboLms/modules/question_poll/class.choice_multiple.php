...
class ChoiceMultiple_Question extends Question {
	...
	function create( $id_poll, $back_poll ) {
		...
		if(isset($_POST['add_question'])) {
			...
			if(!$id_quest) {
				errorCommunication($lang->def('_POLL_ERR_INS_ANSWER').getBackUi(ereg_replace('&', '&amp;', $back_poll), $lang->def('_BACK')));
			}
			//insert answer
	...
	function edit( $back_poll ) {
		...
		if(!isset($_POST['answer_id'])) {
			...
			$i_load = 0;
			while(list($_POST['answer_id'][$i_load], $_POST['answer'][$i_load]) = mysql_fetch_row($re_answer)){
				++$i_load;
			}
			$num_answer = $i_load;
		}
	...
	function copy( $new_id_poll, $back_poll = NULL ) {
		...
		WHERE id_quest = '".(int)$this->id."'
		ORDER BY id_answer");
		while(list($id_answer, $answer, $seq) = mysql_fetch_row($re_answer)) {
