...
class Choice_Question extends Question {
	...
	function _lineAnswer($i) {
		...
		$GLOBALS['page']->add('<tr class="line_answer">'
			.'<td class="image">'
			//answer
			.'<label class="access-only" for="answer_'.$i.'">'.$lang->def('_POLL_TEXT_ANSWER').'</label>'
	...
	function create( $id_poll, $back_poll ) {
		...
		if(isset($_POST['add_question'])) {
			...
			if(!$id_quest) {
				$GLOBALS['page']->add(getErrorUi($lang->def('_POLL_ERR_INS_ANSWER').getBackUi(ereg_replace('&', '&amp;', $back_poll), $lang->def('_BACK'))), 'content');
			}
			//insert answer
		...
		$GLOBALS['page']->add(
			...
			.Form::getDropdown( $lang->def('_POLL_QUEST_CATEGORY'), 'id_category', 'id_category', $categories,
		...
		$GLOBALS['page']->add('<table class="test_answer" cellspacing="0" summary="'.$lang->def('_POLL_ANSWER_SUMMARY').'">'."\n"
			.'<caption>'.$lang->def('_POLL_ANSWER').'</caption>'."\n"
			.'<tr>'
			.'<th>'.$lang->def('_POLL_TEXT_ANSWER').'</th>'
			.'</tr>'."\n", 'content');
		for($i = 0; $i < $num_answer; $i++) {
			$this->_lineAnswer($i);
		}
	...
	function edit( $back_poll ) {
			...
			for($i = 0; $i < $num_answer; $i++) {
				...
				if( isset($_POST['answer_id'][$i]) ) {
					...
					if(!mysql_query($upd_ans_query)) {
						...
						getErrorUi($lang->def('_POLL_ERR_INS_ANSWER').getBackUi(ereg_replace('&', '&amp;', $back_poll), $lang->def('_BACK')));
					}
				} else {
					//insert new answer
					$ins_answer_query = "
		...
		if(!isset($_POST['answer_id'])) {
			list($sel_cat, $quest) = mysql_fetch_row(mysql_query("
			...
			while(list(
				...
				$_POST['answer'][$i_load] ) = mysql_fetch_row($re_answer)){
				++$i_load;
			}
			$num_answer = $i_load;
		}
	...
	function copy( $new_id_poll, $back_poll = NULL ) {
		...
		WHERE id_quest = '".(int)$this->id."'
		ORDER BY id_answer");
		while(list($answer, $sequence) = mysql_fetch_row($re_answer)) {
	...
	function play( $num_quest, $shuffle_answer = false, $id_track = 0, $freeze = false ) {
		...
		if($id_track != 0) {
			...
			if(mysql_num_rows($re_answer_do)) {
				...
				$find_prev = true;
				list($id_answer_do) = mysql_fetch_row($re_answer_do);
			}
		}
		...
		if ($GLOBALS['lms']['no_answer_in_poll'] == 'on')
		{
			$content .=  '<input type="radio" id="quest_'.$id_quest.'_0" '
					...
					.( $find_prev && $freeze ? ' disabled="disabled"' : '' ).' /> '
					.'<label class="text_answer_none" for="quest_'.$id_quest.'_0">'.$lang->def('_QUEST_IDONTWANTTO').'</label>';
		}
		$content .= '</div>'
				.'</div>';
		return $content;
	...
	function playReport( $num_quest, $tot_tracks, &$valid_track ) {
		...
		//find previous answer
		while(list($id_a, $num) = mysql_fetch_row($re_answer_do)) {
		...
		if ($GLOBALS['lms']['no_answer_in_test'] == 'on')
		{
			$content .=  '<tr><td>'.$lang->def('_QUEST_IDONTWANTTO').'</td><td>'
				.( $max != 0 
	...
	function export_CSV( $num_quest, $tot_tracks, &$valid_track ) {
		...
		if ($GLOBALS['lms']['no_answer_in_test'] == 'on')
		{
			$content .=  ';"'.$lang->def('_QUEST_IDONTWANTTO').'";'.( $not_answer != 0 ? $not_answer : '' ).''
				/*.( $max != 0 
