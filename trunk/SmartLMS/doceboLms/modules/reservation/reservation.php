...
if(!$GLOBALS['current_user']->isAnonymous())
{
	...
	function reservation()
	{
		...
		$out->add(getTitleArea('_RESERVATION').'<div class="std_block">', 'content');
			...
			foreach($events as $event)
			{
				...
				if ($can_view || $mod_perm)
				{
					if ($temp_cat != $event[EVENT_CATEGORY_NAME])
					{
						...
						$tb = new TypeOne(100000, $table_caption);
						$tb->setColsStyle($type_h);
						$tb->addHead($cont_h);
					}
					...
					$count[] = $GLOBALS['regset']->databaseToRegional($event[EVENT_DATE], 'date');
					...
					if($event[EVENT_AVAILABLE_PLACES] > 0)
						...
						$count[] = '0';
					$count[] = $GLOBALS['regset']->databaseToRegional($event[EVENT_DEADLINE], 'date');
					...
					else
					{
						if ($event[EVENT_AVAILABLE_PLACES] > 0)
						{
							if ($man_res->controlMaxSubscriptionForCategory($event[EVENT_ID_CATEGORY], getLogUserId()))
								$count[] = '<a href="index.php?modname=reservation&amp;op=add_subscription&amp;id_event='.$event[EVENT_ID].'">'.$lang->def('_REGISTER_ME').'</a>';
							else
							{
	...
	function subscribedUser()
	{
		...
		(
			...
			.Form::getButton('add_event', 'add_event', $lang->def('_ADD_EVENT'))
		...
		if($events)
		{
			...
			if ($mod_perm)
			{
				...
				(
					...
					$lang->def('_NUMBER_SUBSCRIBED'),
					(isset($_GET['order_by']) && $_GET['order_by'] == 'c.name, e.deadLine' ? '<a href="index.php?modname=reservation&op=reservation&amp;active_tab=subscribed_user&amp;order_by=c.name, e.deadLine DESC">'.$lang->def('_DEADLINE').'</a>' : '<a href="index.php?modname=reservation&op=reservation&amp;active_tab=subscribed_user&amp;order_by=c.name, e.deadLine">'.$lang->def('_DEADLINE').'</a>'),
					'<img src="'.getPathImage().'/standard/user.gif" title="'.$lang->def('_VIEW_USER_SUBSCRIBED').'" alt="'.$lang->def('_ENROL_COUNT').'" />',
					'<img src="'.getPathImage().'/standard/add_subscribe.gif" title="'.$lang->def('_ADD_USER').'" alt="'.$lang->def('_ALT_ADD_USER').'" />',
					'<img src="'.getPathImage().'/message/send_upper.gif" title="'.$lang->def('_ALT_SEND_MAIL').'" alt="'.$lang->def('_ALT_SEND_MAIL').'" />',
					'<img src="'.getPathImage().'/standard/mod.gif" title="'.$lang->def('_MOD').'" alt="'.$lang->def('_MOD').'" />',
					'<img src="'.getPathImage().'/standard/rem.gif" title="'.$lang->def('ALT_DEL').'" alt="'.$lang->def('_DEL').'" />',
					''
				);
			}
			...
			foreach($events as $event)
			{
				...
				if ($mod_perm)
				{
					$count[] = '<a href="index.php?modname=reservation&amp;op=view_user_event&amp;id_event='.$event[EVENT_ID].'"><img src="'.getPathImage().'/standard/user.gif" title="'.$lang->def('_VIEW_USER_SUBSCRIBED').'" alt="'.$lang->def('_ENROL_COUNT').'" /></a>';
					$count[] = '<a href="index.php?modname=reservation&amp;op=add_registration&amp;id_event='.$event[EVENT_ID].'&amp;id_course='.$event[EVENT_ID_COURSE].'"><img src="'.getPathImage().'/standard/add_subscribe.gif" title="'.$lang->def('_ADD_USER').'" alt="'.$lang->def('_ALT_ADD_USER').'" /></a>';
					$count[] = '<a href="index.php?modname=reservation&amp;op=send_mail&amp;id_event='.$event[EVENT_ID].'&amp;id_course='.$event[EVENT_ID_COURSE].'"><img src="'.getPathImage().'/message/send_upper.gif" title="'.$lang->def('_ALT_SEND_MAIL').'" alt="'.$lang->def('_ALT_SEND_MAIL').'" /></a>';
					$count[] = '<a href="index.php?modname=reservation&amp;op=mod_event&amp;id_event='.$event[EVENT_ID].'"><img src="'.getPathImage().'/standard/mod.gif" title="'.$lang->def('_MOD').'" alt="'.$lang->def('_MOD').'" /></a>';
					$count[] = '<a href="index.php?modname=reservation&amp;op=del_event&amp;id_event='.$event[EVENT_ID].'"><img src="'.getPathImage().'/standard/rem.gif" title="'.$lang->def('_DEL').'" alt="'.$lang->def('_DEL').'" /></a>';
					$count[] = '<a href="index.php?modname=reservation&amp;op=set_room_view_perm&amp;id_event='.$event[EVENT_ID].'"><img src="'.getPathImage().'chat/user_perm.gif" alt="'.$lang->def('_SET_ROOM_VIEW_PERM').'" title="'.$lang->def('_SET_ROOM_VIEW_PERM').'" /></a>';
				}
				$tb->addBody($count);
			}
			$out->add($tb->getTable()
	...
	function viewUserEvent()
	{
		...
		$out->add
		(
			getTitleArea($lang->def('_VIEW_EVENT_USER'), '', $lang->def('_ALT_EVENT'))
			.'<div class="std_block">'
		);
		...
		if ($user_info)
		{
			$tb = new TypeOne(10, $lang->def('_RESERVATION_CAPTION'), $lang->def('_RESERVATION_SUMMARY'));
			...
			$out->add('<br/>');
			$out->add('<a href="index.php?modname=reservation&amp;op=excel&amp;id_event='.$id_event.'" target="_blank">'.$lang->def('_GET_EXCEL_FILE').'</a>', 'content');
		}
		else
		{
	...
	function getExcelFile()
	{
		...
		$excel_header = '"'.$lang->def('_USER_ID').'"'."\t";
				...
				foreach ($extra_field as $field_extra)
				{
					...
					switch ($field_type)
					{
						case 'country':
							...
							list($value) = mysql_fetch_row(mysql_query($query_value));
						...
						case 'date':
							$array_field[] = $GLOBALS['regset']->databaseToRegional($res, 'datetime');
						break;
	...
	function addSubscription()
	{
		...
		else
		{
			...
			$out->add
			(
				getTitleArea($lang->def('_ADD_SUBSCRIPTION_TITLE'), '', $lang->def('_ALT_EVENT'))
				.'<div class="std_block">'
	...
	function delSubscription()
	{
		...
		$GLOBALS['page']->add
		(
			getTitleArea($lang->def('_DEL_SUBSCRIPTION_TITLE'), '', $lang->def('_ALT_EVENT'))
			.'<div class="std_block">'
	...
	function switchSubscription()
	{
		...
		$out->add
		(
			getTitleArea($lang->def('_SWITCH_REGISTRATION_TITLE'), '', $lang->def('_ALT_EVENT'))
			.'<div class="std_block">'
		);
	...
	function sendRegistration()
	{
		...
		if ($wrong_result)
			jumpTo('index.php?modname=reservation&op=reservation&active_tab=subscribed_user&error=insert');
		jumpTo('index.php?modname=reservation&op=reservation&active_tab=subscribed_user');
	}
	...
	function addEvent()
	{
		...
		if (isset($_GET['confirm']))
		{
			...
			$date = $GLOBALS['regset']->regionalToDatabase($date, 'date');
			
			$deadline = $GLOBALS['regset']->regionalToDatabase($deadline, 'date');
			...
			if ($date < date('Y-m-d') || $date < $deadline || $deadline < date('Y-m-d'))
				jumpTo('index.php?modname=reservation&op=add_event&amp;error=date&id_laboratory='.$id_laboratory.'&id_category='.$id_category.'&title='.$title.'&description='.$description.'&date='.$_POST['date'].'&max_user='.$max_user.'&deadline='.$_POST['deadline']);
			
			if ($from_time >= $to_time)
				jumpTo('index.php?modname=reservation&op=add_event&amp;error=time&id_laboratory='.$id_laboratory.'&id_category='.$id_category.'&title='.$title.'&description='.$description.'&date='.$_POST['date'].'&max_user='.$max_user.'&deadline='.$_POST['deadline']);
			
			if ($confirm)
				$result = $man_res->addEvents($id_course, $id_laboratory, $id_category, $title, $description, $date, $max_user, $deadline, $from_time, $to_time);
			if ($result)
				jumpTo('index.php?modname=reservation&op=reservation&active_tab=subscribed_user');
			jumpTo('index.php?modname=reservation&op=add_event&amp;error=laboratory&id_laboratory='.$id_laboratory.'&id_category='.$id_category.'&title='.$title.'&description='.$description.'&date='.$_POST['date'].'&max_user='.$max_user.'&deadline='.$_POST['deadline']);
		}
		
		$out->add
		(
			getTitleArea($lang->def('_ADD_EVENT'), '', $lang->def('_ALT_EVENT'))
			.'<div class="std_block">'
		);
		...
		(
			...
			.Form::closeElementSpace()
			.Form::openButtonSpace()
			.Form::getButton('send_event', 'send_event', $lang->def('_SEND_EVENT'))
	...
	function modEvent()
	{
		...
		if (isset($_GET['confirm']))
		{
			...
			if ($date < date('Y-m-d') || $date < $deadline || $deadline < date('Y-m-d'))
				jumpTo('index.php?modname=reservation&op=mod_event&amp;id_event='.$id_event.'&amp;error=date');
			
			if ($from_time >= $to_time)
				jumpTo('index.php?modname=reservation&op=mod_event&amp;id_event='.$id_event.'&amp;error=time');
			...
			if ($result)
				jumpTo('index.php?modname=reservation&op=reservation&active_tab=subscribed_user');
			jumpTo('index.php?modname=reservation&op=mod_event&amp;id_event='.$id_event.'&amp;error=laboratory');
		}
		
		$out->add
		(
			getTitleArea($lang->def('_SAVE'), '', $lang->def('_ALT_EVENT'))
			.'<div class="std_block">'
		);
		...
		$date = $GLOBALS['regset']->databaseToRegional($event[EVENT_DATE], 'date');
		$deadline = $GLOBALS['regset']->databaseToRegional($event[EVENT_DEADLINE], 'date');
	...
	function delEvent()
	{
		...
		if (isset($_GET['confirm']))
		{
			...
			if ($result)
				...
				jumpTo('index.php?modname=reservation&op=reservation&amp;error=del_event');
		}
		
		$GLOBALS['page']->add
		(
			getTitleArea($lang->def('_DEL_EVENT_TITLE'), '', $lang->def('_ALT_EVENT'))
			.'<div class="std_block">'
			.getDeleteUi(	$lang->def('_AREYOUSURE_DEL_EVENT'),
							$lang->def('_DEL_EVENT_INFO'),
							true,
		...
		$tb = new TypeOne(10, $lang->def('_RESERVATION_CAPTION'), $lang->def('_RESERVATION_SUMMARY'));
			...
			foreach($category as $categ)
			{
				...
				if ($mod_perm)
				{
					...
					{
						$count[] = '<a href="index.php?modname=reservation&amp;op=mod_category&amp;id_category='.$categ[CATEGORY_ID].'"><img src="'.getPathImage().'/standard/mod.gif" title="'.$lang->def('_MOD').'" alt="'.$lang->def('_MOD').'" /></a>';
						$count[] = '<a href="index.php?modname=reservation&amp;op=del_category&amp;id_category='.$categ[CATEGORY_ID].'"><img src="'.getPathImage().'/standard/rem.gif" title="'.$lang->def('_DEL').'" alt="'.$lang->def('_DEL').'" /></a>';
					}
					else
					{
						$count[] = '<img src="'.getPathImage().'/standard/mod.gif" title="'.$lang->def('_MOD').'" alt="'.$lang->def('_MOD').'" />';
						$count[] = '<img src="'.getPathImage().'/standard/rem.gif" title="'.$lang->def('ALT_DEL').'" alt="'.$lang->def('_DEL').'" />';
					}
				}
				$tb->addBody($count);
			}
			if($mod_perm)
			{
				$tb->addActionAdd('<a href="index.php?modname=reservation&amp;op=add_category&amp;id_course='.$id_course.'">
					<img src="'.getPathImage().'standard/add.gif" title="'.$lang->def('_ADD_CATEGORYT').'" alt="'.$lang->def('_ADD').'" /> '
					.$lang->def('_ADD_CATEGORY').'</a>');
			}
			$out->add($tb->getTable()
	...
	function addCategoy()
	{
		...
		if (isset($_GET['confirm']))
		{
			...
			if ($name == '')
				jumpTo('index.php?modname=reservation&op=add_category&error=name&id_course='.$id_course.'&name='.$name.'&max_subscription='.$max_subscription);
			
			if ($confirm)
				$result = $man_res->addCategory($name, $max_subscription, $id_course);
			if ($result)
				jumpTo('index.php?modname=reservation&op=view_category');
			jumpTo('index.php?modname=reservation&op=add_category&error=category&id_course='.$id_course.'&name='.$name.'&max_subscription='.$max_subscription);
		}
		
		$out->add
		(
			getTitleArea($lang->def('_ADD_CATEGORY'), '', $lang->def('_ALT_CATEGORY'))
			.'<div class="std_block">'
		);
		...
		(
			...
			.Form::closeElementSpace()
			.Form::openButtonSpace()
			.Form::getButton('send_category', 'send_category', $lang->def('_SEND_CATEGORY'))
	...
	function modCategoy()
	{
		...
		if (isset($_GET['confirm']))
		{
			...
			if ($name == '')
				jumpTo('index.php?modname=reservation&op=mod_category&amp;error=name&amp;id_category='.$id_category);
			
			if ($confirm)
				$result = $man_res->modCategory($id_category, $name, $max_subscription);
			if ($result)
				jumpTo('index.php?modname=reservation&op=view_category');
			jumpTo('index.php?modname=reservation&op=mod_category&amp;id_category='.$id_category.'&amp;error=category');
		}
		
		$out->add
		(
			getTitleArea($lang->def('_MOD_CATEGORY'), '', $lang->def('_ALT_CATEGORY'))
			.'<div class="std_block">'
		);
		...
		if ($error !== '')
		{
			switch ($error)
			{
				...
				case 'category':
					$out->add(getErrorUi($lang->def('_WRONG_MOD_CATEGORY')));
				break;
			} 
		}
		...
		(
			...
			.Form::closeElementSpace()
			.Form::openButtonSpace()
			.Form::getButton('mod_category', 'mod_category', $lang->def('_MOD_CATEGORY'))
			.Form::getButton('undo_cat', 'undo_cat', $lang->def('_UNDO'))
			.Form::closeButtonSpace()
			.Form::closeForm()
		);
	...
	function delCategoy()
	{
		...
		if (isset($_GET['confirm']))
		{
			...
			if ($result)
				...
				jumpTo('index.php?modname=reservation&op=view_category&error=del_category');
		}
		
		$GLOBALS['page']->add
		(
			getTitleArea($lang->def('_DEL_CATEGORY_TITLE'), '', $lang->def('_ALT_CATEGORY'))
			.'<div class="std_block">'
			.getDeleteUi(	$lang->def('_AREYOUSURE_DEL_CATEGORY'),
	...
	function saveclassroom() {
		...
		if($load == 1) {
			...
			jumpTo('index.php?modname=reservation&op=classroom&result=ok');
			echo $query_insert;
		} else {
	...
	function delclassroom() {
		...
		if(isset($_POST['confirm'])) {
			...
			else jumpTo('index.php?modname=reservation&op=classroom&result=ok');
		} else {
...
function reservationSendMail()
{
	...
	{
		$mail = array();
		$mail = $man_res->getEventUserMail($id_event);
		
		$subject = importVar('subject', false, '[Nessun Oggetto]');
		$body = importVar('body', false, '');
		$user_info = $acl_man->getUser(getLogUserId());
		if ($user_info)
		{
			foreach ($user_info as $info_user)
			{
		...
		sendMail($mail, $subject, $body, $sender);
		//_sendMail($subject, $body, $mail, $sender);
		
		jumpTo('index.php?modname=reservation&op=reservation&active_tab=events');
	}
	else
	{
		require_once($GLOBALS['where_framework'].'/lib/lib.form.php');
...
/*
function _sendMail($subject, $body, &$mail_recipients, $sender)
{
	...
	$intestazione .= "Return-Path: ".$GLOBALS['framework']['sender_event'].$GLOBALS['mail_br'];
	$intestazione .= "Reply-To: ".$GLOBALS['framework']['sender_event'].$GLOBALS['mail_br'];
	$intestazione .= "X-Sender: ".$GLOBALS['framework']['sender_event'].$GLOBALS['mail_br'];
	$intestazione .= "X-Mailer: PHP/". phpversion().$GLOBALS['mail_br'];

	// send mail
	while(list($recipient) = each($mail_recipients))
	{
		@mail($recipient , stripslashes($subject), stripslashes(nl2br($body)), $from.$intestazione."\r\n");
	}
}*/
...
function infoLocation()
{
	...
	$tb = new TypeOne(15, $lang->def('_LOCATION_INFO'), $lang->def('_LOCATION_INFO'));
	...
	$tb->addBody(array('<b>'.$lang->def('_NOTE').':</b>', $note));
...
function reservationDispatch($op)
{
	if (isset($_POST['add_event']))
		jumpTo('index.php?modname=reservation&amp;op=add_event');
	if (isset($_POST['category_gestion']))
		jumpTo('index.php?modname=reservation&amp;op=view_category&amp;id_course='.$_SESSION['idCourse']);
	if (isset($_POST['location_gestion']))
		jumpTo('index.php?modname=reservation&amp;op=classroom&amp;id_course='.$_SESSION['idCourse']);
	if (isset($_POST['undo']))
		jumpTo('index.php?modname=reservation&op=reservation&active_tab=subscribed_user');
