...
function mycourses(&$url) {
	...
	if($access_user_details_short) {
		...
		$profile->init('profile', 'lms', 'index.php?modname=course&op=mycourses', 'ap');
	...
	if($access_career) {
        ...
        while(list($id_meta, $id_certificate) = mysql_fetch_row($result))
        {
	...
	if($access_news) {
		...
		$displayed = 0;
		while(list($id_news, $publish_date, $title, $short_desc, $impo, $viewer) = mysql_fetch_row($re_news)) {
			...
			if(!empty($intersect) || empty($viewer)) {
				...
				$GLOBALS['page']->addStart(
					...
					.'<span class="news_data">'.$GLOBALS['regset']->databaseToRegional($publish_date, 'date').' - </span>'
						.$short_desc
					.'</div>'
				, 'content' );
				$displayed++;
		...
		if(!$displayed) { $GLOBALS['page']->addStart($lang->def('_NO_NEWS'), 'content'); }
	...
	if($ma->currentCanAccessObj('lo_tab')) {
		...
		$GLOBALS['page']->addStart(
			...
			.'<h1>'
				.$lang->def('_WELCOME').': '
				.'<span>'.$GLOBALS['current_user']->getUserName().'</span>'
			.'</h1>'
			...
			.( $course_stats['with_ustatus'][_CUS_END] != $course_stats['total'] ?
				...
				.'<a href="index.php?modname=course&amp;op=mycourses&amp;current_tab=lo_plan"><span>'.$lang->def('_LO_PLAN').'</span></a></li>'
				: ''
			)
			.( $course_stats['with_ustatus'][_CUS_END] != 0 ?
				'<li '.( $current_tab == 'lo_history' ? 'class="now_selected"' : '').'>'
					.'<a href="index.php?modname=course&amp;op=mycourses&amp;current_tab=lo_history"><span>'.$lang->def('_LO_HISTORY').'</span></a></li>'
				: ''
			)
...
function userCourseList(&$url, $use_tab = true) {
	...
	if (isset($GLOBALS['lms']['tablist_mycourses'])) {
		
		$arr_order_course = explode(',', $GLOBALS['lms']['tablist_mycourses'] );
		$arr_temp = array();
	...
	$re_course = mysql_query($select_course
		.$from_course
		." WHERE ".$where_course
		.$group_by_course
		.$order_course);
	...
	$re_edition = mysql_query($select_edition
		.$from_edition
		.$where_edition);
	...
	$preass_list = mysql_query($select_assess
		.$from_assess
		." WHERE ".$where_assess
		." ORDER BY c.name ");
	...
	if($direct_play) {
		...
		YAHOO.util.Event.onDOMReady(function() {
			...
			Shadowbox.close = function() { 
				...
				if(window.frames[\'shadowbox_content\'].playerConfig.backurl != undefined) {
					window.location = window.frames[\'shadowbox_content\'].playerConfig.backurl+\'&from='.$current_tab.'\';
		...
		/*
			var options = {
		        ...
		        overlayOpacity:"0.8",
		        close:"'.def('_CLOSE').'",
		        loadingImage:"'.getPathImage('lms').'standard/loading.gif",
		        overlayBgImage:"'.getPathImage('lms').'standard/overlay-85.png"
		    };
			Shadowbox.init(options);
			Shadowbox.close = function() {
			...
			if(!isset($path_courses[$id_path]) || empty($path_courses[$id_path])) {
				...
				foreach($path_slot[$id_path]  as $id_slot => $slot_info) {
					...
					if($id_slot == 0) {
						...
						$html .= '<h3>'.$lang->def('_COURSE_PATH_MAIN').'</h3>';
...
function dashmycourse(&$url, $lang, &$subscription, $cinfo, $index) {
	...
	$arr_status = array(_CUS_RESERVED 		=> $lang->def('_T_USER_STATUS_RESERVED'),
						_CUS_WAITING_LIST 	=> $lang->def('_T_USER_STATUS_WLIST'),
	...
	DoceboLanguage::createInstance('admin_course_managment', 'lms');
		...
		//if($cinfo['level'] >= 4) {
			...
			$intest .= '<p class="course_support_info">'.
				str_replace(	array('[course_type]', '[create_date]', '[enrolled]', '[course_status]'), 
								array(	$course_type, 
										createDateDistance($cinfo['create_date'], 'coursecatalogue', true), 
				...
				( $cinfo['date_begin'] != '0000-00-00' || $cinfo['date_end'] != '0000-00-00' 
					? 
						str_replace(	array('[date_begin]', '[date_end]'), 
									array(	$GLOBALS['regset']->databaseToRegional($cinfo['date_begin'], 'date'), 
											$GLOBALS['regset']->databaseToRegional($cinfo['date_end'], 'date')), 
		...
		if($access['can']) {
			...
			$intest .= '<a class="course_enter" href="index.php?modname=course&amp;op=aula&amp;idCourse='.$cinfo['idCourse'].'&amp;id_e='.$ed_id.'" '
					...
					.( $ed_info['date_begin'] != '' || $ed_info['date_end'] != '' 
						? ' <span>( '
							.$GLOBALS['regset']->databaseToRegional($ed_info['date_begin'], 'date')
	...
	$html = '<div class="course_container'
		.( $GLOBALS['lms']['use_social_courselist'] == 'on' ? ' double_height' : '' )
	...
	if(($cinfo['use_logo_in_courselist'] == '1' && $cinfo['img_course'] != '') 
		|| $GLOBALS['lms']['use_social_courselist'] == 'on') $html .= '<div class="course_info_container">';
	
	
	if($cinfo['use_logo_in_courselist'] == '1' && $cinfo['img_course'] != '') {
		$html .= '<ul class="course_score"><li>';
	 	$html .= '<img height="70" src="'.$GLOBALS['where_files_relative'].'/doceboLms/'.$GLOBALS['lms']['pathcourse']
	...
	if($GLOBALS['lms']['use_social_courselist'] == 'on' || !empty($there_material)) {
	...
	if(($cinfo['use_logo_in_courselist'] == '1' && $cinfo['img_course'] != '') 
		|| $GLOBALS['lms']['use_social_courselist'] == 'on') $html .= '</div>';
