...
define('_PATH_COURSE', '/doceboLms/'.$GLOBALS['lms']['pathcourse']);
...
function loadMaterials($idCourse) {
	...
	$tb = new TypeOne(0, $lang->def('_MATERIALS'), $lang->def('_MATERIALS_TABLE'));
	...
	$html = '<div class="infocourse_attachment">';
	if(mysql_num_rows($re_file)) {
		
		while(list($idFile, $title, $file) = mysql_fetch_row($re_file)) {
	...
	if($mod_perm) {
		$html .=  '<div class="add_container">'
			.'<a href="index.php?modname=course&amp;op=addfiles">'
			.'<img src="'.getPathImage().'standard/add.gif" alt="'.$lang->def('_ADD').'" />&nbsp;'.$lang->def('_ADDFILE').'</a>'
			.'</div>';
	}
	$html .= '</div>';
	return $html;
...
function infocourse() {
	...
	$GLOBALS['page']->add(
		'<table class="cd_course_info">'
			...
			.'<tr><th scope="row">'.$lang->def('_DIFFICULT_NAME').'</th><td>'.$difficult_lang[$course['difficult']].'</td></tr>'
		...
		$GLOBALS['page']->add(
			'<tr>'
				...
				.( $max_quota == USER_QUOTA_UNLIMIT 
							...
							: ''.$actual_space.' / '.$max_quota.' MB '.drawProgressBar($percent, true, 'progress_bar cp_quota_bar', false, false)
				)
			.'</td></tr>', 'content');
	}
	...
	if($mod_perm) {
		$GLOBALS['page']->add( '<br /><div class="mod_container">'
			.'<a class="infomod" href="index.php?modname=course&amp;op=modcourseinfo">'
			.'<img src="'.getPathImage().'standard/mod.gif" alt="'.$lang->def('_MODINFOIMG').'" />&nbsp;'.$lang->def('_MODINFO').'</a>'
			.'</div>', 'content');
	}
...
function modcourseinfo() {
	...
	$lang =& DoceboLanguage::createInstance('admin_course_managment');
	...
	$out->add(
		...
		.$form->getDropdown($lang->def('_COURSE_LANG_METHOD'), 'course_lang', 'course_lang', $array_lang, 
			$lang_code )
		.$form->getDropdown($lang->def('_COURSE_DIFFICULT'), 'course_difficult', 'course_difficult', $difficult_lang, 
			$course['difficult'] )
		.$form->getDropdown($lang->def('_STATUS'), 'course_status', 'course_status', 
			array( 0 => $lang->def('_CST_PRE_DO'), 1 =>$lang->def('_CST_IN_DO'), 2 =>$lang->def('_CST_AFTER_DO')), 
			$course['status'] )
		...
		.str_replace(
			'[down]', 
			'<img src="'.getPathImage().'standard/down.gif" alt="'.$lang->def('_OTHER_OPTION').'" />', 
...
function upcourseinfo() {
	...
	$msg_composer->setBodyLangText('email', '_ALERT_TEXT_MODCOURSE_INFO', array(	'[url]' => $GLOBALS['lms']['url'], 
																		'[course_code]' => $_POST['course_code'], 
																		'[course]' => $_POST['course_name'] ) );
	
	$msg_composer->setSubjectLangText('sms', '_ALERT_SUBJECT_MODCOURSE_INFO_SMS', false);
	$msg_composer->setBodyLangText('sms', '_ALERT_TEXT_MODCOURSE_INFO_SMS', array(	'[url]' => $GLOBALS['lms']['url'], 
																		'[course_code]' => $_POST['course_code'], 
																		'[course]' => $_POST['course_name'] ) );
	...
	jumpTo( 'index.php?modname=course&op=infocourse&result='.( $re ? 'ok' : 'err' ));
}
...
function insfiles() {
	...
	jumpTo( 'index.php?modname=course&op=infocourse');
}
...
function modfiles() {
	...
	$GLOBALS['page']->add(
		getTitleArea(
			array('index.php?modname=course&amp;op=infocourse' => $lang->def('_INFO'), $lang->def('_MODFILES'))
			, 'infocourse')
		.'<div class="std_block">'
		.getBackUi('index.php?modname=course&amp;op=infocourse', $lang->def('_BACK'))
...
function remfiles() {
	...
	if( isset($_GET['confirm']) && ($_GET['confirm'] == '1')) {
		...
		$size = getFileSize($GLOBALS['where_files_relative']._PATH_COURSE.$old_file);
		if(!sl_unlink(_PATH_COURSE.$old_file)) {
			
			$GLOBALS['page']->add(getErrorUi($lang->def('_ERRREM')));
			return;
		}
		...
		list($title, $file) = mysql_fetch_row(mysql_query("
		SELECT title, path 
		FROM ".$GLOBALS['prefix_lms']."_course_file 
		WHERE id_course = '".(int)$_SESSION['idCourse']."' AND id_file = '".(int)$_GET['id_file']."'"));
		...
		$GLOBALS['page']->add(
			getTitleArea(
				array('index.php?modname=course&amp;op=infocourse' => $lang->def('_INFO'), $lang->def('_DELFILES'))
				, 'infocourse')
			.'<div class="std_block">'
...
function viewprofile() {
	...
	$profile->init('profile', 'lms', 'modname=course&op=profile&infocourse', 'ap');
