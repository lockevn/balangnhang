...
class SOAPLMS {
	...
	function Finish( $idUser, $idReference, $idscorm_item ) {
		...
		if( $trackobj->getParam(SCORM_RTE_CREDIT, false) == 'credit' ) {
			...
			$itemtrack->setStatus($idUser, $idReference, $idscorm_item, $lesson_status );
		}
...
if( (isset($_GET['op']) && $_GET['op'] == 'Finish') ) {
	...
	// call to finish
	echo $soaplms->Finish( $idUser, $idReference, $idscorm_item );
	...
	*/
	...
	if( !(	is_numeric ( $_GET['idReference'] ) && is_numeric ( $_GET['idUser'] ) 
 ...
 */	
	...
	$parameters = '';
	$re = mysql_query("SELECT parameters FROM ".$GLOBALS['prefix_lms']."_scorm_items WHERE idscorm_item = '".$_GET['idscorm_item']."'");
	if($re) list($parameters) = mysql_fetch_row($re);
	
	/*echo $scopath.$path."/".$href.$parameters;
	exit;*/
	jumpTo( $scopath.$path."/".$href.$parameters);
} else {
