...
Class Scorm_ItemsTrack {
	...
	function getTrackingScore( $idscorm_tracking ) {
		...
		$rs = mysql_query($query, $this->dbconn) or die( "Scorm_ItemsTrack::getTrackingScore error in select [$query] ". mysql_error($this->dbconn) );
		list( $score ) = mysql_fetch_row($rs);
		return $score;
	}
	...
	function getSiblingsItems( $idReference, $idscorm_item) {
		...
		if( $resultArray === FALSE )
			return FALSE;
		list($idscorm_parentitem) = mysql_fetch_row($resultArray);
	 ...
	 *	@param $status
	 **/
	function setStatus( $idUser, $idReference, $idscorm_item, $status ) {
		...
		if( strcmp($record['status'], $status) != 0 ) {
			...
			if( mysql_query($query, $this->dbconn) === FALSE ) {
			...
			if( strcmp($status,'completed') == 0 || strcmp($status, 'passed') == 0 ) {
				$this->forwardCompleted($idUser, $idReference, $idscorm_item, TRUE);
			}
		}
	}
	 ...
	 *	@param $isChild
	 **/
	function forwardCompleted( $idUser, $idReference, $idscorm_item, $isChild ) {
		...
		if( isset($record['idscorm_item']) ) {
			if( $record['idscorm_item'] !== NULL ) {
				if( strcmp($status,'completed') == 0 || strcmp($status, 'passed') == 0 ) 
					$this->forwardCompleted($idUser, $idReference, $record['idscorm_item'], TRUE);
				else 
					$this->forwardCompleted($idUser, $idReference, $record['idscorm_item'], FALSE);
			} 
		} else {
			...
			if( strcmp($status,'completed') == 0 || strcmp($status, 'passed') == 0 ) {
				...
				$track_so = new Track_ScormOrg( $record['idscorm_item_track'] );
