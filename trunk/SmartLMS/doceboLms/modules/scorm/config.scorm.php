...
function sl_sal_getUserName() {
	...
	$result = mysql_query("SELECT surname,name FROM ".$GLOBALS['prefix_lms']."_user WHERE idUser=".$_SESSION['sesUser']."");
	list($surname, $name) = mysql_fetch_array($result);*/
	$aclManager = $GLOBALS['current_user']->getACLManager();
	$arr_result = $aclManager->getUser($GLOBALS['current_user']->getIdSt(), FALSE);
