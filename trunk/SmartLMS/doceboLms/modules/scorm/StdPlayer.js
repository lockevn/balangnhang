...
StdUIPlayer.prototype.drawNavigation = function() {
	...
	if(prevExist()) {
		...
		navi +="<div id=\"prevblocklink\" style=\"display:block; float: left;\">\n"
			+"			"
			+"				<img class=\"imgnav\" id=\"imgprev\" src=\""+playerConfig.imagesPath+"../standard/blank.gif\" width=\"32\" alt=\"Back\" />"
			+"			\n"
			+"			<span id=\"prevlink\"></span>"
			+"		</div>\n";
	} 
	if(nextExist()) {
		...
		navi +="<div id=\"prevblocklink\" style=\"display:block; float: left;\">\n"
			+"			"
			+"				<img class=\"imgnav\" id=\"imgprev\" src=\""+playerConfig.imagesPath+"../standard/blank.gif\" width=\"32\" alt=\"Next\" />"
			+"			\n"
			+"			<span id=\"prevlink\"></span>"
			+"		</div>\n";
	}
...
StdUIPlayer.prototype.startItem = function( objItem, level ) {
	...
	if( objItem.isLeaf) var elem = doc.createElement('a');
	else  var elem = doc.createElement('span');
	
	if( objItem.isLeaf && (objItem.status == 'completed' || objItem.status == 'passed') ) {
	...
	} else if( objItem.isLeaf && (objItem.status == 'incomplete' || objItem.status == 'failed') ) {
		var imgStatus = doc.createElement('img');
		...
		elem.appendChild(imgStatus);
	} else if( objItem.isLeaf && objItem.prerequisites == "" ) {
		var imgStatus = doc.createElement('img');
...
StdUIPlayer.initialize = function() {
	...
	/* create Scorm API */
	window.API = new ScormApiUI( 	playerConfig.host, 
	...
	window.API.useWaitDialog( !(playerConfig.useWaitDialog=='off') );
	/* create ScormPlayer */
	...
	window.scormPlayer.setAPI( window.API );
...
trackUnloadOnLms = function() {
	...
	var ajax = new Ajax.Request('./modules/scorm/keep_alive.php', {method: 'get'});
}
