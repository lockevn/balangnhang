...
function modfaqgui( $object_faq ) {
	...
	$GLOBALS['page']->add(getTitleArea($lang->def('_SECT_FAQ'), 'faq')
		.'<div class="std_block">'		
		.getBackUi( ereg_replace('&', '&amp;', $object_faq->back_url).'&amp;mod_result=0', $lang->def('_BACK') )
		
		.'<span class="title">'.$lang->def('_TITLE').' : </span>'.$title_cat
		.'<div class="mod_container">'
		.'<a href="index.php?modname=faq&amp;op=modfaqcat&amp;idCategory='.$object_faq->getId()
			.'&amp;back_url='.$back_coded.'" title="'.$lang->def('_MODFAQCATT').'">'
		.'<img src="'.getPathImage().'standard/mod.gif" alt="'.$lang->def('_MOD').'" /> '.$lang->def('_MODFAQCAT').'</a>'
		.'</div><br />', 'content');
	$tableCat = new typeOne(0, '', $lang->def('_SUMMARY_FAQ'));
	...
	$i = 1;
	while(list($idFaq, $title, $seq) = mysql_fetch_row($result)) {
	...
	$tableCat->addActionAdd('<a href="index.php?modname=faq&amp;op=newfaq&amp;idCategory='
		...
		.'<img src="'.getPathImage().'standard/add.gif" alt="'.$lang->def('_ADD').'" /> '.$lang->def('_ADDFAQ').'</a>');
...
function addfaqcat( $object_faq ) {
	...
	$GLOBALS['page']->add(
		...
		.getBackUi( ereg_replace('&', '&amp;', $object_faq->back_url).'&amp;create_result=0', $lang->def('_BACK') )
...
function insfaqcat() {
	...
	SET title = '".( (trim($_POST['title']) == '') ? def('_NOTITLE', 'faq', 'lms') : $_POST['title'])."',
		description = '".$_POST['description']."',
		author = '".(int)getLogUserId()."'";
	if(!mysql_query($query_ins)) {
		
		$_SESSION['last_error'] = def('_ERRINSFAQCAT', 'faq', 'lms');
		jumpTo($back_url.'&create_result=0');
	}
	
	list($idFaq) = mysql_fetch_row(mysql_query("SELECT LAST_INSERT_ID()"));
	jumpTo($back_url.'&id_lo='.$idFaq.'&create_result=1');
}
...
function upfaqcat() {
	...
	SET title = '".( (trim($_POST['title']) == '') ? def('_NOTITLE', 'faq') : $_POST['title'])."',
		description = '".$_POST['description']."' 
	WHERE idCategory = '".(int)$_POST['idCategory']."'";
	if(!mysql_query($query_ins)) {
		
		$GLOBALS['page']->add(getBackUi(def('_ERRINSFAQCAT', 'faq', 'lms').getBackUi('index.php?modname=faq&op=modfaqgui&idCategory='
					.(int)$_POST['idCategory'].'&back_url='.$back_coded, def('_BACK', 'faq', 'lms'))), 'content');
		return;
	}
	
	require_once($GLOBALS['where_lms'].'/class.module/track.object.php');
	Track_Object::updateObjectTitle($_POST['idCategory'], 'faq', ( (trim($_POST['title']) == '') ? def('_NOTITLE', 'faq') : $_POST['title']));
	
	jumpTo('index.php?modname=faq&op=modfaqgui&idCategory='.(int)$_POST['idCategory'].'&back_url='.$back_coded);
}
...
function movefaq($direction) {
	...
	jumpTo( 'index.php?modname=faq&op=modfaqgui&idCategory='.$idCategory.'&back_url='.$back_coded);
}
...
function fixsequence() {
	...
	$i = 1;
	while(list($idFaq) = mysql_fetch_row($reQuest)) {
		mysql_query("
		UPDATE ".$GLOBALS['prefix_lms']."_faq 
		SET sequence = '".($i++)."' 
		WHERE idFaq = '$idFaq'");
	...
	jumpTo('index.php?modname=faq&op=modfaqgui&idCategory='.$_GET['idCategory'].'&back_url='.$back_coded);
}
...
function insfaq() {
	...
	SET idCategory = '".$idCategory."',
		...
		sequence = '".($seq + 1)."'";
	if(!mysql_query($query_ins)) {
		$GLOBALS['page']->add(getErrorUi(def('_ERRINSFAQ', 'faq').getBackUi('index.php?modname=faq&op=modfaqgui&idCategory='
			.$idCategory.'&back_url='.$back_coded, def('_BACK'))), 'content');
		return;
	}
	jumpTo( 'index.php?modname=faq&op=modfaqgui&idCategory='.$idCategory.'&back_url='.$back_coded);
}
...
function modfaq() {
	...
	list($idCategory, $question, $title, $keyword, $answer) = mysql_fetch_row(mysql_query("
...
function delfaq() {
	...
	if( isset($_GET['confirm']) ) {
		...
		WHERE idFaq  = '".(int)$_GET['idFaq']."'")) {
			$GLOBALS['page']->add(getErrorUi($lang->def('_ERRREMFAQ').getBackUi('index.php?modname=faq&op=modfaqgui&idCategory='
		...
		list($idCategory, $question, $answer) = mysql_fetch_row(mysql_query("
		...
		$GLOBALS['page']->add(
			'<div class="std_block">'
			.getDeleteUi(	$lang->def('_AREYOUSUREFAQ'),
