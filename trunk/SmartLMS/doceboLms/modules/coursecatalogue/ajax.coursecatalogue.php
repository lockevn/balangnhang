...
if($GLOBALS['current_user']->isAnonymous()) {
	...
	$lang =& DoceboLanguage::createInstance( 'coursecatalogue', 'lms');
	...
	$value = array(
		"next_op" 	=> '',
		"id" 		=> 'course_editions',
		"title" 	=> $lang->def('_COURSE_SUBSCRIPTION', 'coursecatalogue'),
	...
	switch($op) {
		...
		case "getCourseEditionsTable": {
			require_once($GLOBALS["where_lms"]."/modules/coursecatalogue/lib.coursecatalogue.php");
			docebo_cout(getCourseEditionTable((int)$_POST["course_id"]));
		} break;
		case "getdashcourse" : {
		...
		case "course_action_confirm" : {
			...
			if($GLOBALS['current_user']->isAnonymous()) {
				...
				$where_count 	= " WHERE u.idCourse = '".$id_course."' " .
						...
						" AND u.absent = '0'";
				$re_count = mysql_query($select_count.$from_count.$where_count );
				list($cinfo['enrolled']) = mysql_fetch_row($re_count);
		...
		case "course_action_confirm_edition" : {
			...
			if($GLOBALS['current_user']->isAnonymous()) {
				...
				$value = array(
					"next_op" 	=> '',
					"id" 		=> 'course_editions',
					"title" 	=> $lang->def('_COURSE_EDITON_SUBSCRIBE', 'coursecatalogue'),
					"content" 	=> str_replace(array_keys($subst), $subst, $string)
				);
			} else {
		...
		case "addnewcomment" : {
			...
			$ax_rend = new AjaxCommentRender('coursecatalogue', 'lms');

			$man_courseuser 	= new Man_CourseUser();
			$usercourses 		=& $man_courseuser->getUserSubscriptionsInfo(getLogUserId(), true);

			if($GLOBALS['current_user']->isAnonymous()) { $ax_comm->canReply(false); }
			...
			$value = array(
				"next_op" 	=> '',
				"id" 		=> 'course_comment',
				"title" 	=> $lang->def('_COURSE_COMMENT'),
				"content" 	=> $content
			);
		...
		case "course_materials" : {
			...
			if($course_mat != '') {
				...
				$html .= '<li><b>['.$course_man->getValue('code').'] '.$course_man->getValue('name').' '.$course_mat.'</b>'
					...
					.'<a href="index.php?modname='.( $GLOBALS['current_user']->isAnonymous()
								? 'login'
								: 'coursecatalogue' ) .'&amp;op=donwloadmaterials'
							.'&amp;id_course='.$id_course.'">'
					.$lang->def('_DOWNLOAD_MATERIALS').'</a>'
					.'</div>'
					.'</li>';
			}
			...
			while($ed_info = mysql_fetch_assoc($re_edition)) {
				...
				if($ed_info['img_material'] != '') {
					...
					$html .= '<div class="popup_materials">'
							...
							.'<span>'.$lang->def('_DOWNLOAD_MATERIALS').'</span></a>'
							.'</div>';
		...
		case "play_demo" : {
			...
			$value = array(
				...
				"content" 	=> getEmbedPlay('/doceboLms/'.$GLOBALS['lms']['pathcourse'], $course_demo, $ext, '450', '450', false, false, '../../'.$GLOBALS['where_files_relative'])
			);
			require_once($GLOBALS['where_framework'].'/lib/lib.json.php');
		...
		default : {
			$lang =& DoceboLanguage::createInstance( 'standard', 'framework');
			$lang->setGlobal();
			$lang =& DoceboLanguage::createInstance( 'coursecatalogue', 'framework');
