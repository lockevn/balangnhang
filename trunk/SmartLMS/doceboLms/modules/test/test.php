...
function addtest( $object_test ) {
	...
	if( !is_a($object_test, 'Learning_Test') ) {
		$_SESSION['last_error'] = $lang->def('_TEST_INCORRECTOBJECT');
		jumpTo( ''.$object_test->back_url.'&amp;create_result=0');
	}
	...
	$GLOBALS['page']->add(getTitleArea($lang->def('_TEST_SECTION'), 'test')
		.'<div class="std_block">'
		.getBackUi(ereg_replace('&', '&amp;', $object_test->back_url).'&amp;create_result=0', $lang->def('_BACK'))
...
function instest() {
	...
	if($id_test > 0) jumpTo( ''.urldecode($_POST['back_url']).'&id_lo='.$id_test.'&create_result=1' );
...
function modtest() {
	...
	list($test_title, $textof) = mysql_fetch_row( mysql_query("
	...
	$GLOBALS['page']->add(
		...
		.Form::getFormHeader($lang->def('_TEST_MOD_FORM'))
...
function uptest() {
	...
	if( !mysql_query($mod_query) ) {
		...
		errorCommunication($lang->def('_ERR_UPD_TEST')
	...
	jumpTo( 'index.php?modname=test&op=modtestgui&idTest='.$id_test.'&back_url='.$url_encode );
}
...
function modtestgui( $object_test ) {
	...
	list($test_title) = mysql_fetch_row( mysql_query("
	SELECT title 
	FROM ".$GLOBALS['prefix_lms']."_test 
	...
	//other areas
	$GLOBALS['page']->add('<span class="text_bold">'.$lang->def('_TITLE').' : </span>'.$test_title.'<br />'
		.'<div class="mod_container">'
		.'<a href="index.php?modname=test&amp;op=modtest&amp;idTest='
		.$object_test->getId().'&amp;back_url='.$url_encode.'" title="'.$lang->def('_REG_MODTEST_TITLE').'">'
		.'<img src="'.getPathImage().'standard/mod.gif" alt="'.$lang->def('_MOD').'" />&nbsp;'.$lang->def('_REG_MODTEST').'</a>'
		.'</div>'
		
		.'<br /><div class="align_left">'
		//test modality
		.'<div class="link_button">'
		.'<a href="index.php?modname=test&amp;op=defmodality&amp;idTest='
		.$object_test->getId().'&amp;back_url='.$url_encode.'" title="'.$lang->def('_TEST_MODALITY').'">'
			.$lang->def('_TEST_MODALITY').'</a>'
		.'</div>&nbsp;&nbsp;'
		//test times
		.'<div class="link_button">'
		.'<a href="index.php?modname=test&amp;op=deftime&amp;idTest='
		.$object_test->getId().'&amp;back_url='.$url_encode.'" title="'.$lang->def('_TEST_COMPILE_TIME').'">'
			.$lang->def('_TEST_COMPILE_TIME').'</a>'
		.'</div>&nbsp;&nbsp;'
		//test point assign
		.'<div class="link_button">'
		.'<a href="index.php?modname=test&amp;op=defpoint&amp;idTest='
		.$object_test->getId().'&amp;back_url='.$url_encode.'" title="'.$lang->def('_TEST_POINT_ASSIGNEMENT').'">'
			.$lang->def('_TEST_POINT_ASSIGNEMENT').'</a>'
		.'</div>'
		.'</div><br />', 'content');
	
	$caption = str_replace('%tot_page%' , $num_page, str_replace('%tot_element%' ,$num_quest , $lang->def('_TEST_CAPTION')));
	
	$tab = new TypeOne( 0, $caption, $lang->def('_TEST_SUMMARY'));
 
	$tab->setColsStyle(array('image', 'image', '', 'image', 'image', 'image', 'image', 'image'));
	$tab->addHead(
		array($lang->def('_QUEST'), $lang->def('_TYPE'), $lang->def('_TEST_QUEST_TITLE'), $lang->def('_TEST_QUEST_ORDER'), 
		'<img src="'.getPathImage().'standard/down.gif" alt="'.$lang->def('_DOWN').'" longdesc="'.$lang->def('_MOVE_DOWN').'" />', 
		'<img src="'.getPathImage().'standard/up.gif" alt="'.$lang->def('_UP').'" longdesc="'.$lang->def('_MOVE_UP').'" />', 
		'<img src="'.getPathImage().'standard/mod.gif" alt="'.$lang->def('_MOD').'" longdesc="'.$lang->def('_TEST_MODTEST').'" />', 
		'<img src="'.getPathImage().'standard/rem.gif" alt="'.$lang->def('_DEL').'" longdesc="'.$lang->def('_TEST_REMTEST').'" />' ));
	...
	while(list($id_quest, $type, $title, $sequence, $page) = mysql_fetch_row($re_quest)) {
		...
		( ($i != ($num_quest - 1)) ? 
			'<a href="index.php?modname=test&amp;op=movedown&amp;idQuest='.$id_quest.$uri_back.'" title="'.$lang->def('_MOVE_DOWN').'">'
				.'<img src="'.getPathImage().'standard/down.gif" alt="'.$lang->def('_DOWN').' : '.$lang->def('_TEST_ROWS').' '.($i + 1).'" /></a>' : '' ),
		( ($i != 0) ? 
			'<a href="index.php?modname=test&amp;op=moveup&amp;idQuest='.$id_quest.$uri_back.'" title="'.$lang->def('_MOVE_UP').'">'
				.'<img src="'.getPathImage().'standard/up.gif" alt="'.$lang->def('_UP').' : '.$lang->def('_TEST_ROWS').' '.($i + 1).'" /></a>' : '' ),
				
		( $type != 'break_page' ? '<a href="index.php?modname=test&amp;op=modquest&amp;idQuest='.$id_quest.$uri_back.'" title="'.$lang->def('_TEST_MODTEST').'">'
			.'<img src="'.getPathImage().'standard/mod.gif" alt="'.$lang->def('_MOD').' : '.$lang->def('_TEST_ROWS').' '.($i + 1).'" longdesc="'.$lang->def('_TEST_MODTEST').'" /></a>' : '' ),
		'<a href="index.php?modname=test&amp;op=delquest&amp;idQuest='.$id_quest.$uri_back.'" title="'.$lang->def('_TEST_REMTEST').'">'
			.'<img src="'.getPathImage().'standard/rem.gif" alt="'.$lang->def('_DEL').' : '.$lang->def('_TEST_ROWS').' '.($i + 1).'" longdesc="'.$lang->def('_TEST_REMTEST').'" /></a>',
		);
	...
	if($num_quest > 1) {
		...
		$move_quest .= '<label class="text_bold" for="source_quest">'.$lang->def('_TEST_MOVE_QUEST').'</label>&nbsp;'
		...
		$move_quest .= '</select>';
		$move_quest .= '<label class="text_bold" for="dest_quest"> '.$lang->def('_TEST_LABEL_AFTER_QUEST').'</label>&nbsp;'
...
/*	$re_type = mysql_query("
	...
	*/
	...
	if($seq_error_detected) {
		...
		$GLOBALS['page']->add(
			' <a href="index.php?modname=test&amp;op=fixsequence&amp;idTest='.$object_test->getId().$uri_back.'" title="'.$lang->def('_FIX_SEQ_ERROR').'">'
			.$lang->def('_SEQUENCE_ERROR').'</a>'
		, 'content');
	}
...
function movequestion($direction) {
	...
	list($seq, $idTest) = mysql_fetch_row(mysql_query("
	...
	jumpTo( 'index.php?modname=test&op=modtestgui&idTest='.$idTest.'&back_url='.$back_coded);
}
...
function movequest() {
	...
	list($idQuest) = mysql_fetch_row(mysql_query("
	SELECT idQuest 
	FROM ".$GLOBALS['prefix_lms']."_testquest 
...
function modquest() {
	...
	list($idTest, $type_quest) = mysql_fetch_row(mysql_query("
...
function delquest() {
	...
	if(isset($_GET['confirm'])) {
		...
		$GLOBALS['page']->add(
			...
			.'</div>', 'content');
			/*
			.'<div class="boxinfo_title">'.$lang->def('_TEST_AREYOUSURE').'</div>'
			...
			.'<a href="index.php?modname=test&amp;op=delquest&amp;idQuest='.$idQuest.'&amp;back_url='
				.$url_coded.'&amp;confirm=1">'
				.'<img src="'.getPathImage().'standard/rem.gif" alt="'.$lang->def('_CONFIRM').'" />&nbsp;'
				.$lang->def('_CONFIRM').'</a>&nbsp;&nbsp;'
			.'<a href="index.php?modname=test&amp;op=modtestgui&amp;idTest='.$idTest.'&amp;back_url='.$url_coded.'">'
				.'<img src="'.getPathImage().'standard/undo.gif" alt="'.$lang->def('_UNDO').'" />&nbsp;'
				.$lang->def('_UNDO').' </a>'
			.'</div>'*/
	}
}
...
function defmodality() {
	...
	list($title, $description, $display_type, $order_type, $shuffle_answer, $question_random_number, 
		...
		$max_attempt, $hide_info) = mysql_fetch_row( mysql_query("
	...
	$GLOBALS['page']->add(
		...
		.'<legend>'.$lang->def('_TEST_MM_ONE').'</legend>'."\n"
		...
		.'<br />'
		//-order-----------------------------------------------------
		.'<div class="text_bold">'.$lang->def('_TEST_MM1_ORDER').'</div>'
	...
	$GLOBALS['page']->add(
		...
		.Form::getRadio($lang->def('_TEST_MM1_ANSWER_RANDOM'), 'shuffle_answer_random', 'shuffle_answer', 1, $shuffle_answer)
		.'</div>'
		
		.'</fieldset>'
		
		.'<fieldset class="configuration">'."\n"
		.'<legend>'.$lang->def('_TEST_MM_TWO').'</legend>'."\n"
		.'<div class="field_content">'
		...
		.'</fieldset>'
		.'<fieldset class="configuration">'."\n"
		.'<legend>'.$lang->def('_TEST_MM_FOUR').'</legend>'."\n"
		.'<div class="field_content">'
			//can modify answer
...
function updatemodality() {
	...
	list($time_dependent) = mysql_fetch_row( mysql_query("
	...
	." WHERE idTest = '$idTest'")) {
		errorCommunication($lang->def('_ERR_SAVE')
...
function deftime() {
	...
	list($time_dependent, $time_assigned, 
		$penality_test, $penality_time_test, $penality_quest, $penality_time_quest) = mysql_fetch_row(mysql_query("
	...
	if(isset($_POST['settime_button'])) {
		...
		switch($_POST['time_limit']) {
			case 0 : {
				...
				jumpTo('index.php?modname=test&op=modtestgui&idTest='.$idTest.'&back_url='.$url_coded.'&mod_operation=1');
			};break;
			case 1 : {
				...
				$GLOBALS['page']->add(
					...
					.Form::getLineBox($lang->def('_TEST_TM2_TIMETOTAL'), $time_assigned.' '.$lang->def('_SECONDS'))
			...
			case 2 : {
				...
				list($actual_tot_time) = mysql_fetch_row(mysql_query("
				...
				$GLOBALS['page']->add(
					...
					.Form::getLineBox($lang->def('_TEST_TM2_ACTUALTIME'), $actual_tot_time.' '.$lang->def('_SECONDS'))
					...
					.Form::getRadio($lang->def('_TEST_TM2_DIFFICULT'), 'point_diffcult', 'point_assignement',0)
...
function updatetime() {
	...
	if(isset($_POST['undo'])) jumpTo('index.php?modname=test&op=modtestgui&idTest='.$idTest.'&back_url='.$url_coded);
	...
	if($_POST['time_limit'] == 2) {
		...
		jumpTo('index.php?modname=test&op=modassigntime&idTest='.$idTest.'&back_url='.$url_coded
...
function modassigntime() {
	...
	if(isset($_POST['saveandexit'])) {
		...
		jumpTo('index.php?modname=test&op=modtestgui&idTest='.$idTest.'&back_url='.$url_coded.'&mod_operation='.($re ? 1 : 0));
	}
	...
	list($tot_quest, $tot_difficult, $actual_tot_time) = mysql_fetch_row(mysql_query("
	...
	//table header---------------------------------------------------
	$tab_quest = new typeOne(0, $lang->def('_TEST_SUMMARY'), $lang->def('_TEST_SUMMARY'));
	$tab_quest->setColsStyle(array('image', 'image', '', 'image', 'image'));
	$tab_quest->addHead(array(
		$lang->def('_TEST_QUEST_ORDER'), $lang->def('_TYPE'), $lang->def('_TEST_QUEST_TITLE'), $lang->def('_TEST_QUEST_DIFFICULT'), 
		$lang->def('_TEST_QUEST_TIME_ASSIGNED').' ('.$lang->def('_SECONDS').')' ) );
	
	$i = 1;
	$effective_time = $effective_difficult = 0;
	//tabel body--------------------------------------------------------
	while(list($idQuest, $type_quest, $title_quest, $difficult, $time_assigned) = mysql_fetch_row($re_quest)) {
	...
	$tab_quest->addBodyCustom('<tr class="line-top-bordered">'
		.'<td colspan="3" class="align_right">'.$lang->def('_QUEST_TM2_TOTAL').'</td>'
...
function defpoint() {
	...
	list($title, $description, $point_type, $point_required) = mysql_fetch_row( mysql_query("
	...
	$GLOBALS['page']->add(
		...
		.'<fieldset class="configuration">'."\n"
		.'<legend>'.$lang->def('_TEST_PM_MINIMUM').'</legend>'."\n"
		.'<div class="field_content">'."\n"
		.Form::getHidden('idTest', 'idTest', $idTest)
		.Form::getHidden('back_url', 'back_url', $url_coded)
		...
		.'<fieldset class="configuration">'."\n"
		.'<legend>'.$lang->def('_TEST_PM_ONE').'</legend>'."\n"
		.'<div class="field_content">'."\n"
		...
		.'<fieldset class="configuration">'."\n"
		.'<legend>'.$lang->def('_TEST_PM_TWO').'</legend>'."\n"
		.'<div class="field_content">'
	...
	$GLOBALS['page']->add(
		...
		.'<div class="label_effect">'.$lang->def('_TEST_QUEST_MAXTESTSCORE').'</div>'.$max_score.' '.$lang->def('_TEST_QUEST_POINT').'</div>'
		.Form::getTextfield($lang->def('_TEST_QUEST_NEWMAXTESTSCORE'), 'new_assigned_score', 'new_assigned_score', 255, 
			$max_score, $lang->def('_TEST_QUEST_NEWMAXTESTSCORE'), $lang->def('_TEST_QUEST_POINT'))
...
function modassignpoint() {
	...
	list($tot_quest, $tot_difficult) = mysql_fetch_row(mysql_query("
	...
	$tab_quest = new typeOne(0, $lang->def('_TEST_SUMMARY'), $lang->def('_TEST_SUMMARY'));
	$tab_quest->setColsStyle(array('image', 'image', '', 'image', 'image'));
	$tab_quest->addHead(array($lang->def('_TEST_QUEST_ORDER'), $lang->def('_TYPE'), $lang->def('_TEST_QUEST_TITLE'), 
		$lang->def('_TEST_QUEST_DIFFICULT'), $lang->def('_TEST_QUEST_MAXSCORE') ) );
	...
	if(!isset($score_assign)) {
		...
		$GLOBALS['page']->add(
			'<div class="set_time_row">'
				...
				.Form::getButton('setpoint', 'setpoint', $lang->def('_TEST_TM2_SETSCORE'), 'button_nowh')
			.'</div>'
			.Form::openButtonSpace()
...
function doimportquest() {
	...
	cout(
		'<!--CSS file (default YUI Sam Skin) -->'."\n"
		.'<link type="text/css" rel="stylesheet" href="'.$GLOBALS['where_framework_relative'].'/addons/yui/datatable/assets/skins/sam/datatable.css">'."\n"
...
switch($GLOBALS['op']) {
	...
	case "modtestgui" : {
		$GLOBALS['page']->add('<script type="text/javascript" src="'.$GLOBALS['where_framework_relative'].'/lib/lib.elem_selector.js"></script>', 'page_head');
