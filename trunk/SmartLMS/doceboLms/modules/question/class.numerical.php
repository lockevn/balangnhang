...
class Numerical_Question extends Question {
	...
	function storeAnswer( $id_track, &$source, $can_overwrite = false ) {
		...
		FROM ".$GLOBALS['prefix_lms']	."_testquestanswer 
		WHERE idQuest = '".(int)$this->id."'");
		while(list($id_answer, $coded_answer, $score_corr, $score_incorr) = mysql_fetch_row($re_answer)) {
	...
	function displayUserResult( $id_track, $num_quest, $show_solution ) {
		...
		FROM ".$GLOBALS['prefix_lms']	."_testquestanswer 
		WHERE idQuest = '".(int)$this->id."'");
		while(list($id_answer, $coded_answer, $comment, $score_corr, $score_incorr) = mysql_fetch_row($re_answer)) {
