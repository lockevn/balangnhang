...
class Report {
	...
	function Report($id_report, $report_name = false) {
		...
		if($report_name == false) {
			...
			$this->report_name 	= $lang->def('_'.$report_name);
			$this->report_descr = $lang->def('_'.$report_name.'_DESCRIPTION');
		}
	}
	...
	function _load() {
		...
		list($report_name) = mysql_fetch_row($re_report);
	...
	function &getAllUserIdst() {
		...
		if( $userlevelid != ADMIN_GROUP_GODADMIN ) {
			...
			$p_dr->intersectGroupFilter(
				$adminManager->getAdminTree($GLOBALS['current_user']->getIdSt())
			);
		}
