...
function chooselevel() {
	...
	if ($course_info["course_type"] === "elearning") {
		...
		$tt=new TimeTable();
		$busy_users=$tt->getResourcesInUse("user", $course_info["date_begin"], $course_info["date_end"], TRUE, FALSE, TRUE);
	}
...
function backcourse($result = false) {
	...
	jumpTo('index.php?modname=course&op=course_list'.( $result !== false ? '&result='.$result : '' ) );
}
...
function subscribemod() {
	...
	//addScriptaculousJs();
	addYahooJs(array('json'=>'json-min.js'));
	$GLOBALS['page']->add(
		...
		.' function reloadInfo() {
			...
			var objAjax = YAHOO.util.Connect.asyncRequest("POST",
		        ...
		        {success: callback_change}, data
		    );
			YAHOO.util.Dom.get("extra_info").disabled = true;
		} '
		
		.' function callback_change(o) {
			
			var result = YAHOO.lang.JSON.parse(o.responseText);
			var table = YAHOO.util.Dom.get("subscribed_list");
			...
			YAHOO.util.Dom.get("extra_info").disabled = false;
		}'
	...
	$field = array(
		'name' => $lang->def('_FIRST_NAME_LAST_NAME'),
	...
	$content_h = array($lang->def('_USERNAME'),
		//$lang->def('_FIRST_NAME_LAST_NAME')
...
function waitinguser() {
	...
	$page_title = array(
		'index.php?modname=course&amp;op=course_list' => $lang->def('_COURSE', 'admin_course_managment', 'lms'),
		$course_info['name'],
...
{	
	...
	$out->add($form->getHidden('import_separator', 'import_separator', 1, ','));
...
function loadImportCourseUser2()
{
	...
	if (is_array($row = $src->get_first_row()) && !empty($row))
	{
		...
		else
		{
			...
			else
			{
				$user_error++;
				$id_user_added[$counter]['id_user'] = $id_user;
				$id_user_added[$counter]['status'] = '_ERROR';
			}
		}
	...
	$tb = new TypeOne(false, $lang->def('_USER_SUBSCRIBED'), $lang->def('_USER_SUBSCRIBED'));
	...
	$out->add(	getBackUi($back_url, $lang->def('_BACK'))
				...
				.'<b>'.$lang->def('_INSERT_ERROR').' : '.'</b>'.$user_error.' '
...
function subscribeFromCourse()
{
	...
	$id_course = get_req('id_course', DOTY_INT, 0);
	$edition_id = get_req('edition_id', DOTY_INT, 0);
	...
	if(isset($_POST['subscribe_import']))
	{
		...
		$id_course = get_req('id_course', DOTY_INT, 0);
		$level = get_req('level', DOTY_INT, 0);
		$status = get_req('status', DOTY_INT, 0);
		$alert = get_req('alert', DOTY_INT, 0);
		
		$query =	"SELECT idUser, level"
					." FROM ".$GLOBALS['prefix_lms']."_courseuser"
		...
		while(list($id_user, $lv_sel) = mysql_fetch_row($result)) {
	...
	$array_level['0'] = $lang->def('_ANY');
	
	$arr_status = array(	'-2'					=> $lang->def('_ANY'),
							_CUS_CONFIRMED 		=> $lang->def('_USER_STATUS_CONFIRMED'),
							_CUS_SUBSCRIBED 	=> $lang->def('_USER_STATUS_SUBS'),
							_CUS_BEGIN 			=> $lang->def('_USER_STATUS_BEGIN'),
							_CUS_END 			=> $lang->def('_USER_STATUS_END'),
	...
	$out->add(	getTitleArea($lang->def('_TITLE_SUBSCRIBE_FROM_COURSE'))
				.'<div class="std_block">'
				...
				.Form::getCheckbox($lang->def('_SEND_ALERT'), 'alert', 'alert', '1')
				.Form::closeElementSpace());
