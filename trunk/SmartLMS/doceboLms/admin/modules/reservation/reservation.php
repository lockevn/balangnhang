...
if(!$GLOBALS['current_user']->isAnonymous())
{
	...
	function viewEvent()
	{
		...
		if(count($events))
		{
			$tb = new TypeOne(10, $lang->def('_EVENT_CAPTION'), $lang->def('_EVENT_SUMMARY'));
			...
			if ($mod_perm)
			{
				...
				(
					...
					$lang->def('_AVAILABLE_PLACES'),
					$lang->def('_DEADLINE'),
					'<img src="'.getPathImage().'chat/user_perm.gif" title="'.$lang->def('_SET_ROOM_VIEW_PERM').'" alt="'.$lang->def('_SET_ROOM_VIEW_PERM').'" />',
			...
			if ($events)
			{
				foreach($events as $event)
				{
					...
					if ($mod_perm)
					{
						...
						$count[] = '<a href="index.php?modname=reservation&amp;op=mod_event&amp;id_event='.$event[EVENT_ID].'"><img src="'.getPathImage().'/standard/mod.gif" title="'.$lang->def('_MODT').'" alt="'.$lang->def('_MODT').'" /></a>';
			...
			if($mod_perm)
			{
				$tb->addActionAdd('<a href="index.php?modname=reservation&amp;op=add_event">
					<img src="'.getPathImage().'standard/add.gif" title="'.$lang->def('_ADD_EVENTT').'" alt="'.$lang->def('_ADD_EVENTT').'" /> '
					.$lang->def('_ADD_EVENTT').'</a>');
			}
			$out->add($tb->getTable()
		...
		else
		{
			if($mod_perm)
			{
				$out->add('<div class="events_action_top"><p><a href="index.php?modname=reservation&amp;op=add_event">'.$lang->def('_ADD_EVENTT').'</a></p></div>', 'content');
			}
	...
	function addEvent()
	{
		...
		if ($date < date('Y-m-d') || $date < $deadline)
				jumpTo('index.php?modname=reservation&op=add_event&amp;error=date');
			
			if ($from_time >= $to_time)
				jumpTo('index.php?modname=reservation&op=add_event&amp;error=time');
			
			if ($confirm)
				$result = $man_res->addEvents($id_course, $id_laboratory, $id_category, $title, $description, $date, $max_user, $deadline, $from_time, $to_time);
			if ($result)
				jumpTo('index.php?modname=reservation&op=view_event');
			jumpTo('index.php?modname=reservation&op=add_event&amp;error=laboratory');
		}
	...
	function modEvent()
	{
		...
		(
			...
			.Form::getDropdown($lang->def('_LOCATION'), 'id_laboratory', 'id_laboratory', $man_res->getLaboratories(), $event[EVENT_ID_LABVORATORY])
	...
	function delEvent()
	{
		...
		if (get_req('confirm', DOTY_INT, 0) == 1)
		{
			...
			if ($result)
				...
				jumpTo('index.php?modname=reservation&op=view_event&error=del_event');
		}
	...
	function viewCategoy()
	{
		...
		$tb = new TypeOne(10, $lang->def('_CATEGORY_CAPTION'), $lang->def('_CATEGORY_SUMMARY'));
			$tb->initNavBar('ini', 'button');
			...
			if ($category)
			{
				foreach($category as $categ)
				{
					...
					if ($mod_perm)
					{
						$count[] = '<a href="index.php?modname=reservation&amp;op=mod_category&amp;id_category='.$categ[CATEGORY_ID].'"><img src="'.getPathImage().'/standard/mod.gif" title="'.$lang->def('_MODT').'" alt="'.$lang->def('_MOD').'" /></a>';
			...
			if($mod_perm)
			{
				$tb->addActionAdd('<a href="index.php?modname=reservation&amp;op=add_category">
					<img src="'.getPathImage().'standard/add.gif" title="'.$lang->def('_ADD_CATEGORYT').'" alt="'.$lang->def('_ADD_CATEGORYT').'" /> '
					.$lang->def('_ADD_CATEGORYT').'</a>');
			}
			$out->add($tb->getTable()
	...
	function addCategoy()
	{
		...
		if (isset($_GET['confirm']))
		{
			...
			if ($name == '')
				jumpTo('index.php?modname=reservation&op=add_category&amp;error=name');
			
			if ($confirm)
				$result = $man_res->addCategory($name, $max_subscription);
			if ($result)
				jumpTo('index.php?modname=reservation&op=view_category');
			jumpTo('index.php?modname=reservation&op=add_category&amp;error=category');
		}
		...
		if(count($events))
		{
			...
			if ($mod_perm)
			{
				...
				(
					...
					$lang->def('_NUMBER_SUBSCRIBED'),
					$lang->def('_DEADLINE'),
					'<img src="'.getPathImage().'/standard/user.gif" title="'.$lang->def('_VIEW_USER_SUBSCRIBED').'" alt="'.$lang->def('_VIEW_USER_SUBSCRIBED').'" />',
					'<img src="'.getPathImage().'/standard/add_subscribe.gif" title="'.$lang->def('_ADD_USER').'" alt="'.$lang->def('_ADD_USER').'" />',
					'<img src="'.getPathImage().'/standard/mod.gif" title="'.$lang->def('_MOD').'" alt="'.$lang->def('_MOD').'" />',
					'<img src="'.getPathImage().'/standard/rem.gif" title="'.$lang->def('_DEL').'" alt="'.$lang->def('_DEL').'" />'
				);
			}
			...
			foreach($events as $event)
			{
				...
				if ($mod_perm)
				{
					$count[] = '<a href="index.php?modname=reservation&amp;op=view_user_event&amp;id_event='.$event[EVENT_ID].'"><img src="'.getPathImage().'/standard/user.gif" title="'.$lang->def('_VIEW_USER_SUBSCRIBED').'" alt="'.$lang->def('_VIEW_USER_SUBSCRIBED').'" /></a>';
					$count[] = '<a href="index.php?modname=reservation&amp;op=add_registration&amp;id_event='.$event[EVENT_ID].'&amp;id_course='.$event[EVENT_ID_COURSE].'"><img src="'.getPathImage().'/standard/add_subscribe.gif" title="'.$lang->def('_ADD_USER').'" alt="'.$lang->def('_ADD_USER').'" /></a>';
					$count[] = '<a href="index.php?modname=reservation&amp;op=mod_event&amp;id_event='.$event[EVENT_ID].'"><img src="'.getPathImage().'/standard/mod.gif" title="'.$lang->def('_MODT').'" alt="'.$lang->def('_MODT').'" /></a>';
					$count[] = '<a href="index.php?modname=reservation&amp;op=del_event&amp;id_event='.$event[EVENT_ID].'"><img src="'.getPathImage().'/standard/rem.gif" title="'.$lang->def('_DEL').'" alt="'.$lang->def('_DEL').'" /></a>';
				}
				$tb->addBody($count);
		...
		if ($user_info)
		{
			...
			if ($mod_perm)
			{
				...
				(
					...
					$lang->def('_LASTNAME'),
					$lang->def('_EMAIL'),
					'<img src="'.getPathImage().'/standard/rem.gif" title="'.$lang->def('_REM_USER').'" alt="'.$lang->def('_REM_USER').'" />'
				);
				$type_h = array('', '', '', '', 'img');
			}
			...
			if ($user_info)
			{
				foreach ($user_info as $info_user)
				{
					...
					$count[] = '<a href="index.php?modname=reservation&amp;op=del_registration&amp;id_user='.$info_user[ACL_INFO_IDST].'&amp;id_event='.$id_event.'"><img src="'.getPathImage().'/standard/rem.gif" title="'.$lang->def('_REM_USER').'" alt="'.$lang->def('_REM_USER').'" /></a>';
	...
	function sendRegistration()
	{
		...
		if ($wrong_result)
			jumpTo('index.php?modname=reservation&op=view_registration&error=insert');
		jumpTo('index.php?modname=reservation&op=view_registration');
	}
	...
	function delRegistration()
	{
		...
		if (get_req('confirm', DOTY_INT, 0) == 1)
		{
			...
			if ($result)
				jumpTo('index.php?modname=reservation&op=view_user_event&id_event='.$id_event);
			jumpTo('index.php?modname=reservation&op=view_user_event&id_event='.$id_event.'&amp;error=del_registration');
		}
...
function setRoomViewPerm()
{
	...
	if( isset($_POST['okselector']) )
	{
		...
		list($id_course) = mysql_fetch_row(mysql_query("SELECT idCourse FROM ".$GLOBALS['prefix_lms']."_reservation_events WHERE idEvent = '".$id_event."'"));
