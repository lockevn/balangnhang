...
function news() {
	...
	if($mod_perm) {
		$tb->addActionAdd(
			'<a href="index.php?modname=news&amp;op=addnews" title="'.$lang->def('_TITLE_NEW_NEWS').'">'
	...
	if($mod_perm) {
		...
		$out->add(
			...
			.$form->getTextfield($lang->def('_NEWSINHOMEPAGE'), 'howmuch', 'howmuch', 3, $GLOBALS['lms']['visuNewsHomePage'])
...
function savenews() {
	...
	if($load == 1) {
		...
		WHERE idNews = '".$id_news."'";
		if(!mysql_query($query_insert)) jumpTo('index.php?modname=news&op=news&result=err');
		jumpTo('index.php?modname=news&op=news&result=ok');
	} else {
...
function delnews() {
	...
	if(get_req('confirm', DOTY_INT, 0) == 1) {
		...
		WHERE idNews = '".$id_news."'";
		if(!mysql_query($query_news)) jumpTo('index.php?modname=news&op=news&result=err_del');
		else jumpTo('index.php?modname=news&op=news&result=ok');
	} else {
