		...
		while(list( $var_name, $var_value, $value_type, $max_size ) = mysql_fetch_row( $reSetting ) ) {
			...
			switch( $value_type ) {
				...
				case "sel_news" : {
					$mode = array(
						'off' => def('_DONT_SHOW'), 
						'link' => def('_SHOW_AS_LINK'), 
						'block' => def('_SHOW_AS_BLOCK'));
					$html .= Form::getDropdown( $lang->def('_'.strtoupper($var_name)), 
												$var_name, 
												'option['.$var_name.']', 
												$mode, 
				...
				case "tablist_coursecatalogue" : {
					...
					$lang_c 	=& DoceboLanguage::createInstance('coursecatalogue', 'lms');
					...
					$tab_list = array(
						...
						'all' 		=> $lang_c->def('_TAB_VIEW_ALL')
					);
					if($GLOBALS['lms']['use_coursepath'] == '1') {
						$tab_list['pathcourse'] = $lang_c->def('_TAB_VIEW_PATHCOURSE');
					}
