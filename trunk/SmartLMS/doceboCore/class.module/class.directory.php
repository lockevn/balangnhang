...
class Module_Directory extends Module {
	...
	function directory_create_TabView( $op ) {
		global $_tab_op_map;
		$arr_tabs = array();
		require_once(dirname(__FILE__).'/../lib/lib.tab.php');
	...
	function org_manageuser() {
		...
		$profile->init('profile', 'lms', 'modname=directory&op=org_manageuser&id_user='.importVar('id_user', true, 0), 'ap');
	...
	function loadPeopleView($url = '') {
		...
		if( $lv->getOp() == 'newitem' ) {
			...
			if( !$this->selector_mode ) {
				...
				$GLOBALS['page']->add( '<div class="std_block">', 'content' );
				$GLOBALS['page']->addEnd( '</div>', 'content' );
				$GLOBALS['page']->add( '<form id="dirctory_listpeople" action="index.php?modname=directory&amp;op=listuser" method="post">', 'content' );
...
*/
	...
	function deleteGroup($groupid) {
		...
		$GLOBALS['page']->add( getTitleArea($this->lang->def( '_DIRECTORY_GROUPVIEWTITLE' )
								.': '.$this->lang->def('_DIRECTORY_DELGROUP'), 'directory_group' ), 'content' );
		...
		$GLOBALS['page']->add( '<div class="std_block">'
			...
			.'title="'.$this->lang->def('_DIRECTORY_DELGROUP').' '.$groupid.'"/><b>'
			.$groupid.'</b> [ '
			.$description.']', 'content');
		$GLOBALS['page']->add( '</div>', 'content');
		...
		$GLOBALS['page']->add( Form::getHidden('idst', 'idst', $idst), 'content' );
		$GLOBALS['page']->add( Form::openButtonSpace(), 'content' );
		$GLOBALS['page']->add( Form::getButton("deletegroup","deletegroup",$this->lang->def( '_DIRECTORY_DELGROUP' )), 'content' );
	...
	function editGroup($groupid = FALSE) {
		...
		$GLOBALS['page']->add( '<div class="std_block">', 'content');
		$GLOBALS['page']->add( Form::getFormHeader( $this->lang->def( '_DIRECTORY_EDITGROUP' ) ), 'content' );
		$GLOBALS['page']->add( Form::openForm( 	'directoryeditgroup',
								'index.php?modname=directory&amp;op=listgroup'),
								'content' );
		$GLOBALS['page']->add( Form::openElementSpace(), 'content' );

		$GLOBALS['page']->add( Form::getOpenFieldset( $this->lang->def( '_DIRECTORY_EDITGROUP' ).' - '.$groupLabel ), 'content' );
	...
	function waitingUserGroup($groupid) {
		...
		$tb = new TypeOne(0, $this->lang->def( '_DIRECTORY_GROUPWAIT_ACCORDECLINE' ),
	...
	function membersGroup($groupid, $simple = FALSE) {
		...
		if( $simple === FALSE ) {
			if( isset( $_POST['membersgroupadd'] ) )
				jumpTo( 'index.php?modname=directory&op=addtogroup&groupid='.$groupid );
			if( isset( $_POST['membersgroupcancel'] ) )
				jumpTo( 'index.php?modname=directory&op=listgroup' );
			$GLOBALS['page']->add( getTitleArea($this->lang->def( '_DIRECTORY_GROUPVIEWTITLE' )
	...
	function importToGroup() {
		...
		$tree = getTitleArea( $this->lang->def( '_IMPORT_GROUPUSER_STEP_1' ), 'directory_group' )
			.'<div class="std_block">'

			.getInfoUi($this->lang->def('_IMPORT_GROUPUSER_INSTRUCTION') )

			.$form->getFormHeader( $this->lang->def( '_IMPORT_GROUPUSER' ) )
	...
	function importToGroup_step2() {
		...
		if($_FILES['file_import']['name'] == '') {
			...
			jumpTo( 'index.php?modname=directory&amp;op=listgroup&import_result=-1' );
		} else {
			$path = '/doceboCore/';
		...
		$tree = getTitleArea( $this->lang->def( '_IMPORT_GROUPUSER_STEP_2' ), 'directory_group' )
			.'<div class="std_block">'
	...
	function importToGroup_step3() {
		...
		$tree .= getResultUi(str_replace("[count]", $result[0], $this->lang->def('_GROUP_IMPORT_GROUPUSER_COUNT')) );

		if( count($result) > 1 ) {
			require_once($GLOBALS['where_framework'].'/lib/lib.newtypeone.php');
			$tree .= str_replace("[count]", count($result)-1, $this->lang->def('_GROUP_IMPORT_GROUPUSER_ERR_COUNT'));
			$table = new TypeOne($GLOBALS['visuItem'], $this->lang->def('_GROUP_IMPORT_GROUPUSER_ERR_TABLE'), $this->lang->def('_GROUP_IMPORT_GROUPUSER_ERR_TABLE'));
			$table->setColsStyle(array('',''));
			$table->addHead(array(	$this->lang->def('_GROUP_IMPORT_GROUPUSER_ERR_ROW'),
									$this->lang->def('_GROUP_IMPORT_GROUPUSER_ERR_TEXT')
							));
		...
		if( $treeView->op != '' ) {
			...
			switch( $treeView->op ) {
				...
				case 'create_user':
					...
					$processed = TRUE;
					jumpTo( 'index.php?modname=directory&op=org_createuser&treeid='.$treeView->getSelectedFolderId() );
				break;
				case 'addtotree':
					$processed = TRUE;
					jumpTo( 'index.php?modname=directory&op=addtotree&treeid='.$treeView->getSelectedFolderId() );
				break;
				case 'waiting_user':
					$processed = TRUE;
					jumpTo( 'index.php?modname=directory&op=org_waitinguser&treeid='.$treeView->getSelectedFolderId() );
				break;
			}
	...
	function addToTree($treeid) {
		...
		if( isset($_POST['okselector']) ) {
			...
			jumpTo( 'index.php?modname=directory&op=org_chart' );
		} elseif( isset($_POST['cancelselector']) ) {
			jumpTo( 'index.php?modname=directory&op=org_chart' );
		} else {
			if( !isset($_GET['stayon']) ) {
	...
	function org_createUser( $treeid = FALSE ) {
		...
		list($number_of_folder) = mysql_fetch_row(mysql_query($query_org_chart));
		...
		if($control_view && ($GLOBALS['use_org_chart'] == '1' || $GLOBALS['use_groups'] == '1') ) {
			if( isset($_POST['okselector']) ) {
				...
				if( $GLOBALS['use_org_chart'] == '1' && $number_of_folder != 0) {
	...
	function org_waitingUser() {
		...
		if( isset($_POST['ok_waiting']) ) {
			...
			$cont_h = array(
				...
				'<img src="'.getPathImage('framework').'directory/wuser_accept.gif" alt="'.$this->lang->def('_ACCEPT_USER').'" '
	...
	function loadAssignField2( $groupid ) {
		...
		$GLOBALS['page']->add( '<div class="std_block">');
		$GLOBALS['page']->add( $form->getFormHeader( $this->lang->def( '_DIRECTORY_ASSIGNFIELDGROUP_MANDATORY' ) ));
		...
		$GLOBALS['page']->add(
				...
				.'<div class="label_head">'.$this->lang->def('_DIRECTORY_GROUP_FIELD_MANDATORY').'</div>'
		...
		foreach( $arr_fields as $id_filed => $status ) {
			if( $status == GROUP_FIELD_NORMAL ) {
				...
				$GLOBALS['page']->add( ' />' );
				$GLOBALS['page']->add( $form->getLabel( DIRECTORY_ID.'_'.$id_filed.'_mandatory', $this->lang->def('_DIRECTORY_GROUP_FIELD_MANDATORY'), 'label_bold access-only' ) );
