...
$GLOBALS['page']->add(
		'<link href="'.$GLOBALS["where_framework_relative"].'/templates/'.getTemplate().'/style/style_treeview.css" rel="stylesheet" type="text/css" />'."\n"
...
function show_personal_media(& $out, & $lang) {
	...
	if($GLOBALS['framework']['hteditor'] == 'tinymce') {
	...
	$head[] = "<img src=\"".getPathImage()."standard/attach.gif\" alt=\"".$lang->def("_ATTACHMENT")."\" title=\"".$lang->def("_ATTACHMENT")."\" />";
	...
	if (($q) && (mysql_num_rows($q) > 0)) {
		while($row = mysql_fetch_array($q)) {
			...
			if (!empty($row["media_url"])) {
				...
				$rowcnt[] = "<img src=\"".$file."\" alt=\"".$row["fname"]."\" title=\"".$row["fname"]."\" />";
			}
			...
			$img = "<img src=\"".getPathImage('fw')."standard/attach.gif\" alt=\"".$lang->def("_ATTACHMENT")."\" title=\"".$lang->def("_ATTACHMENT")."\" />\n";
			$rowcnt[] = '<a href="'.$sel_url.'" '
				.( $GLOBALS['framework']['hteditor'] == 'tinymce'
					? 'onclick="FileBrowserDialogue.mySubmit(\''.cleanUrlPath($site_url.$row["real_fname"]).'\'); return false;"' : '' )
				.'>'
				.$img."</a>\n";

			$img = "<img src=\"".getPathImage('fw')."standard/rem.gif\" alt=\"".$lang->def("_DEL")."\" title=\"".$lang->def("_DEL")."\" />\n";
	...
	if(isset($_GET['result'])) {
		switch($_GET['result']) {
			case "invalid_ext" : $out->add(getErrorUi($lang->def('_INVALID_EXTENSION')));break;
			case "upload_err" 	: $out->add(getErrorUi($lang->def('_ERROR_UPLOAD')));break;
			case "upload_ok" 	: $out->add(getResultUi($lang->def('_OPERATION_SUCCESSFUL')));break;
		}
	}
...
function ins_personal_media() {
	...
	jumpTo($url.'&result='.( $q ? 'upload_ok' : 'upload_err' ));
}
...
function del_personal_media(& $out, & $lang) {
	...
	if (isset($_POST["canc_del"])) {
		jumpTo(getPopupBaseUrl()."&amp;op=personal");
	}
		...
		$out->add($form->openForm("popup_form", getPopupBaseUrl()."&amp;op=delpersonal"));
...
function show_cms_media(& $out, & $lang) {
	...
	*/
	$path =(strlen(dirname($_SERVER['PHP_SELF'])) != 1 ? dirname($_SERVER['PHP_SELF']) : '' ).'/';
	$path.=$GLOBALS["where_files_relative"];
	$site_url=cleanUrlPath("http://".$_SERVER['HTTP_HOST'].$path._FPATH_INTERNAL);
	...
	switch ($type) {
		...
		case "streaming": {
			...
			if (!empty($media_url)) {
				...
				$site_file_path =cleanUrlPath(getPLSetting($current_pl, "url").$base_relative_path);
