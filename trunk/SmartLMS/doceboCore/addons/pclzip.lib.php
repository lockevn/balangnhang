...
//    $v_path = "./";
  ...
  function properties()
  {
    ...
    // ----- Look if file exists
    if (@sl_is_file($this->zipname))
    {
      // ----- Open the zip file
      //--(MAGIC-PclTrace)--//PclTraceFctMessage(__FILE__, __LINE__, 3, "Open file in binary read mode");
      if (($this->zip_fd = @sl_fopen($this->zipname, 'rb')) == 0)
      {
        $this->privSwapBackMagicQuotes();
...
// --------------------------------------------------------------------------------
  ...
  function privCheckFormat($p_level=0)
  {
    ...
    if (!sl_is_readable($this->zipname)) {
      // ----- Error log
  ...
  function privFileDescrExpand(&$p_filedescr_list, &$p_options)
  {
    ...
    for ($i=0; $i<sizeof($p_filedescr_list); $i++) {
      ...
      if (file_exists($v_descr['filename'])) {
        if (@sl_is_file($v_descr['filename'])) {
  ...
  function privAdd($p_filedescr_list, &$p_result_list, &$p_options)
  {
    ...
    if ((!sl_is_file($this->zipname)) || (sl_filesize($this->zipname) == 0))
    {
    ...
    //--(MAGIC-PclTrace)--//PclTraceFctMessage(__FILE__, __LINE__, 3, "Open file in binary read mode");
    if (($v_zip_temp_fd = @sl_fopen($v_zip_temp_name, 'wb')) == 0)
    {
      $this->privCloseFd();
      $this->privSwapBackMagicQuotes();
  ...
  function privOpenFd($p_mode)
  {
    ...
    if (($this->zip_fd = @sl_fopen($this->zipname, $p_mode)) == 0)
    {
      // ----- Error log
...
//      if (   (sl_is_file($p_filedescr_list[$j]['filename']))
...
//      if (sl_is_file($p_filename))
      if (   ($p_filedescr['type'] == 'file')
        ...
        if ($p_filedescr['type'] == 'file') {        
          ...
          if (($v_file = @sl_fopen($p_filename, "rb")) == 0) {
  ...
  function privExtractFile(&$p_entry, $p_path, $p_remove_path, $p_remove_all_path, &$p_options)
  {
    ...
    if (sl_file_exists($p_entry['filename']))
    {
      ...
      else if (!sl_is_writeable($p_entry['filename']))
      {
      ...
      else if (sl_filemtime($p_entry['filename']) > $p_entry['mtime'])
      {
    ...
    if ($p_entry['status'] == 'ok') {
      ...
      if (!(($p_entry['external']&0x00000010)==0x00000010))
      {
        ...
        if ($p_entry['compression'] == 0) {
          ...
          if (($v_dest_file = @sl_fopen($p_entry['filename'], 'wb')) == 0)
          {
          ...
          sl_touch($p_entry['filename'], $p_entry['mtime']);
        ...
        if (isset($p_options[PCLZIP_OPT_SET_CHMOD])) {
          ...
          @sl_chmod($p_entry['filename'], $p_options[PCLZIP_OPT_SET_CHMOD]);
        }
...
//      if ($p_entry['compressed_size'] == $p_entry['size'])
  ...
  function privReadEndCentralDir(&$p_central_dir)
  {
    ...
    $v_size = sl_filesize($this->zipname);
  ...
  function privMerge(&$p_archive_to_add)
  {
    ...
    if (!sl_is_file($p_archive_to_add->zipname))
    {
