...
class  Cpdf {
   ...
   */
  function  Cpdf ($pageSize = array(0, 0, 612, 792), $orientation = 'P') {
	...
	function getPdf($html, $name, $img = false, $download = true, $facs_simile = false)
	{
  ...
  $this->line(16, $y, $w - 16, $y, $color, 1);

  $y += $text_height;

  $text = "Job: 132-003";
  $this->text(16, $y, $text, $font, $size, $color);

  $this->close_object();
  $this->add_object($foot, "all");
  ...
  $this->text($w - 16 - $width - 38, $y, $text, $font, $size, $color);
  $this->rectangle($w - 16 - 36, $y - 2, 36, $text_height + 4, array(0.5,0.5,0.5), 0.5);
    

  $this->close_object();
  $this->add_object($initials);
 
  // Mark the document as a duplicate
  $this->text(110, $h - 240, "DUPLICATE", Font_Metrics::get_font("verdana", "bold"),
             110, array(0.85, 0.85, 0.85), 0, -52);

  $text = "F a c s - s i m i l e";  

  // Center the text
  $width = Font_Metrics::get_text_width($text, $font, $size);
  $this->page_text($w / 2 - $width / 2, $y, $text, $font, $size, $color);
</script>
