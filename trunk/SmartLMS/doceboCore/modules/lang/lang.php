...
require_once( dirname(__FILE__) . "/../../lib/lib.lang.php" );

function canEditLang($lang_code) {
	
	if($GLOBALS['current_user']->getUserLevelId() == ADMIN_GROUP_GODADMIN) return true;
	$langs =  unserialize(urldecode($GLOBALS['current_user']->preference->getPreference('admin_rules.user_lang_assigned')));
	if(isset($langs[$lang_code])) return true;
	return false;
}
...
function lang_lang() {
	...
	require_once(dirname(__FILE__) . "/../../lib/lib.typeone.php");
	
	$arrLang = $GLOBALS['globLangManager']->getAllLanguages();
	sort($arrLang);
	$stats = $GLOBALS['globLangManager']->getLangStat();
	...
	$tableLang = new typeOne( $visuItem );
	...
	$contentH = array( 	$lang->def( '_LANGUAGE' ), 
						...
						'<img src="'.getPathImage().'standard/modelem.gif" alt="'.$lang->def( '_TRANSLATELANG' ).'" title="'.$lang->def( '_TRANSLATELANGG' ).'" />');
	$typeH = array( '', '', '', 'img');
	
	if($modPerm) {
		$contentH[] = '<img src="'.getPathImage().'standard/mod.gif" alt="'.$lang->def( '_SAVE' ).'" title="'.$lang->def( '_SAVE' ).'" />';
		$typeH[] = 'img';
	}
	if($remPerm) {
		$contentH[] = '<img src="'.getPathImage().'standard/rem.gif" alt="'.$lang->def( '_DEL' ).'" title="'.$lang->def( '_DEL' ).'" />';
		$typeH[] = 'img';
	}
	/*
	...
	*/
	...
	for( $index = $ini; $index < $maxItem; $index++ ) {
		...
		$rowContent[] = '<a href="index.php?modname=lang&amp;op=translator&amp;tranm='
						...
						.'<img src="'.getPathImage().'standard/modelem.gif" alt="'
		...
		if($modPerm) {
			if(canEditLang($arrLang[$index][0])) {
				...
				$rowContent[] = '<a href="index.php?modname=lang&amp;op=modlang&amp;lang_code='.str_replace(' ', '_', $rowContent[0]).'">'
								.'<img src="'.getPathImage().'standard/mod.gif" alt="'.$lang->def( '_SAVE' ).'" /></a>';
			} else $rowContent[] = '';
		}
		if($remPerm) {
			if(canEditLang($arrLang[$index][0])) {
		...
		*/
		if(!isset($stats[$arrLang[$index][0]])) {			
			...
			$rowContent[] = drawProgressBar(0, true, FALSE, FALSE,FALSE, FALSE);
			$rowContent[] = '0';

		} else {

			$percent = (int)(($stats[$arrLang[$index][0]] / $stats['tot_lang'])*100);
			$rowContent[] = drawProgressBar(	$percent, true, FALSE, FALSE,FALSE, FALSE);			
	...
	if($newPerm) {
		$GLOBALS['page']->add($tableLang->WriteAddRow('<a href="index.php?modname=lang&amp;op=addlang">
							<img src="'.getPathImage().'standard/add.gif" title="'.$lang->def( '_ADD' ).'" alt="'.$lang->def( '_ADD' ).'" /> '.$lang->def( '_ADD' ).'</a>'),
							'content');
	}
...
function lang_editlang( $lang_code = FALSE ) {
	...
	require_once(dirname(__FILE__) . "/../../lib/lib.form.php");
	$lang =& DoceboLanguage::createInstance('admin_lang', 'framework');
...
function lang_uplang() {
	...
	if(isset($_POST['editkeycancel'])) jumpTo( 'index.php?modname=lang&op=lang' );
	...
	if( $globLangManager->existLanguage($lang_code) ) {
		...
		if( !$globLangManager->updateLanguage(	$lang_code, 
			...
			$GLOBALS['page']->add( getErrorUi( $lang->def( '_ERRUPDATE' ) ), 'content' );
	} else {
		...
		if( !$globLangManager->insertLanguage(	$lang_code, 
												...
												$_POST['lang_browsercode'],
												$_POST['lang_direction']) )
			$GLOBALS['page']->add( getErrorUi( $lang->def( '_ERRINSERT' ) ), 'content' );
	}
	jumpTo( 'index.php?modname=lang&op=lang' );
}
...
function lang_dellang() {
	...
	if( isset($_GET['confirm']) && ($_GET['confirm'] == '1')) {
		...
		if(!$globLangManager->deleteLanguage($lang_code)) {
			$GLOBALS['page']->add( getErrorUi( $lang->def( '_ERRREM' ) ), 'content' );
			return;
		}
		jumpTo( 'index.php?modname=lang&op=lang' );
	}
...
function lang_translator() {
	...
	*/
	...
	$platformf			= importVar('platformf',FALSE, 	$GLOBALS['platform'] );
	...
	if( isset($_POST['addkey']) ) {
		...
		if( $modulef == '' ) {
			$GLOBALS['page']->add( getErrorUi( $lang->def( '_ERRINCORRECTPARAMS' ) ), 'content' );
			return;			
		}
...
function lang_translator_listKey($modulef, $platformf, $tranm, $tranc, $onlyempty, $trans_contains, $order_by = false) {
	...
	*/
	...
	$arrModules = $GLOBALS['globLangManager']->getAllModules($platformf);
	$all_module = array();
	...
	$arrLanguages = $GLOBALS['globLangManager']->getAllLanguages();
	...
	if($tranc != '') {
		$browser_code = $GLOBALS['globLangManager']->getLanguageBrowsercode($tranc);
	...
	// show filter ------------------------------------------------------------------
	$GLOBALS['page']->add(
		Form::getOpenFieldset($lang->def( '_FILTERDATA' ))
		...
		.Form::getButton( "applyfilter", "applyfilter", $lang->def( '_LANG_APPLY' ), 'button_nowh' )
	...
	$GLOBALS['page']->add( join('","', $GLOBALS['globLangManager']->getAllModules('framework')), 'page_head' );
	$GLOBALS['page']->add( '"];'."\n", 'page_head');
	$GLOBALS['page']->add( 'var arrLmsModules = ["', 'page_head' );
	$GLOBALS['page']->add( join('","', $GLOBALS['globLangManager']->getAllModules('lms')), 'page_head' );
	$GLOBALS['page']->add( '"];'."\n", 'page_head');
	$GLOBALS['page']->add( 'var arrCmsModules = ["', 'page_head' );
	$GLOBALS['page']->add( join('","', $GLOBALS['globLangManager']->getAllModules('cms')), 'page_head' );
	$GLOBALS['page']->add( '"];'."\n", 'page_head');
	$GLOBALS['page']->add( 'var arrScsModules = ["', 'page_head' );
	$GLOBALS['page']->add( join('","', $GLOBALS['globLangManager']->getAllModules('scs')), 'page_head' );
	$GLOBALS['page']->add( '"];'."\n", 'page_head');
	$GLOBALS['page']->add( 'var arrEcomModules = ["', 'page_head' );
	$GLOBALS['page']->add( join('","', $GLOBALS['globLangManager']->getAllModules('ecom')), 'page_head' );
	$GLOBALS['page']->add( '"];'."\n", 'page_head');
	$GLOBALS['page']->add( 'var arrCrmModules = ["', 'page_head' );
	$GLOBALS['page']->add( join('","', $GLOBALS['globLangManager']->getAllModules('crm')), 'page_head' );
	$GLOBALS['page']->add( '"];'."\n", 'page_head');
	...
	$order_symbol = '<img src="'.getPathImage('fw').'standard/down.gif" alt="'.$lang->def('_ORDERED_BY').'" /> ';
	
	$tableLang = new typeOne(0);
	$GLOBALS['page']->add( $tableLang->OpenTable( preg_replace('/%module%/', $modulef, $lang->def( '_TRANSALTIONTABLEFOR' )) ), 'content');
	
	$contentH = array( 	$lang->def( '_LANG_MODULE' ), 
		$lang->def( '_LANG_KEY' ), 
		$lang->def( '_LANG_ATTRIBUTES' ),
		$lang->def( '_LANG_TRANSLATION' ).': <span xml:lang="en">'.$tranm.'</span>' 
	);
	
	$typeH = array( '', '', 'image nowrap', 'text_translation');
	if($tranc != '') {
		
		$contentH[] = $lang->def( '_LANG_TRAN_COMPARE' ).': <span xml:lang="en">'.$tranc.'</span>';
		$typeH[] = '';
	}
	$contentH[] = '<img src="'.getPathImage().'standard/mod.gif" alt="'.$lang->def( '_TRANSLATELANG' ).'" title="'.$lang->def( '_TRANSLATELANGG' ).'" />';
	$typeH[] = 'img';
	if($can_edit) {
		$contentH[] = '<img src="'.getPathImage().'standard/rem.gif" alt="'.$lang->def( '_DEL' ).'" title="'.$lang->def( '_DEL' ).'" />';
		$typeH[] = 'img';
	...
	$arrTranslations = $GLOBALS['globLangManager']->getModuleLangTranslations($platformf,$module_param,$tranm, $trans_contains, $attributes, $order_by );
	if( $tranc != '' ) {
		$arrCompare = $GLOBALS['globLangManager']->getModuleLangTranslations($platformf,$module_param,$tranc, $trans_contains, $attributes, 'translation');
	}
	$t = $trans_contains != '';
	$e = $onlyempty == '1';
		...
		if( ( $r && !$e ) || ( !$r && !$t )) {
			...
			if( in_array( 'accessibility', $arr_attributes ) )
				$str_attributes = '<img src="'.getPathImage().'standard/accessibility.gif"'
								...
								.' title="'.$lang->def( '_LANG_ACCESSIBILITYTITLE' ).'" >';
			...
			$rowContent[] = '<input type="submit" 
								...
								style="background-image: url( \''.getPathImage().'standard/mod.gif\' )"
			...
			if($can_edit) {
				...
				$rowContent[] = '<input type="submit" 
									class="graphic_submit22"
									style="background-image: url( \''.getPathImage().'standard/rem.gif\' )"
...
function lang_edit_key($key, $modulef, $platformf, $tranm, $tranc, $onlyempty, $other_filter = false, $order_by = '') {
	...
	$GLOBALS['page']->add( Form::getSimpleTextarea( $lang->def( '_DESCRIPTION' ), 
							"description", 
							"description",
							($key != '')?$GLOBALS['globLangManager']->getKeyDescription($key,$modulef,$platformf):''), 'content' );
							
	$arr_attributes = split( ',', $GLOBALS['globLangManager']->getKeyAttributes($key,$modulef,$platformf) );
	...
	foreach( $arrLanguages as $langElem) {
		...
		$translation_text = $GLOBALS['globLangManager']->getLangTranslationText($langElem[0], $key, $modulef,$platformf);
...
function lang_del_key($key, $modulef, $platformf, $tranm, $tranc, $onlyempty) {
	...
	$GLOBALS['page']->add( getTitleArea($lang->def('_LANGUAGE_MANAGMENT'), 'manlanguage', $lang->def('_LANGUAGE_MANAGMENT'))
		.'<div class="std_block">', 'content');
	//$GLOBALS['page']->add( Form::getFormHeader( $lang->def( '_LANG_DELKEY' ) ), 'content' );
	...
	$GLOBALS['page']->add( '<div class="boxinfo_title">'
		.$lang->def( '_AREYOUSUREDELKEY' )
		.'</div>'
		.'<div class="boxinfo_container">'
		.'<span class="text_bold">'.$key.'</span> ['.def($key, $modulef, $platformf, $tranm).' ]'
		.'</div>', 'content');
	$GLOBALS['page']->add( 
...
function lang_inport() {
	...
	if( isset($_GET['confirm']) && ($_GET['confirm'] == '1')) {
		...
		$GLOBALS['page']->add( getTitleArea($lang->def('_LANGUAGE_MANAGMENT'), 'manlanguage', $lang->def('_LANGUAGE_MANAGMENT'))
			...
			.'<b>'.$lang->def( '_AREYOUSUREIMPORT' ).'</b>', 'content');
				...
				for( $iModule = 0; $iModule < $arrModules->length; $iModule++ ) {
					...
					for( $iKey = 0; $iKey < $arrKey->length; $iKey++ ) {
						...
						if( mysql_num_rows($rs) == 0) {
							...
							list($translation_text) = mysql_fetch_row($re);
...
function lang_exportXML() {
	...
	if( isset($_POST['lang_code']) && isset($_POST['platform']) ) {
		...
		foreach( $_POST['lang_code'] as $lang_code ) {
			...
			$elemText = $doc->createTextNode(urlencode($GLOBALS['globLangManager']->getLanguageDescription($lang_code)));
			$elem->appendChild($elemText);
			$lang->appendChild($elem);
			
			$elem = $doc->createElement("lang_charset");
			$elemText = $doc->createTextNode($GLOBALS['globLangManager']->getLanguageCharset($lang_code));
			$elem->appendChild($elemText);
			$lang->appendChild($elem);	
			
			$elem = $doc->createElement("lang_browsercode");
			$elemText = $doc->createTextNode($GLOBALS['globLangManager']->getLanguageBrowsercode($lang_code));
			$elem->appendChild($elemText);
			$lang->appendChild($elem);
			...
			foreach( $_POST['platform'] as $platform ) {
				...
				if( !isset($_POST['modules']) || $_POST['modules'][0] == '-' )
					$arrModules = $GLOBALS['globLangManager']->getAllModules($platform);
				else {
					$arrModules = array();
				...
				foreach( $arrModules as $module ) {
					...
					$arrTranslations = $GLOBALS['globLangManager']->getModuleLangTranslations($platform,$module,$lang_code, '', false, false, true);
					foreach( $arrTranslations as $tran ) {
						$elem = $doc->createElement("key");
						$elem->setAttribute('id',$GLOBALS['globLangManager']->composeKey( $tran[1], $module, $platform) );
		...
		foreach( $_POST['lang_code'] as $lang_code ) {
			...
			foreach( $_POST['platform'] as $platform ) {
				...
				foreach( $arrModules as $module ) {
					...
					$arrTranslationsEng = $GLOBALS['globLangManager']->getModuleLangTranslations($platform,$module,'english', '', false, false, true);
					$arrTranslationsOrg = $GLOBALS['globLangManager']->getModuleLangTranslations($platform,$module,$lang_code, '', false, false, true);
...
function lang_importexport() {
	...
	*/
	...
	$GLOBALS['page']->add( Form::getOpenFieldset( $lang->def( '_LANG_IMPORTXML' ) ) );
	...
	$GLOBALS['page']->add( $lang->def( '_LANG_FILEIMPORTXML_DESCR' ).'<br /><br />'
							.Form::getFilefield(	$lang->def( '_FILE' ), 
													'file', 
													'attach', 
													'',
													$lang->def( '_FILE' )) 
	...
	$GLOBALS['page']->add( Form::openButtonSpace() );
	$GLOBALS['page']->add( Form::getButton("importXML","importXML",$lang->def( '_LANG_IMPORTXML' )) );
	$GLOBALS['page']->add( Form::closeButtonSpace() );
	$GLOBALS['page']->add( Form::getCloseFieldset() );
	$GLOBALS['page']->add( Form::closeForm() );

	// export
	$GLOBALS['page']->add( Form::openForm( 	'exportXML', 
											'index.php?modname=lang&amp;op=exportXML') );
	$GLOBALS['page']->add( Form::getOpenFieldset( $lang->def( '_LANG_EXPORTXML' ) ));
	...
	$arr_modules = array();
	$arr_tmp = $GLOBALS['globLangManager']->getAllModules('framework');
	$arr_modules['-'] = '---all modules---';
	foreach( $arr_tmp as $module ) {
		$arr_modules['framework-'.$module] = 'framework-'.$module;
	}
	$arr_tmp = $GLOBALS['globLangManager']->getAllModules('lms');
	foreach( $arr_tmp as $module ) {
		$arr_modules['lms-'.$module] = 'lms-'.$module;
	}
	$arr_tmp = $GLOBALS['globLangManager']->getAllModules('cms');
	foreach( $arr_tmp as $module ) {
	...
	$GLOBALS['page']->add( Form::openButtonSpace() );
	$GLOBALS['page']->add( Form::getButton("exportXML","exportXML",$lang->def( '_LANG_EXPORTXML' )) );
	$GLOBALS['page']->add( Form::closeButtonSpace() );
	$GLOBALS['page']->add( Form::getCloseFieldset() );
	$GLOBALS['page']->add( Form::closeForm() );
	/*
...
function clean_lang() {
	...
	//$standard_def = $GLOBALS['globLangManager']->getModuleLangTranslations('framework', 'standard', 'italian');
	...
	foreach( $all_platform as $platform => $name ) {
		...
		$all_modules = $GLOBALS['globLangManager']->getAllModules($platform);
		
		foreach( $all_modules as $module ) {
			
			$all_translations = $GLOBALS['globLangManager']->getModuleLangTranslations($platform, $module, 'italian');
			foreach( $all_translations as $tran ) {
				...
				if(!isset($GLOBALS['parsed_define'][$tran[1]])) {
					...
					$GLOBALS['globLangManager']->deleteKey($tran[1], $module,$platform);
				} else {
					//echo '<tr><td>'.$i++.'</td><td>conserve'.$platform.'</td><td>'.$module.'</td><td>'.$tran[1].'</td></tr>';
				}
				//$GLOBALS['globLangManager']->composeKey( $tran[1], $module, $platform);
			}
		}
	}
