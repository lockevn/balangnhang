...
class FeedReader {
	...
	function readFeed($feed_id, $force_refresh=FALSE, $ignore_expired=FALSE) {
		...
		require_once($GLOBALS["where_framework"]."/addons/feeds/read/lastRSS.php");
		$rss=new lastRSS();
		$rss->cache_dir="";
...
class FeedReaderManager {
	 ...
	 * @return array
	 */
	function getFeedListArray($platform=FALSE) {
		$feed_list=array();

		$fields ="t1.feed_id, t1.title";
		$qtxt ="SELECT ".$fields." FROM ".$this->_getFeedsTable()." as t1 ";
		if (($platform !== FALSE) && (!empty($platform))) {
			$qtxt.="WHERE show_on_platform LIKE '%".$platform."%' AND zone = 'public'";
		}
		$qtxt.="ORDER BY t1.ord ";
		$q=$this->_executeQuery($qtxt);
	...
	function getFeedInfo($feed_id) {
		...
		if (isset($this->feed_info[$feed_id]))
			...
			$qtxt="SELECT * FROM ".$this->_getFeedsTable()." WHERE feed_id='".$feed_id."' ORDER BY ord";
			$q=$this->_executeQuery($qtxt);
	...
	function saveFeed($data) {
		...
		if ($id == 0) {
			...
			$ord=$this->getLastOrd($this->_getFeedsTable())+1;

			$field_list="title, url, active, refresh_time, show_on_platform, ord";
			$field_val="'".$title."', '".$url."', '1', '".$refresh_time."', '".$show_on_platform."', '".$ord."'";
			...
			$qtxt.="refresh_time='".$refresh_time."', show_on_platform='".$show_on_platform."' ";
...
class FeedGenerator {
	...
	function FeedGenerator($key1=FALSE, $key2=NULL, $platform=FALSE, $pfm_prefix=FALSE, $dbconn=NULL) {
		...
		$this->key1=$key1;
		$this->key2=$key2;
		$this->platform=($platform !== FALSE ? $platform : $GLOBALS["platform"]);
	...
	function _executeQuery( $query ) {
		if (($GLOBALS['do_debug'] == 'on') && ($this->getAllowDebug())) {
	...
	function createFeedCode($title, $data_arr, $url=FALSE) {
		require_once($GLOBALS["where_framework"]."/addons/feeds/write/rss_generator.inc.php");
