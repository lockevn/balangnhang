...
class NavBar {
	...
	function NavBar($var_name, $element_per_page, $element_total, $kind_of = false, $modname = false, $platform = false) {
		...
		$this->symbol = array(
			'start' => array( 
				'img' => '<img src="'.getPathImage('fw').'standard/start.gif" alt="'.def('_START').'" title="'.def('_START').'" />',
				'src' => getPathImage('fw').'standard/start.gif',
				'alt' => def('_START', $modname, $platform)
			),
			'prev' => array( 
				'img' => '<img src="'.getPathImage('fw').'standard/prev.gif" alt="'.def('_PREV').'" title="'.def('_PREV').'" />',
				'src' => getPathImage('fw').'standard/prev.gif',
				'alt' => def('_PREV', $modname, $platform)
			),
			'next' => array( 
				'img' => '<img src="'.getPathImage('fw').'standard/next.gif" alt="'.def('_NEXT').'" title="'.def('_NEXT').'" />',
				'src' => getPathImage('fw').'standard/next.gif',
				'alt' => def('_NEXT', $modname, $platform)
			),
			'end' => array( 
				'img' => '<img src="'.getPathImage('fw').'standard/end.gif" alt="'.def('_END').'" title="'.def('_END').'" />',
				'src' => getPathImage('fw').'standard/end.gif',
				'alt' => def('_END', $modname, $platform)
			)
		);
	}
	...
	function getInfo() {
		...
		return '<span class="number-of-result">'.def('_RE').' : </span>'.$this->element_total.'&nbsp;'
			.'<span class="total-page">'.def('_PG').' : </span>'.$pages;
	}
	...
	function getNavBarLink($element_selected) {
		...
		$html .= '</ul>';
		$html .= '<div class="no_float"></div>'.'</div>';
		return $html;
	}
