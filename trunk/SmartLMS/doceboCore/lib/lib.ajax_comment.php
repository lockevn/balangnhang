...
class AjaxComment {
	...
	function AjaxComment($resource_type, $resource_platform) {
		...
		else $this->reply_status = !$GLOBALS['current_user']->isAnonymous();
	}
	
	// some abstraction function
	function _query($query) { $re = mysql_query($query); doDebug($query); return $re; }
	function _fetch_row($resource) { return mysql_fetch_row($resource); }
...
class AjaxCommentRender {
	...
	function setCommentToDisplay($data) {
		...
		$this->_acl_man = $GLOBALS['current_user']->getAclManager();
		$this->_authors = $this->_acl_man->getUsers($users);
		
		
		$this->profile = new UserProfile( 0 );
		$this->profile->init('profile', 'lms', '', 'ap');
	}
	...
	function nextComment() {
		...
		if ($GLOBALS['current_user']->getUserLevelId()=="/framework/level/godadmin") {
			$html .= '<p><a href="javascript:;" onclick="delComment('.$comment_data[AJCOMM_ID].','.$comment_data[AJCOMM_EXTKEY].')">'
				.'<img src="'.getPathImage().'standard/rem.gif" alt="'.def('_DEL', 'standard', 'framework').'" />'
				.'</a></p>';
		}
		$html .= '</div>';
	...
	function getAddCommentMask($ext_key) {
		...
		$html = ''
			...
			.'<p>'
			.Form::getLabel('ajaxcomment_textof', def('_COMMENT_TEXT', $this->_module, $this->_platform), 'label_bold')
			...
			.Form::getButton(	'ajaxcomment_send',
								'ajaxcomment_send', 
								def('_COMMENT_SEND', $this->_module, $this->_platform), 
								'' )
			.'</p>'
	...
	function getAddCommentMask_2($ext_key) {
		...
		return ""
			...
			."<p>"
			."<label class=\"label_bold\" for=\"ajaxcomment_textof\">".def('_COMMENT_TEXT', $this->_module, $this->_platform)."</label>"
			."</p>"
			."<p>"
			."<textarea class=\"textarea\" id=\"ajaxcomment_textof\" name=\"ajaxcomment_textof\" rows=\"5\" cols=\"40\">"
			."</textarea></p><p><button onclick=\"addajaxcomment();\">".def('_COMMENT_SEND', $this->_module, $this->_platform)."</button>"
			."</p>"
			//."</div>"
