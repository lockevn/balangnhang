...
class SimpleSelector {
	...
	function SimpleSelector($use_multi_sel, & $lang) {
		...
		$this->mdir=new Module_Directory();
	...
	function getOp() {
		...
		if ((isset($_POST["okselector"])) && (!$manual))
			...
			jumpTo($this->getLink("back", "", true));
		else if ((isset($_POST["cancelselector"])) && ($manual))
			jumpTo($this->getLink("main", "", true));
	...
	function orgchartSelector()
	{
		...
		$tab = new typeOne(100, $lang->def("_ORGCHART_TITLE"), $lang->def("_ORGCHART_SUMMARY"));
		$form = new Form();
		...
		while (list($id_org, $level, $translation) = mysql_fetch_row($result))
		{
			$cont = array();
			
			list($idst) = mysql_fetch_row(mysql_query("SELECT idst FROM ".$GLOBALS['prefix_fw']."_group WHERE groupid = '".('/oc_'.$id_org)."'")); 
			
			$cont_temp = '';
			
			if (($level - 1))
				for ($i = 0; $i < ($level-1); $i++)
					$cont_temp .= '<img src="'.getPathImage('lms').'standard/blank.gif'.'" />';
			...
			foreach ($this->getPermList() as $key=>$val)
			{
				...
				$check_box =$form->getLabel( $key."_".$idst."_", $lang->def("_ALT_VIEW")." ".$translation, "access-only");
	...
	function loadSimpleSelector($anonymous = true, $orgchart_button = false) {
		...
		$tab=new typeOne(2, $lang->def("_SIMPLESEL_TITLE"), $lang->def("_SIMPLESEL_SUMMARY"));
		...
		foreach ($users_list as $idst=>$label) {
			...
			foreach ($this->getPermList() as $key=>$val) {
				...
				$check_box =$form->getLabel( $key."_".$idst."_", $lang->def("_ALT_VIEW")." ".$label, "access-only");
				$check_box.=$form->getInputCheckbox($key."_".$idst."_", $key."[".$idst."]", 1, $chk, NULL);
				$rowcnt[]=$check_box;
			}
	...
	function getSimpleUserList() {
		...
		$users_list=array();
		$users_list[$anonymous_idst]=def("_ANONYMOUS_USERS", "standard", "cms");
		$users_list[$regusers_idst]=def("_REGISTERED_USERS", "standard", "cms");
	...
	function getSaveInfoOrg() {
		...
		while (list($id_org, $translation) = mysql_fetch_row($result))
		{
			list($idst) = mysql_fetch_row(mysql_query("SELECT idst FROM ".$GLOBALS['prefix_fw']."_group WHERE groupid = '".('/oc_'.$id_org)."'"));
