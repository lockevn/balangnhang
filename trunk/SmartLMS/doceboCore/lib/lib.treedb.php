...
class Folder {
	...
	function Folder( &$tdb, $arrayValues, $childInfo = FALSE ) {
		$this->tdb = $tdb;
	...
	function getChildrens() {
		...
		$coll = new FoldersCollection( $this->tbd, $rs );
		return $coll;
	}
...
class TreeDb {
	...
	function &getFolderById( $id ) {
		...
		if( $id == 0 ) {
			...
			$folder = new Folder( $this, mysql_fetch_row($rs) );
		}
		return $folder;
	}
	...
	**/
	function getPathFromFolderId( $arr_id ) {
		...
		$arr_result = array();
		while( list( $idFolder, $path ) = mysql_fetch_row($rs) ) {
	...
	function addFolderByPath( $fullPath, $createAll ) {
		...
		$parentFolder = new Folder( $this, mysql_fetch_row($rs) );
	...
	function renameFolder( &$folder, $newName ) {
		...
		$query = "UPDATE ". $this->table
				...
				.$this->_getFilter() .")";
		$this->_executeQuery( $query )
			or $this->_printSQLError( 'renameFolder ['.$query.']' );
		$this->_propagateChange( $oldFolder, $folder);
	}
...
class FoldersCollection {
	...
	function FoldersCollection( &$tdb, $rs, $childInfo = FALSE ) {
		$this->tdb = $tdb;
		$this->rs = $rs;
	...
	function getNext() {
		$array = mysql_fetch_row( $this->rs );
		if( $array === FALSE )
			return FALSE;
		$folder = new Folder( $this->tdb, $array, $this->childInfo );
		return $folder;
	}
}
