...
Class CoreWikiAdmin {
	...
	function CoreWikiAdmin($source_platform) {
		$this->lang =& DoceboLanguage::createInstance('admin_wiki', "framework");
	...
	function getWikiListTable($vis_item, $view_link=FALSE) {
		...
		$table_caption=$this->lang->def("_TABLE_WIKI_CAP");
...
/*			$img ="<img src=\"".getPathImage('fw')."standard/export.gif\" alt=\"".$this->lang->def("_EXPORT")."\" ";
	...
	function addeditWiki($id=0) {
		...
		if($GLOBALS['platform'] !== 'framework')
			$res.=$form->getCheckbox($this->lang->def("_PUBLIC_WIKI"), "public", "public", 1, $public);
	...
	function showWikiPerm($wiki_id) {
		...
		switch($op) {
			...
			case "manual_init":{
				...
				$ssel->setSavedData($saved_data);
				$ssel->loadManualSelector($this->lang->def("_WIKI_PERMISSIONS"));
			} break;
			case "manual": {
				$ssel->loadManualSelector($this->lang->def("_WIKI_PERMISSIONS"));
			} break;
...
Class CoreWikiPublic {
	...
	function wikiPageHistory($vis_item=FALSE) {
		...
		$table_caption=$this->lang->def("_TABLE_PAGE_HISTORY_CAP");
		$table_summary=$this->lang->def("_TABLE_PAGE_HISTORY_SUM");
		...
		for($i=0; $i<$tot; $i++ ) {
			...
			$rowcnt[]=$GLOBALS["regset"]->databaseToRegional($data_arr[$i]["rev_date"]);
...
Class CoreWikiManager {
	...
	function correctSonsPath($wiki_id, $page_id, $first = FALSE)
	{
		...
		list($parent_id, $parent_level, $first_page_path) = mysql_fetch_row(mysql_query($query));
		
		$query =	"SELECT page_path"
					." FROM ".$this->getWikiPageTable()
					." WHERE page_id = '".$parent_id."'";
		
		list($parent_path) = mysql_fetch_row(mysql_query($query));
		...
		if(mysql_num_rows($result))
			while(list($sons_id) = mysql_fetch_row($result))
			{
...
/*		$qtxt ="DELETE FROM ".$this->_getFaqTable()." ";
	...
	function getLanguageArr($include_other=FALSE) {
		...
		if ($include_other)
			$res["other"]=def("_OTHER_LANGUAGE", "admin_wiki", "framework");
		...
		if( $folder->id == 0 )
			return "wiki";//$this->rootname;
				...
				if ($title !== NULL) {
					...
					$res=$page_code." (".def("_NOT_TRANSLATED", "wiki", "framework").")";
				}
