...
class DoceboUserNotifier extends DoceboEventConsumer {
	...
	function actionEvent( &$event ) {
		...
		$send_to_field = $GLOBALS['framework']['sms_cell_num_field'];
...
/*
		...
		$from = "From: ".$GLOBALS['framework']['sender_event'].$GLOBALS['mail_br'];
    ...
    $fromemail = $GLOBALS['framework']['sender_event'];
    while(list(, $recipient) = each($mail_recipients)) {
    	
    	if(!sendMail($recipient, $subject, $body, $fromemail, $fromemail)) {
				//...
			} else {
				//...
			}
		...
		$mailer->SendMail($GLOBALS['framework']['sender_event'], $mail_recipients, 
				$subject, $body, false, 
				array(MAIL_REPLYTO => $GLOBALS['framework']['sender_event'], MAIL_SENDER_ACLNAME => false));
	}
	...
	function _sendSms($body, &$sms_recipients) {
		...
		/* switch($GLOBALS['framework']['sms_gateway']) {
			case "smsmarket": {
...
function usernotifier_getUserEventChannel( $idst, $event_class ) {
	...
	$rs_manager = mysql_query( $query );
	if( $rs_manager === FALSE )
		return array();
	if( mysql_num_rows($rs_manager) == 0 )
		return array();

	list($channel, $permission) = mysql_fetch_row( $rs_manager );
	...
	if( $permission == 'not_used' ) {
		...
		if( mysql_num_rows($rs_user) == 1 ) {
			list($channel) = mysql_fetch_row( $rs_user );

			if( strlen($channel) > 0 ) {
				$media = explode( ',', $channel );
			}
		} else {
			list($channel) = mysql_fetch_row( $rs_manager );
			if( strlen($channel) > 0 ) {
				$media = explode( ',', $channel );
			}
		}
