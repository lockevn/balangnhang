...
require_once(dirname(__FILE__).'/../lib/lib.aclmanager.php');
require_once( dirname(__FILE__).'/../lib/lib.listview.php' );
...
class PeopleListView extends ListView {
	...
	function _getEditLabel() { return $this->lang->def('_DIRECTORY_EDITPERSON'); }
	function _getEditAlt() { return $this->lang->def('_DIRECTORY_EDITPERSON'); }
	function _getEditImage() { return getPathImage().'standard/mod.gif'; }

	function _getOpDeleteItemId() 	{ return '_listview_opdeleteitem_';	}
	function _getDeleteLabel() { return $this->lang->def('_DIRECTORY_DELPERSON'); }
	function _getDeleteAlt() { return $this->lang->def('_DIRECTORY_DELPERSON'); }
	function _getDeleteImage() { return getPathImage().'standard/rem.gif'; }

	function _getOpSuspendItemId() 	{ return '_listview_opsuspenditem_';	}
	function _getSuspendLabel() { return $this->lang->def('_DIRECTORY_SUSPENDPERSON'); }
	function _getSuspendAlt() { return $this->lang->def('_DIRECTORY_SUSPENDPERSON'); }
	function _getSuspendImage() { return getPathImage().'standard/suspend.gif'; }

	function _getOpSort() { return '_listview_opsort_'; }
	function _getSortLabel() { return $this->lang->def('_DIRECTORY_SORTCOLUMN'); }
	function _getSortAlt() { return $this->lang->def('_DIRECTORY_SORTCOLUMN'); }
	...
	function _getRecoverLabel() { return $this->lang->def('_DIRECTORY_RECOVERPERSON'); }
	function _getRecoverAlt() { return $this->lang->def('_DIRECTORY_RECOVERPERSON'); }
	...
	function _getRowsPage() { return $GLOBALS['framework']['visuUser']; }
		...
		if( !$this->selector_mode ) {
			if( !$this->tree_mode ) {
				if($this->mod_perm) {
					$colInfos[] = array( 	'hLabel' => '<img src="'.$this->_getEditImage().'" '
												.'alt="'.$this->lang->def( '_DIRECTORY_EDITPERSON' ).'" '
												.'title="'.$this->lang->def( '_DIRECTORY_EDITPERSON' ).'" />',
											'hClass' => "image",
											'fieldClass' => "image",
											'data' => 'edit',
											'toDisplay' => true,
					...
					$colInfos[] = array( 	'hLabel' => '<img src="'.$this->_getSuspendImage().'" '
											.'alt="'.$this->lang->def( '_DIRECTORY_SUSPENDPERSON' ).'" '
											.'title="'.$this->lang->def( '_DIRECTORY_SUSPENDPERSON' ).'" />',
										'hClass' => "image",
										'fieldClass' => "image",
										'data' => 'valid',
										'toDisplay' => true,
			...
			if($this->del_perm) {
				$colInfos[] = array( 	'hLabel' => '<img src="'.$this->_getDeleteImage().'" '
										.'alt="'.$this->lang->def( '_DIRECTORY_DELPERSON' ).'" '
	...
	function extendedParsing( $arrayState ) {
		...
		if( isset( $arrayState[$this->id] ) ) {
			...
			$this->cFields = unserialize(urldecode($GLOBALS['current_user']->preference->getPreference( 'ui.directory.custom_columns' )));
			$this->arr_fields_order = unserialize(urldecode($GLOBALS['current_user']->preference->getPreference( 'ui.directory.order_columns' )));
			$this->arr_fields_filter = unserialize(urldecode($GLOBALS['current_user']->preference->getPreference( 'ui.directory.filters.current' )));
		}
					...
					if( isset( $fvalue['fieldname'] ) ) {
						...
						if( $fvalue[FIELD_INFO_TYPE] == 'upload' ) {
							...
							elseif($fvalue[FIELD_INFO_TYPE] == 'date')
								$where = " ($fname.user_entry LIKE '%".$GLOBALS['regset']->regionalToDatabase($fvalue['value'], 'date')."%' ) ";
							else
	...
	function fetchRecord() {
		...
		if(!$this->selector_mode && $userid != 'Anonymous') {
			...
			$arrResult['userid'] =
				...
				'<input type="image" class="button_image'.( $arrResult['idst'] == $this->_expand_user ? '' : ' block_hide' ).'" '
					.'src="'.getPathImage('fw').'standard/less.gif" '
					.'alt="'.$this->lang->def('_LESS_INFO').'" '
		...
		$arrResult['fullname'] = ( $arrResult['valid']!='1' && $arrResult['idst'] != $this->anonymous_idst
					? '<em class="user_suspended">'.$this->lang->def('_SUSPENDED_USER').'</em> '
		...
		if( !$this->selector_mode ) { // Normal mode
			...
			$lev_current_user = $GLOBALS['current_user']->getUserLevelId();
	...
	function userExtraData($id_user) {
		...
		require_once(dirname(__FILE__).'/lib.user_profile.php');
	...
	function printOut() {
		...
		$profile->init('profile', 'lms', 'modname=directory&op=org_chart', 'ap');
		...
		if( $this->select_all ) {
			...
			if( $rs_all !== FALSE ) {
				...
				while( list($all_idst) = mysql_fetch_row($rs_all) )
		...
		if($this->show_simple_filter === TRUE) {
			...
			if(is_array($filter_to_show) && !empty($filter_to_show)) {
				...
				$out .= $filter->getButton( $this->id.'_add_filter',
											$this->id.'[add_filter]',
											$this->lang->def('_DIRECTORY_FILTER_ADD'),
											'button_nowh');
			}
			...
			foreach( $this->arr_fields_filter as $field_id => $field_prop ) {
				...
				if( !isset( $field_prop['fieldname'] ) ) {
					...
					$del_spot = '<input type="image" class="cancel_filter" '
								...
								.' title="'.$this->lang->def('_DIRECTORY_FILTER_DEL').'"'
								.' alt="'.$this->lang->def('_DIRECTORY_FILTER_DEL').'" />';
...
class GroupListView extends ListView {
	...
	function _getOpEditItemId() 	{ return '_listview_opedititem_';	}
	function _getEditLabel() { return $this->lang->def('_DIRECTORY_EDITGROUP'); }
	function _getEditAlt() { return $this->lang->def('_DIRECTORY_EDITGROUP'); }
	function _getEditImage() { return getPathImage().'standard/mod.gif'; }

	function _getOpDeleteItemId() 	{ return '_listview_opdeleteitem_';	}
	function _getDeleteLabel() 	{ return $this->lang->def('_DIRECTORY_DELGROUP'); }
	function _getDeleteAlt() 		{ return $this->lang->def('_DIRECTORY_DELGROUP'); }
	function _getDeleteImage() 	{ return getPathImage().'standard/rem.gif'; }
	...
	function _getWaitingLabel() 		{ return $this->lang->def('_DIRECTORY_GROUPWAITING'); }

	function _getRowsPage() { return $GLOBALS['framework']['visuItem']; }
		...
		if( !$this->selector_mode ) {
			...
			if($this->mod_perm) {
				$colInfos[] = array( 	'hLabel' => '<img src="'.getPathImage('fw').'/directory/group_waiting.gif" '
											.'alt="'.$this->lang->def( '_DIRECTORY_GROUPWAITING' ).'" '
											.'title="'.$this->lang->def( '_DIRECTORY_GROUPWAITING' ).'" />',
									'hClass' => "image",
									'fieldClass' => "image",
									'data' => 'waiting_user',
									'toDisplay' => true,
			...
			if($this->mod_perm) {
				$colInfos[] = array( 	'hLabel' => '<img src="'.$this->_getEditImage().'" '
											.'alt="'.$this->lang->def( '_DIRECTORY_EDITGROUP' ).'" '
											.'title="'.$this->lang->def( '_DIRECTORY_EDITGROUP' ).'" />',
										'hClass' => "image",
										'fieldClass' => "image",
										'data' => 'edit',
										'toDisplay' => true,
			...
			if($this->del_perm) {
				$colInfos[] = array( 	'hLabel' => '<img src="'.$this->_getDeleteImage().'" '
											.'alt="'.$this->lang->def( '_DIRECTORY_DELGROUP' ).'" '
											.'title="'.$this->lang->def( '_DIRECTORY_DELGROUP' ).'" />',
										'hClass' => "image",
										'fieldClass' => "image",
										'data' => 'delete',
										'toDisplay' => true,
...
class GroupMembersListView extends ListView {
	...
	function _getMemberTypeUser() { return getPathImage().'directory/user.gif'; }
