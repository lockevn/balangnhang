...
/// Determine if feed has changed since the if-modified-since HTTP header and exit if it hasn't
    if ($CFG->debug != DEBUG_DEVELOPER) {
        ...
        if (isset($_SERVER['HTTP_IF_MODIFIED_SINCE'])) {
            if ($mtimegm == preg_replace('/;.*$/', '', $_SERVER['HTTP_IF_MODIFIED_SINCE'])) {
    ...
    if ($full) {
        if ($full == md5($accesstoken.$viewer.OUBLOG_VISIBILITY_COURSEUSER) && $user = get_record('user', 'id', $viewer)) {
...
/// Generate feed in RSS or ATOM format
    if ($format == 'rss') {
        ...
        echo rss_standard_header($feedname, '', $feedsummary);
