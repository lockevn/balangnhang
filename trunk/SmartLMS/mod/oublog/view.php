...
/// Print the header
    ...
    if ($oublog->global) {
        $blogtype = 'personal';
        $returnurl = 'view.php?user='.$user;

        $name = $oubloginstance->name;

        $navlinks = array();
        $navlinks[] = array('name' => fullname($oubloguser), 'link' => "../../user/view.php?id=$oubloguser->id", 'type' => 'misc');
        $navlinks[] = array('name' => format_string($oubloginstance->name), 'link' => "view.php?user=$oubloguser->id", 'type' => 'activityinstance');
        if ($extranav) {
            $navlinks[] = $extranav;
        }
...
EOF;
        ...
        if($hideunusedblog) {
            ...
            print_header_simple(format_string($oublog->name), "", $navigation, "", get_oublog_meta_tags($oublog, $oubloginstance, $currentgroup, $context), true,
...
/// Links
    ...
    // Feeds
    if ($feeds = oublog_get_feedblock($oublog, $oubloginstance, $currentgroup, false, $context)) {
