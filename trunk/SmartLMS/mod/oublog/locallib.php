...
function oublog_has_userblog_permission($capability,$oublog,$oubloginstance,$context) {    
    ...
    if($oublog->global && $oubloginstance && $USER->id == $oubloginstance->userid && 
        has_capability('mod/oublog:post', $context)) {
        return true;
    }
 ...
 * @uses $USER
 */
function oublog_get_feedurl($format, $oublog, $bloginstance, $groupid, $comments, $postid, $context) {
    global $CFG, $USER;
    ...
    if (!isloggedin() || isguestuser()) {
        ...
        if (has_capability('mod/oublog:post', $context)) {
            $url .= '&amp;full='.md5($accesstoken.$USER->id.OUBLOG_VISIBILITY_COURSEUSER);
        } else {
 ...
 * @uses $CFG
 */
function oublog_get_feedblock($oublog, $bloginstance, $groupid, $postid, $context) {
    global $CFG;
    ...
    $blogurlatom = oublog_get_feedurl('atom',  $oublog, $bloginstance, $groupid, false, false, $context);
    $blogurlrss = oublog_get_feedurl('rss',  $oublog, $bloginstance, $groupid, false, false, $context);

    if (!is_string($bloginstance)) {
    	$commentsurlatom = oublog_get_feedurl('atom',  $oublog, $bloginstance, $groupid, true, $postid, $context);
        $commentsurlrss = oublog_get_feedurl('rss',  $oublog, $bloginstance, $groupid, true, $postid, $context);
    }
 ...
 * @return string
 */
function get_oublog_meta_tags($oublog, $bloginstance, $groupid, $context) {
    global $CFG;
