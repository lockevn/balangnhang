...
define('PAGE_QUIZ_REVIEW',   'mod-quiz-review');


page_map_class(PAGE_QUIZ_REVIEW, 'page_quiz_review');

$DEFINEDPAGES = array(PAGE_QUIZ_REVIEW);
...
class page_quiz_review extends page_generic_activity {

    function init_quick($data) {
        if(empty($data->pageid)) {
            error('Cannot quickly initialize page: empty course id');
        }
        $this->activityname = 'quiz_review';
        parent::init_quick($data);
    }
  
    function get_type() {
        return PAGE_QUIZ_REVIEW;
    }
    ...
    /*danhut added to add mod-quiz-review option in admin/stickyblocks.php*/
	...
	function url_get_path() {
        ...
        if ($id == PAGE_QUIZ_REVIEW) {
            return $CFG->wwwroot.'/mod/quiz/review.php';
	...
	function print_header($title) {
        ...
        $replacements = array(
                              '%fullname%' => get_string('stickyblocksquizreview','admin')
        );
        ...
        $nav = get_string('stickyblocksquizreview','admin');
