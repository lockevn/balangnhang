...
function listroom() {
	...
	$out->add(getTitleArea($lang->def('_ROOM_MAN'), 'admin_conf')
			.'<div class="std_block">');
	$tb_room = new TypeOne(0, $lang->def('_ALL_ROOMS'), $lang->def('_ALL_ROOMS_SUMMARY'));
	...
	$re_rooms = mysql_query($query_rooms);
	while(list($id_room, $room_name, $room_type) = mysql_fetch_row($re_rooms)) {
		...
		if($mod_perm) {
			...
			$cont[] = '<a href="index.php?modname=room&amp;op=modroom&amp;id_room='.$id_room.'"'
				...
				.'<img src="'.getPathImage('fw').'standard/mod.gif" alt="'.$lang->def('_MOD').' : '.$room_name.'" /></a>';
				
			$cont[] = '<a href="index.php?modname=room&amp;op=delroom&amp;id_room='.$id_room.'"'
					.' title="'.$lang->def('_DEL').' : '.$room_name.'">'
				.'<img src="'.getPathImage('fw').'standard/rem.gif" alt="'.$lang->def('_DEL').' : '.$room_name.'" /></a>';
		}
	...
	if($mod_perm) {
		...
		$tb_room->addActionAdd(
			...
			.'<img src="'.getPathImage('fw').'standard/add.gif" alt="'.$lang->def('_NEW_ROOM').'" />'
			.'&nbsp;'.$lang->def('_NEW_ROOM')
			.'</a>');
	}
...
function insroom() {
	...
	jumpTo('index.php?modname=room&amp;op=room&amp;result='.( $re ? 'ok' : 'err' ));
}
