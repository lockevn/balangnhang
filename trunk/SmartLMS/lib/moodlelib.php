...
function required_param($parname, $type=PARAM_CLEAN) {
	...
	if (isset($_POST[$parname])) {       // POST has precedence
		...
		// HACK: lockevn of gurucore
		// hack to display friendly and fast erro for API call
		if(defined('AJAX_CALL'))
		{
			errorForAjaxCall('A required parameter ('.$parname.') was missing');
		}
		else
		{
			error('A required parameter ('.$parname.') was missing');
		}
		// HACK: end, lockevn of gurucore
	}
