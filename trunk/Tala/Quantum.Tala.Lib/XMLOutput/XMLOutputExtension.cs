...
namespace Quantum.Tala.Lib.XMLOutput
{
    ...
    /// Lớp Util tĩnh
    /// </summary>
    public static class XMLOutputExtension
    {
        /// <summary>
        ...
        /// </summary>
        /// <returns></returns>
        public static XElement ToXElement(this IAPIData thisAPIData)
        {
            /// XML Element, đại diện cho chính DataEntry đang được xử lý này
            string sThisElementName = string.Empty;
            object[] classNameAttribs = thisAPIData.GetType().GetCustomAttributes(typeof(ElementXMLExportAttribute), false);

            sThisElementName = thisAPIData.GetType().Name.ToStringSafetyNormalize();
            ...
            PropertyInfo[] arrPro = thisAPIData.GetType().GetProperties(BindingFlags.Public | BindingFlags.Instance);
            ...
            foreach (PropertyInfo pro in arrPro)
            {
                ...
                object oProValue = pro.GetValue(thisAPIData, null);
