...
require_once($CFG->dirroot.'/smartcom/lomanagement/contextmoveq_form.php');

$ids = required_param('ids',PARAM_SEQUENCE); // lo ids
...
if (!$los = get_records_sql("
	SELECT l.*, t.*, c.contextid 
	FROM {$CFG->prefix}lo l, {$CFG->prefix}$tblName t, {$CFG->prefix}question_categories c  
	WHERE l.instance IN ($ids) 
		AND c.id = l.category 
		AND l.instance = t.id
		AND l.lotype = '$lotype'")) {
    print_error('lodoesnotexist', 'smartcom', $returnurl);
}
...
$fromcoursefilesid = 0;
foreach (array_keys($los) as $id){
    lo_require_capability_on($los[$id], 'move');
    //get_question_options($los[$id]);
    $los[$id]->context = get_context_instance_by_id($los[$id]->contextid);
    $thisfilesid =  get_filesdir_from_context($los[$id]->context);
    if ($fromcoursefilesid && $thisfilesid != $fromcoursefilesid){
       error('You can\'t use this script to move LOs that have files associated with them from different areas.');
    } else {
       $fromcoursefilesid =  $thisfilesid;
    }
    if ($tocoursefilesid != $fromcoursefilesid){
        $urls = array_merge_recursive($urls, find_file_links($los[$id], $fromcoursefilesid));
    }
}
    ...
    if (isset($moveformdata->urls) && is_array($moveformdata->urls)){
        ...
        foreach ($moveformdata->urls as $key => $urlaction){
            ...
            switch ($urlaction){
                ...
                case QUESTION_FILEMOVELINKSONLY :
                    $url = $flipurls[$key];
                    $loswithlinks = array_unique($urls[$url]);
