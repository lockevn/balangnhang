...
require_once("$CFG->dirroot/smartcom/lomanagement/category_form.php");
...
class question_category_list_item extends list_item {
    ...
    function set_icon_html($first, $last, &$lastitem){
        ...
        $this->icons['edit']= $this->image_icon(get_string('editthiscategory'),
                "{$CFG->wwwroot}/smartcom/lomanagement/category.php?".$this->parentlist->pageurl->get_query_string(array('edit'=>$category->id)), 'edit');
    ...
    function item_html($extraargs = array()){
        ...
        $questionbankurl = "{$CFG->wwwroot}/smartcom/lomanagement/edit.php?".
...
//------------------------------------------------------
     ...
     */
    function lo_category_object($page, $pageurl, $contexts, $currentcat, $defaultcategory, $todelete, $addcontexts) {
        global $CFG, $COURSE;
    ...
    function update_category($updateid, $newparent, $newname, $newinfo) {
        ...
        $lastcategoryinthiscontext = lo_is_only_toplevel_category_in_context($updateid);
