 ...
 *
 * $Header: /cvs/bbclone/show_detailed.php,v 1.95 2006/12/28 13:12:33 christoph Exp $
 *
...
elseif (is_readable(_BBCLONE_DIR."cms_constants.php")) require_once(_BBCLONE_DIR."cms_constants.php");
else return;
...
function bbc_show_connect_field($connect, $field, $lng, $titles = false) {
  ...
  $id = empty($connect['id']) ? 0 : $connect['id'];
	$url="index.php?modname=stats&amp;op=showviews&amp;id=".$id."&amp;lng=".$lng;
  ...
  switch ($field) {
    ...
    case "visits":
      return "<div align=\"right\"><a href=\"".$url."\" target=\"_blank\">".$connect[$field]
            ."</a>&nbsp;</div>\n";
...
function bbc_rows_gen(& $bbc_html) {
  global $_, $BBC_DETAILED_STAT_FIELDS, $BBC_MAXVISIBLE, $last;
...
bbEchoInDocebo( $bbc_html->html_begin()
    .$bbc_html->topbar()
