...
function chat_openconfBlockEdit(& $out, & $lang, & $form, $block_id, $sub_id) {
...
function chat_openconfBlockSave($block_id, $sub_id) {
...
function chat_openconfBlockAdd($block_id, $sub_id) {

	saveParam($block_id, "css", 1);

}

function chat_openconfBlockOption(& $out, & $lang, & $form, $block_id, $sub_id, $blk_op) {
...
function selModPerm(& $out, & $lang, & $form, $block_id, $sub_id) {
	...
	$role_id="/cms/chat/openconf/block/".$block_id."/moderate";
	...
	$title=$lang->def("_SEL_OPENCONFERENCE_CHAT_MODERATORS");
	...
	switch($op) {
		...
		case "save_manual": {
			...
			$save_info=$ssel->getSaveInfo();
			saveOpenconfChatModPerm($st_id, $save_info["selected"], $save_info["database"]);
...
function saveOpenconfChatModPerm($st_id, $selected, $database) {
