var serverUrl="ajax.server.php?plf=cms&mn=calendar&";
var calClass="lms";
...
function showEvent(year,month,day,id) {
	...
	var data="calEventClass="+calEventClass+"&op=getForm"+"&cal_id="+cal_id;
	/*
...
/*	var evFormText = ObjReq.responseText;
	...
	buttns += '<input type="submit" value="'+_TT._SAVE+'" onclick="sendEvent();return false;" />';

	if (id) 
	buttns += '&nbsp;&nbsp;&nbsp;<input type="submit" value="'+_TT._DEL_EVENT+'" onclick="delEvent(\''+id+'\')";return false;" />';

	buttns += '&nbsp;&nbsp;&nbsp;<input type="submit" value="'+_TT._CLOSE+'" onclick="destroyWindow(\'wEvent\')";return false;" /><input type="hidden" name="idev" id="idev" value="'+id+'" />';
...
function viewEvent (ObjReq) {
	...
	var buttns='';
	if (mode!="view") {
		buttns += '<input type="submit" value="'+_TT._EDT_EVENT+'" onclick="openEvent('+eYear+','+eMonth+','+eDay+','+id+')" />';
	}
		...
		switch (evForm.form[i].type) {
			...
			case "hidden":
				...
				if (evForm.form[i].permissions && evForm.form[i].permissions!=permissions) {
					...
					} else if(evForm.form[i].type=="textarea") {
						if(YAHOO.util.Dom.get(evForm.form[i].id).value != undefined)
							cValue=YAHOO.util.Dom.get(evForm.form[i].id).value;
						else 
							cValue=YAHOO.util.Dom.get(evForm.form[i].id).innerHTML;
                    }else {
...
//	if (YAHOO.util.Dom.get('wDelEvent')) 
	...
	buttns = '<input type="submit" value="'+_TT._YES+'" onclick="sendDelEvent(\''+id+'\');return false;" />'
		+'&nbsp;&nbsp;&nbsp;<input type="submit" value="'+_TT._NO+'" onclick="enableCal();destroyWindow(\'wDelEvent\');return false;" />';
...
function loadCalendar() {
	...
	var data="calClass="+calClass+"&op=get&month="+month+"&year="+year+"&cal_id="+cal_id;
...
function setup_cal(passed_url,passed_class,passed_eventclass,passed_mode,passed_perm,passed_idst, passed_cal_id) {
