...
/* Copyright (c) 2004 by Fabio Pirovano <gishell[AT]tiscali-it>          */
/* http://www.docebo.com                                                 */
/*                                                                       */
...
class sys_forum {
	...
	// function get_page_url($idArea=0, $pb=0) { // Used to get the right url of the page wherever you are
	                                          // (admin, index (block) or index (mod.))
		// Usage example:
		/*
				// From index as a block:
				$sf=new sys_forum("my_thread_id");
				$idArea=get_block_idArea($idBlock); // you need to know the id of the current block
				$sf->url=$sf->get_page_url($idArea, $idBlock);
		...
		switch (strtolower($fn)) {
			...
			case "index.php": {
				...
				if ($_GET["mn"] != "") {
					...
					if ($idArea == 0) $idArea=getIdArea();
					if ($pb != 0) $this->pb=$pb;
					return "index.php?special=changearea&newArea=".$idArea;
	...
	function message($tk, $perm, $url, $pb) {
		...
		ORDER BY posted";
		$reMessage = mysql_query($qtxt);

		// find info about authors
		$reNumPost = mysql_query("
		SELECT author, COUNT(*)
		...
		$authors = array();
		while( list($id_a, $num_post_a) = mysql_fetch_row($reNumPost) ) {
		...
		while( list($idM,
					$author,
					$posted,
					$title, $textof, $attach, $locked_m) = mysql_fetch_row($reMessage)) {
			...
			$posted_datetime = $GLOBALS["regset"]->databaseToRegional($posted);
			$posted_time = $GLOBALS["regset"]->ddate->getTimeStamp();

			$mess = '';
			$mess .= '<div class="forum_post_posted">'
					.$this->lang->def("_POSTED_HOURTIME").' : '.$posted_datetime.' '.$this->loadDistance($posted_time)
					.'</div>';
			if($attach) {
			...
			if($author == $GLOBALS["current_user"]->getIdSt() && ($write_perm || $mod_perm) && !$locked_m && $author != "1") {
				$action .= ' <a href="'.$url.'&amp;sf_op=modmessage'.$pbtxt.'&amp;idMessage='.$idM.$this->getUrlExtra().'">'
						.'<img src="'.getPathImage().'standard/mod.gif" alt="'.$this->lang->def("_MOD").'" title="'.$this->lang->def("_MOD").'" /></a>';
			}
			$action .= '</div>';
		...
		if( (!$locked_m && $write_perm) || $mod_perm ) {
			...
			$this->out->add($tab->addActionAdd('<div class="forumAdd"><a href="'.$url_add.'">
				<img src="'.getPathImage().'standard/add.gif" title="'.$this->lang->def("_ADDMESSAGE").'" alt="'.$this->lang->def("_ADDMESSAGE").'" /> '
	...
	function addmessage($tk, $perm, $url, $pb) {
		...
		if(isset($_GET['idMessage'])) {
			list($title_mess, $textof) = mysql_fetch_row(mysql_query("
			SELECT title, textof
			FROM ".$this->prefix."_sysforum
		...
		$res.=$form->getButton('save', 'save', $this->lang->def('_SENDMESSAGE'));
	...
	function insmessage($tk, $perm, $url) {
		...
		/*if(!$mod_perm) {
			...
			list( $idF, $title, $locked_f ) = mysql_fetch_row($re_forum);
		}
		else {
			list( $idF, $title, $locked_f ) = mysql_fetch_row(mysql_query("
	...
	function modmessage($tk, $perm, $url, $pb) {
		...
		$title_mess = $textof = '';
		if(isset($_GET['idMessage'])) {
			list($title_mess, $textof, $author) = mysql_fetch_row(mysql_query("
		...
		if( $author != $GLOBALS["current_user"]->getIdSt() ) return;
	...
	function upmessage($tk, $perm, $url, $pb) {
		...
		list($author, $old_file) = mysql_fetch_row(mysql_query("
		...
		WHERE idMessage = '".(int)$_POST['idMessage']."' AND
			author = '".(int)$GLOBALS["current_user"]->getIdSt()."'";
		...
		if($file['name'] != '') {
			...
			if(!file_exists ($path.$savefile)) {
				...
				if(!sl_upload($file['tmp_name'], $path.$savefile)){
					...
					// errorCommunication(_ERRORUPLOAD);
					return '';
				}
				sl_close_fileoperations();
				return $savefile;
