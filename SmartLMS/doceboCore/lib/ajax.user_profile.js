...
function ask_a_friend(id_user) {
	...
	var butt = "";
	butt += '<input type="submit" value="' + user_profile_lang._SEND_MESSAGE + '" onclick="destroyWindow(\'' + w.id + '\');" /> ';
...
function send_a_message(id_user) {
	...
	var content = '<div class="form_a_message">'
		+ '<label for="message_subject">' + user_profile_lang._MESSAGE_SUBJECT + '</label>'
	...
	w.title 	= user_profile_lang._MESSAGE_SUBJECT;
	w.content 	= content;
	w.form 		= forms;
	w.buttons 	= butt;
	w.show();	
...
function send_message_callback(objReq) {
	...
	NoticeMsg.display( ( result.re ? user_profile_lang._MESSAGE_SEND : user_profile_lang._MESSAGE_FAIL ), null, ( result.re ? '' : 'error' ), 30000 );
	NoticeMsg.endLoading();
}
