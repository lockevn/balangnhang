...
define("UP_FRIEND_LIMIT", 6);

define("PFL_POLICY_FREE", 0);
define("PFL_POLICY_TEACHER", 1);
define("PFL_POLICY_FRIENDS", 2);
define("PFL_POLICY_NOONE", 3);
define("PFL_POLICY_TEACHER_AND_FRIENDS", 4);
...
class UserProfile {
	...
	function init($module_name, $platform, $std_query, $varname_action = 'ap', $db_conn = NULL) {
		...
		$options = array('user_use_photo' => true, 'user_use_avatar' => true);
		...
		WHERE pack = 'log_option' 
			AND ( param_name = 'user_use_photo' OR param_name = 'user_use_avatar' ) ");
		
		while(list($var_name, $var_value, $value_type) = mysql_fetch_row($reSetting)) {
			
			if( $var_value == 'on' ) $options[$var_name] = true;
			else $options[$var_name] = false;
		}
		
		
		$this->_use_photo 	= $options['user_use_photo'];
	...
	function initLang($module_name, $platform) {
		...
		$this->_lang =& DoceboLanguage::createInstance($module_name, 'framework');
	}
	...
	function usePhoto() { return $this->_use_photo; } 
	...
	function getAction() {
		...
		if(isset($_REQUEST[$this->_varname_action])) return importVar($this->_varname_action);
	...
	function performAction($viewer = false, $start_action = false) {
		...
		$ap = importVar($this->_varname_action, false, $start_action);
		if(isset($_POST['undo'])) $ap = 'profileview';
		
		switch($ap) {
			case "goprofile" : {

				$ext_prof = new UserProfile( importVar('id_user', true), false );
			...
			case "save_policy" : {
				...
				if($this->_up_data_man->setFieldAccessList($this->_id_user, $this->_up_viewer->getFilledPolicy())) {
					...
					if($this->_end_url !== false) jumpTo($this->_end_url);

					return getResultUi($this->_lang->def('_OKSAVEPOLICY'))
						.$this->getProfile();
				} else {
					// some error saving ---------------------------
			...
			case "saveinfo" : {
				...
				if($this->saveUserInfo()) {
					...
					if (isset($_GET['modname']) && $_GET['modname'] == 'reservation')
					{
						...
						jumpTo('index.php?modname=reservation&op=reservation');
					}
					else
						return getResultUi($this->_lang->def('_OKSAVEPROFILE')).$this->getProfile();
				} else {
					// some error saving ---------------------------
					return getErrorUi($this->_lang->def('_ERR_SAVE'))
						.$this->getModUser();
				}
			};break;
			// save password -----------------------------------------
			case "savepwd" : {
				...
				if($this->saveUserPwd()) {
					...
					$out =  getResultUi($this->_lang->def('_OKSAVEPWD'));
			...
			case "saveavatar" : {
				...
				if($this->saveUserAvatar()) {
					...
					return getResultUi($this->_lang->def('_OKSAVEAVATAR'))
						.$this->getProfile();
				} else {
					// some error saving ---------------------------
					return getErrorUi($this->_lang->def('_FAILSAVEAVATAR'))
						.$this->_up_viewer->modAvatarGui();
				}
			};break;
			case "uploadphoto" : {

				return $this->_up_viewer->modPhotoGui();
			};break;
			case "savephoto" : {

				if($this->saveUserPhoto()) {
					// all ok --------------------------------------
					$this->_up_viewer->unloadUserData();

					if($this->_end_url !== false) jumpTo($this->_end_url);

					return getResultUi($this->_lang->def('_OKSAVEPHOTO'))
						.$this->getProfile();
				} else {
					// some error saving ---------------------------
					return getErrorUi($this->_lang->def('_FAILSAVEPHOTO'))
						.$this->_up_viewer->modPhotoGui();
				}
			};break;
			// teacher curriculum
			...
			case "save_teach_curric" : {
				...
				if($this->_up_data_man->saveTeacherCurriculumAndPublication($this->_id_user, $this->_up_viewer->getFilledCurriculum(), false)) {
					...
					return getResultUi($this->_lang->def('_OKSAVECURRICULUM'))
						.$this->getProfile();
				} else {
					// some error saving ---------------------------
					return getErrorUi($this->_lang->def('_FAILSAVECURRICULUM'))
			...
			case "save_teach_publ" : {
				...
				if($this->_up_data_man->saveTeacherCurriculumAndPublication($this->_id_user, false, $this->_up_viewer->getFilledPublications())) {
					...
					return getResultUi($this->_lang->def('_OKSAVEPUBLICATIONS'))
						.$this->getProfile();
				} else {
					// some error saving ---------------------------
			...
			// display the profile ------------------------------------
			case "view_files" : {
				$this->_id_user = importVar('id_user', true, getLogUserId() );
	...
	function getProfile($viewer = false) { 
		...
		if($this->_id_viewer != $this->_id_user) {
			if($this->_id_viewer == getLogUserId()) {
				...
				if(!$GLOBALS['current_user']->isAnonymous()) $this->_up_data_man->addView($this->_id_user, $this->_id_viewer);
			} else {
	...
	/**
	 * save the user photo displaied by the mod photo gui
	 * @return bool true if all data was saved correctly, false in case of trouble
	 */
	function saveUserPhoto() {

		$photo = $this->_up_viewer->getFilledPhoto();
		if($photo == 'delete_current') {

			return $this->_up_data_man->deletePhotoData($this->_id_user);
		} elseif($photo == 'nop') {

			return true;
		}
		return $this->_up_data_man->savePhotoData($this->_id_user, $photo, $this->_up_viewer->max_dim_avatar, $this->_up_viewer->max_dim_avatar);
	}
...
class UserProfileViewer {
	...
	function addStyleSheet($from_platform) {
		...
		if(isset($GLOBALS['page'])) {
			addCss('style_profile', $from_platform);
			return true;
		}
		return false;
	}
	...
	/**
	 * this function simplify the display of an avatar or a photo
	 *
	 * @return string the path to the photo/avatar
	 */
	function getPASrc($image, $alt, $base_reduce_class) {
		...
		$class_image = '';
		if($img_size[0] > $this->max_dim_avatar && $img_size[0] > $img_size[1]) $class_image .= $base_reduce_class.'_width';
		elseif($img_size[1] > $this->max_dim_avatar) $class_image .= ' '.$base_reduce_class.'_height';
	...
	function getAvailablePhotoAvatar($dimension = 'medium') {
		...
		// photo ------------------------------------------------------------------------------
		if($this->user_info[ACL_INFO_PHOTO] != "") {

			return $this->getPASrc(	$this->user_info[ACL_INFO_PHOTO],
									$this->_lang->def('_PHOTO'),
									$class_picture);
		} elseif($this->user_info[ACL_INFO_AVATAR] != "") {
		...
		$img_size = getimagesize(getPathImage().'profile/user.png');
		...
		return '<img'.( $class_picture != '' ? ' class="'.$class_picture.'"' : '' ).' '
			.'src="'.getPathImage().'profile/user.png'.'" '
			.'alt="'.$this->_lang->def('_NOPHOTO').'" />';
	}
	...
	function initAjax() {
		...
		$GLOBALS['page']->add('<script type="text/javascript">'
			...
			." var user_profile_lang = {"
				...
				."_SEND_MESSAGE : '"			.addslashes($lang->def('_SEND'))."',  "
				."_UNDO : '"					.addslashes($lang->def('_UNDO'))."', "
				."_ASK_FRIEND_SEND : '"		.addslashes($lang->def('_SEND'))."', " 
				."_ASK_FRIEND_FAIL : '"		.addslashes($lang->def('failed'))."',  "
				."_MESSAGE_SUBJECT :  '"		.addslashes($lang->def('_MESSAGE_SUBJECT'))."', " 
				."_MESSAGE_TEXT : '"			.addslashes($lang->def('_MESSAGE_TEXT'))."',  "
				."_MESSAGE_SEND : '"			.addslashes($lang->def('_MESSAGE_SEND'))."',  "
				."_MESSAGE_FAIL : '"			.addslashes($lang->def('_MESSAGE_FAIL'))."' "
			."};"
	...
	function getUserRelatedAction($selected, $with_file = true, $reduced = false) {
		...
		if(!$is_friend) {
			...
			$action[] = '<li id="profile_friend_request">'
					.'<a href="javascript:;" onclick="ask_a_friend(\''.$this->_id_user.'\'); return false;">'
					.'<img src="'.getPathImage('fw').'profile/'.$pre.'add_to_friend_enabled.png" alt="'.$this->_lang->def('_EN_ADD_FRIEND_ALT').'" /></a>'	
				.'</li>';
		} elseif(!$reduced) {
			
			$action[] = '<li id="profile_friend_request">'
					.'<img src="'.getPathImage('fw').'profile/'.$pre.'add_to_friend_disabled.png" alt="'.$this->_lang->def('_DIS_ADD_FRIEND_ALT').'" />'
					.'</li>';
		}*/
		/*if($send_msg) {	
			$action[] = '<li>'
					.'<a href="javascipt:void(0)" onclick="send_a_message(\''.$this->_id_user.'\'); return false;">'
					.'<img src="'.getPathImage('fw').'profile/'.$pre.'message_enabled.png" alt="'.$this->_lang->def('_EN_SEND_MESAGE_ALT').'" /></a>'
				.'</li>';
		}elseif(!$reduced) {
			$action[] = '<li>'
					.'<img src="'.getPathImage('fw').'profile/'.$pre.'message_disabled.png" alt="'.$this->_lang->def('_DIS_SEND_MESAGE_ALT').'" />'
				.'</li>';		
		}*/
		...
		if($with_file) {
			...
			foreach($file_type as $type) {
				...
				if($files_info[$type]) {
					...
					$action[] =  '<li>'
						.'<img src="'.getPathImage('fw').'profile/'.$pre.'dis_'.$type.'.png" alt="'.$this->_lang->def('_DIS_USER_'.strtoupper($type).'_ALT').'" />'
					.'</li>';
				}*/
		...
		if($is_online && $is_online !== 'unk') {
			...
			$action[] = '<li>'
				.'<img src="'.getPathImage('fw').'profile/'.$pre.'is_online.png" alt="'.$this->_lang->def('_UP_ONLINE').'" />'
				.'</li>';
		} else/*if(!$reduced)*/ {
		
			$action[] = '<li>'
				.'<img src="'.getPathImage('fw').'profile/'.$pre.'is_offline.png" alt="'.$this->_lang->def('_UP_OFFLINE').'" />'
				.'</li>';
		}
	...
	function getUserInfo() {
		...
		// photo and avatar -------------------------------------------------------------------
		if($this->_user_profile->usePhoto() || $this->_user_profile->useAvatar()) {
		...
		if($this->_user_profile->useAvatar()) {
			...
			$html .= ( $edit_mode ? '<a href="'.$this->_url_man->getUrl($this->_varname_action.'=uploadavatar').'" title="'.$this->_lang->def('_NEWAVATAR').'">' : '' )
	
				.( ($this->user_info[ACL_INFO_AVATAR] != "")
					? $this->getPASrc(	$this->user_info[ACL_INFO_AVATAR],
										$this->_lang->def('_AVATAR'),
										$class_picture )
					: '<img src="'.getPathImage().'profile/user.png" alt="'.$this->_lang->def('_NOAVATAR').'" />' )
				.'<br />'
				. $this->_lang->def('_AVATAR')
		...
		// show photo -------------------------------------------------------------------------
		if($this->_user_profile->usePhoto()) {
			
		$html .=( $edit_mode ? '<a href="'.$this->_url_man->getUrl($this->_varname_action.'=uploadphoto').'" title="'.$this->_lang->def('_NEWAVATAR').'">' : '' )

			.( ($this->user_info[ACL_INFO_PHOTO] != "")
				? $this->getPASrc(	$this->user_info[ACL_INFO_PHOTO],
									$this->_lang->def('_PHOTO'),
									$class_picture )
				: '<img src="'.getPathImage().'profile/user.png" alt="'.$this->_lang->def('_NOPHOTO').'" />' )
			.'<br />'
			.$this->_lang->def('_PHOTO')
		...
		if($this->_user_profile->godMode()) {
			...
			$html .= $this->getUIRowCode($this->_lang->def('_USER_LEVEL'), $user_level_string);
		}
		if($viewer == $this->_id_user) {
			...
			$html .= '<tr><th scope="row" id="up_quota">'.$this->_lang->def('_USER_QUOTA').'</th>'
				...
				.( $max_quota == USER_QUOTA_UNLIMIT
							...
							: drawProgressBar($percent, true, 'progress_bar up_quota_bar', false, false)
						)
				.'</td></tr>';
		}
			...
			if($this->_user_profile->policyMode()) {
				...
				$html .='<li id="up_modify_policy">'
						.'<a href="'.$this->_url_man->getUrl($this->_varname_action.'=mod_policy').'"
							 title="'.$this->_lang->def('_MOD_POLICY').'">'
	...
	function getViewUserFiles() {
		...
		switch($type) {
			...
			case "image" : {
				...
				$html .= '<h2 class="up_type1">'.$this->_lang->def('_USER_IMAGES').'</h2>'
				...
				if($re_files && $num_files > 0) {
					...
					while($video = $user_file->fetch_row($re_files)) {
						...
						$html .= '<li>'
								...
								.$video[MYFILE_TITLE].( $play_code === false ? ' ('.$this->_lang->def('_CLICK_TO_DOWNLOAD').')' : '' ) 
								.'</a>'
								.'</li>';
					}
					$html .= '</ul>';
				...
				if($re_files && $num_files > 0) {
					...
					while($audio = $user_file->fetch_row($re_files)) {
						...
						$html .= '<li>'
								...
								.$audio[MYFILE_TITLE].( $play_code === false ? ' ('.$this->_lang->def('_CLICK_TO_DOWNLOAD').')' : '' ) 
								.'</a>'
								.'</li>';
					}
					$html .= '</ul>';
	...
	function tinyUserInfo($picture) {
		...
		$html .=( ($this->user_info[ACL_INFO_PHOTO] != "")
				? $this->getPASrc($this->user_info[ACL_INFO_PHOTO], $this->_lang->def('_PHOTO'), $class_picture)
				: '<img class="'.$class_picture.'" src="'.getPathImage().'profile/user.png" alt="'.$this->_lang->def('_NOPHOTO').'" />' )
			.'<br />'
			.$this->_lang->def('_PHOTO');
		// end photo --------------------------------------------------------------------------
		$html .= '</div>';
	...
	function minimalUserInfo($picture, $link_to = false) {
		...
		$online_status = ( $is_online === 'unk'
							? $this->_lang->def('_HIDDEN_ONLINE_STATUS')
		...
		if($this->_user_profile->useAvatar()) {
			...
			$html .=( ($this->user_info[ACL_INFO_AVATAR] != "")
					...
					: '<img class="'.$class_picture.'" src="'.getPathImage().'profile/user.png" alt="'.$this->_lang->def('_NOAVATAR').'" />' )
				.'<br />'
				.$this->_lang->def('_AVATAR');
			// end avatar --------------------------------------------------------------------------
			$html .= '</div>';
	...
	function getUserPanelData($picture) {
		...
		$data['photo'] = ( ($this->user_info[ACL_INFO_PHOTO] != "")
				? $this->getPASrc($this->user_info[ACL_INFO_PHOTO], $this->_lang->def('_PHOTO'), $class_picture)
				: '<img class="'.$class_picture.'" src="'.getPathImage().'profile/user.png" alt="'.$this->_lang->def('_NOPHOTO').'" />' );
	...
	function getUserInfoModUi() {
		...
		// user standard info -----------------------------------------------------------------
		$html .= Form::openForm('mod_up', $this->_url_man->getUrl($this->_varname_action.'=saveinfo'));
		if($this->_user_profile->godMode()) {
			...
			$html .= Form::getTextfield($this->_lang->def('_USERNAME'),
										...
										'up_userid',
										'255',
										importVar('up_userid', false, $this->acl_man->relativeId($this->user_info[ACL_INFO_USERID]), true ) );
		} else {
		...
		$html .= Form::getTextfield(	$this->_lang->def('_LASTNAME'),
										...
										'up_lastname',
										'255',
										importVar('up_lastname', false, $this->user_info[ACL_INFO_LASTNAME], true ) )
				.Form::getTextfield(	$this->_lang->def('_FIRSTNAME'),
										'up_firstname',
										'up_firstname',
										'255',
										importVar('up_firstname', false, $this->user_info[ACL_INFO_FIRSTNAME], true ) )
				.Form::getTextfield(	$this->_lang->def('_EMAIL'),
										'up_email',
										'up_email',
										'255',
										importVar('up_email', false, $this->user_info[ACL_INFO_EMAIL], true ) );
		// user extra field ------------------------------------------------------------------
		...
		if($this->_user_profile->godMode()) {
			...
			$html .= Form::getPassword(	$this->_lang->def('_NEW_PWD'),
									'up_new_pwd',
									'up_new_pwd',
									'255' );
									
			$html .= Form::getPassword(	$this->_lang->def('_RE_PWD'),
									'up_repeat_pwd',
									'up_repeat_pwd',
									'255' );
			$lv_lang =& DoceboLanguage::createInstance('admin_directory', 'framework');
			...
			$html .= Form::getDropdown(	$this->_lang->def('_NEW_LEVEL'),
									'up_level',
									'up_level',
									$level_list,
		...
		$html .= Form::getTextarea(	$this->_lang->def('_SIGNATURE'),
									'up_signature',
									'up_signature',
									importVar('up_signature', false, $this->user_info[ACL_INFO_SIGNATURE], true ) );
	...
	function getFilledData() {
		...
		$std_filled = array(
			'lastname' 		=> importVar('up_lastname', false, '' ),
			'firstname' 	=> importVar('up_firstname', false, '' ),
			'email' 		=> importVar('up_email', false, '' ),
			'signature' 	=> importVar('up_signature', false, '' )
		);
		if($this->_user_profile->godMode()) {
			$std_filled['userid'] 		= importVar('up_userid', false, '' );
			$std_filled['new_pwd'] 		= importVar('up_new_pwd', false, '' );
			$std_filled['repeat_pwd'] 	= importVar('up_repeat_pwd', false, '' );
			$std_filled['level'] 		= importVar('up_level', false, '' );
		}
		return $std_filled;
	}
	...
	function getUserPwdModUi() {
		...
		$html .= Form::getPassword(	$this->_lang->def('_NEW_PWD'),
									...
									'up_new_pwd',
									'255' )
			.Form::getPassword(	$this->_lang->def('_RE_PWD'),
									'up_repeat_pwd',
									'up_repeat_pwd',
									'255' );
		...
		$html .= Form::openButtonSpace()
				.Form::getButton('save', 'save', $this->_lang->def('_SAVE'));
		if(!isset($GLOBALS['framework']['profile_only_pwd']) && $GLOBALS['framework']['profile_only_pwd'] == 'off') {
	...
	function checkUserPwd() {
		...
		if($_POST['up_new_pwd'] != $_POST['up_repeat_pwd']) {
			...
			return $this->_lang->def('_PASSNOMATCH');
		}
		return true;
	}
	...
	function getFilledPwd() {
		...
		return importVar('up_new_pwd', false, '' );
	}
	...
	function modAvatarGui() {
		...
		if($this->user_info[ACL_INFO_AVATAR] != '') {
			...
			$html .= Form::getButton('delete_current', 'delete_current', $this->_lang->def('_DELETE_CURRENT'), 'up_delete_current');
		}
	...
	function modPhotoGui() {
		...
		// user standard info -----------------------------------------------------------------
		$html .= getInfoUi( str_replace('[max_px]', $this->max_dim_avatar, $this->_lang->def('_AVATAR_PHOTO_INSTRUCTION') ) );
		$html .= Form::openForm('mod_photo', $this->_url_man->getUrl($this->_varname_action.'=savephoto'), false, false, 'multipart/form-data');

		$html .= Form::getFilefield(	$this->_lang->def('_PHOTO'),
										'up_photo',
										'up_photo' );
		if($this->user_info[ACL_INFO_PHOTO] != '') {

			$html .= Form::getButton('delete_current', 'delete_current', $this->_lang->def('_DELETE_CURRENT_PHOTO'), 'up_delete_current');
		}
		$html .= Form::openButtonSpace()
				.Form::getButton('save', 'save', $this->_lang->def('_SAVE'))
				.Form::getButton('undo', 'undo', $this->_lang->def('_UNDO'))
	...
	function getFilledPhoto() {

		if(isset($_POST['delete_current'])) return 'delete_current';
		if(isset($_FILES['up_photo'])) return $_FILES['up_photo'];
		return 'nop';
	}
	...
	function modUserPolicyGui() {
		...
		$policy_arr = array(
			PFL_POLICY_FREE => $this->_lang->def('_PFL_POLICY_FREE'),
			PFL_POLICY_FRIENDS => $this->_lang->def('_PFL_POLICY_FRIENDS'),
		...
		$html .= '<tbody>'
			.$this->getUIPolicyCode(	$this->_lang->def('_LASTNAME'),
										...
										Form::getInputDropdown(	'dropdown_wh',
											...
											$policy_arr,
											(isset($field_policy['lastname']) ? $field_policy['lastname'] : 0 ) ,
											''
										)
									)
			...
			.$this->getUIPolicyCode(	$this->_lang->def('_FIRSTNAME'),
										...
										Form::getInputDropdown(	'dropdown_wh',
											...
											$policy_arr,
											(isset($field_policy['firstname']) ? $field_policy['firstname'] : 0 ) ,
											''
										)
									);
		...
		while(list($id, $value) = each($user_field)) {
			...
			$html .= $this->getUIPolicyCode(	$value['name'],
												...
												Form::getInputDropdown(	'dropdown_wh',
																		...
																		'policy_selected['.$id.']',
																		$policy_arr,
																		(isset($field_policy[$id]) ? $field_policy[$id] : 0 ) ,
											''
																	)
											);
		}
		...
		$html .= $this->getUIPolicyCode(	$this->_lang->def('_PRIVATE_MESSAGE_FROM'),
											...
											Form::getInputDropdown(	'dropdown_wh',
												...
												$reduced_policy_arr,
												(isset($field_policy['message_recipients']) ? $field_policy['message_recipients'] : PFL_POLICY_TEACHER_AND_FRIENDS ) ,
												''
											)
										);
		$html .= $this->getUIPolicyCode(	$this->_lang->def('_SHOWME_ONLINE'),
											...
											Form::getInputDropdown(	'dropdown_wh',
												...
												$reduced_policy_arr,
												(isset($field_policy['online_satus']) ? $field_policy['online_satus'] : PFL_POLICY_TEACHER_AND_FRIENDS ) ,
												''
											)
										);
		...
		$html .= '<div class="up_left_block">'
			.'<h3>'.str_replace('[firstname]', $this->resolveUsername(true), $this->_lang->def('_ACTIVITY_OF')).'</h3>'
				...
				.'<b>'.$this->_lang->def('_REGISTER_DATE')	.': </b>'
					.$GLOBALS['regset']->databaseToRegional($this->user_info[ACL_INFO_REGISTER_DATE], 'date')
			.'</div>';
		if(!empty($friend_list) && is_array($friend_list)) {
			...
			while((list($id, $info) = each($friend_list)) && $i < 7) {
				...
				$html .= '<li>'
						...
						.( $info[ACL_INFO_PHOTO] != ""
							? $this->getPASrc(	$info[ACL_INFO_PHOTO],
												$this->_lang->def('_PHOTO'),
												'image_limit_small')
							: '<img class="image_limit_small" src="'.getPathImage().'profile/user.png" alt="'.$this->_lang->def('_NOPHOTO').'" />' )
						.$friend_username
						.'</a></li>';
				$i++;
			}
	...
	function getFileInfo() {
		...
		$html .= '<div class="up_box_files">'
				.'<h3 id="up_image">'.$this->_lang->def('_USER_IMAGES').'</h3>';
		...
		if($this->_user_profile->editMode()) {
			...
			$html .= '<li id="tp_del_curriculum">'
						.'<a href="'.$this->_url_man->getUrl($this->_varname_action.'=del_teach_curric').'"
							 title="'.$this->_lang->def('_DEL_CURRICULUM_T').'">'
							.$this->_lang->def('_DEL_CURRICULUM').'</a>'
					.'</li>';
			$html .= '<li id="tp_mod_curriculum">'
						.'<a href="'.$this->_url_man->getUrl($this->_varname_action.'=mod_teach_curric').'"
							 title="'.$this->_lang->def('_CHANGE_CURRICULUM_T').'">'
							.$this->_lang->def('_CHANGE_CURRICULUM').'</a>'
					.'</li>'
					.'<li id="tp_mod_publications">'
						.'<a href="'.$this->_url_man->getUrl($this->_varname_action.'=mod_teach_publ').'"
							 title="'.$this->_lang->def('_CHANGE_PUBLICATIONS_T').'">'
							.$this->_lang->def('_CHANGE_PUBLICATIONS').'</a>'
					.'</li>'
					.'</ul>';
		}
		return $html;
	...
	function delTeacherCurriculumGui()
	{
		if( isset($_GET['confirm']))
		{
			...
			jumpTo('index.php?modname=profile&op=profile');
		}
		else
		{
			...
			$html .= getDeleteUi(
					...
					$this->_lang->def('_DEL_CURRIC'),
					true,
					'index.php?modname=profile&op=profile&ap=del_teach_curric&confirm=1',
	...
	function modTeacherCurriculumGui() {
		...
		$html .= Form::getTextarea(	$this->_lang->def('_TEACHER_CURRICULUM'),
										'tp_curriculum',
										'tp_curriculum',
										importVar('tp_curriculum', false, $this->_up_data_man->getTeacherCurriculum( $this->_user_profile->getIdUser() )) );
	...
	function modTeacherPublicationsGui() {
		...
		$html .= Form::getTextarea(	$this->_lang->def('_TEACHER_PUBLICATIONS'),
										'tp_publications',
										'tp_publications',
										importVar('tp_publications', false, $this->_up_data_man->getTeacherPublications( $this->_user_profile->getIdUser() )) );
	...
	function getUserCourseStatUi($stats_data) {
		...
		$html = '<h2 class="up_coursestat">'.$this->_lang->def('_USERCOURSE_STATS_TITLE').'</h2>';

		$html .= '<table class="up_coursestat" summary="'.$this->_lang->def('_USERCOURSE_STATS_SUMMARY').'">'
				.'<caption>'.$this->_lang->def('_USERCOURSE_CAPTION').'</caption>';
		$html .= '<tr>'
					.'<th scope="col">'.$this->_lang->def('_CODE').'</th>'
					.'<th scope="col">'.$this->_lang->def('_COURSE_NAME').'</th>'
					.'<th scope="col">'.$this->_lang->def('_STATUS').'</th>'

					.'<th scope="col">'.$this->_lang->def('_USERCOURSE_STATUS').'</th>'
					.'<th scope="col">'.$this->_lang->def('_DATE_ISCR').'</th>'
					.'<th scope="col">'.$this->_lang->def('_DATE_FIRST_ACCESS').'</th>'
					.'<th scope="col">'.$this->_lang->def('_DATE_COMPLETE').'</th>'
					.'<th scope="col">'.$this->_lang->def('_LAST_ACCESS').'</th>'

					.'<th scope="col">'.$this->_lang->def('_ACCESS_COUNT').'</th>'
					.'<th scope="col">'.$this->_lang->def('_ACCESS_TIME').'</th>'

					.'<th scope="col">'.$this->_lang->def('_SCORE_INIT').'</th>'
					.'<th scope="col">'.$this->_lang->def('_SCORE_FINAL').'</th>'
					.'<th scope="col">'.str_replace(':', '', $lang_test->def('_TEST_TOTAL_SCORE')).'</th>'
				.'</tr>';
		while(list($id_c, $info) = each($stats_data)) {
			...
			$html .= '<tr>'
						...
						.'<td>'.$info['course_status'].'</td>'

						.'<td>'.$info['user_status'].'</td>'
						.'<td class="nowrap">'.$GLOBALS['regset']->databaseToRegional($info['date_inscr']).'</td>'
						.'<td class="nowrap">'.$GLOBALS['regset']->databaseToRegional($info['date_first_access']).'</td>'
						.'<td class="nowrap">'.$GLOBALS['regset']->databaseToRegional($info['date_complete']).'</td>'
						.'<td class="nowrap">'.$GLOBALS['regset']->databaseToRegional($info['access_last']).'</td>'

						.'<td class="align_right">'.( isset($info['access_count']) ? $info['access_count'] : '&nbsp;' ).'</td>'
						...
						.'</td>'
						.'<td class="align_right">'.( isset($info['score_init']) ? $info['score_init'] : '&nbsp;' ).'</td>'
						.'<td class="align_right">'.( isset($info['score_final']) ? $info['score_final'] : '&nbsp;' ).'</td>'
						.'<td class="align_right">'.$info['point_do'].'</td>'
					.'</tr>';
		}
		$html .= '</table>';
		return $html;
  ...
  function getUserCompetences(&$comp_data) {
    $html = '<h2 class="up_coursestat">'.$this->_lang->def('_USERCOMPETENCES_TITLE').'</h2>';

		$html .= '<table class="up_coursestat" summary="'.$this->_lang->def('_USERCOMPETENCES_SUMMARY').'">'
				.'<caption>'.$this->_lang->def('_USERCOMPETENCES_CAPTION').'</caption>';
		$html .= '<tr>'
					.'<th scope="col">'.$this->_lang->def('_COMPETENCE_CATEGORY').'</th>'
					.'<th scope="col">'.$this->_lang->def('_COMPETENCE_NAME').'</th>'
					.'<th scope="col">'.$this->_lang->def('_TYPE').'</th>'
					.'<th scope="col">'.$this->_lang->def('_COMPETENCE_REQUIRED').'</th>'

					.'<th scope="col">'.$this->_lang->def('_COMPETENCE_SCORE_INIT').'</th>'
					.'<th scope="col">'.$this->_lang->def('_COMPETENCE_SCORE_GOT').'</th>'
					.'<th scope="col">'.$this->_lang->def('_COMPETENCE_SCORE_TOTAL').'</th>'
					
				.'</tr>';
		foreach($comp_data as $key=>$value) {
      $count=count($value['competences']);
			$html .= '<tr>'
			      .'<td rowspan="'.count($value['competences']).'">'.$value['category'].'</td>';
			      
			$first=true;
			foreach ($value['competences'] as $key2=>$value2) {
			 $html .= (!$first ? '<tr>' : '')
						.'<td>'.$value2['name'].'</td>'
						.'<td class="image">'.$value2['type'].'</td>'
						.'<td class="image">'.$value2['required'].'</td>';
						
				switch ($value2['type']) {
          case 'flag': {
            $scoreinit='-';
            $scoregot='-';
            $scoretotal=(isset($value2['score_got']) ? $this->_lang->def('_COMPETENCE_ACQUIRED') : '-');
          } break;
          case 'score': { 
        ...
        $html .= '<td class="image">'.( $scoreinit  == '' ? '0' : $scoreinit  ).'</td>'
						    .'<td class="image">'.( $scoregot   == '' ? '0' : $scoregot   ).'</td>'
						    .'<td class="image">'.( $scoretotal == '' ? '0' : $scoretotal ).'</td>'
					   .'</tr>';
				$first=false;
			}
		}
...
class UserProfileData {
	...
	/**
	 * this function return the path to the folder in which the avatat and photo are stored
	 *
	 * @return string the path to the photo/avatar
	 */
	function getPAPath() { return '/doceboCore/'.$GLOBALS['framework']['pathphoto']; }
	...
	function setCacheForUsers($arr_user) {
		...
		while(list($user, $curr, $pubb) = mysql_fetch_row($re_teacher_data)) {
	...
	function getUserStats($id_user) {
		...
		$forum_post = $forum_cms->getUserForumPostCms($id_user) + $forum_lms->getUserForumPostLms($id_user);
		
		$my_file = new MyFile($id_user);
		$stats = array(
			'forum_post' => $forum_post,
			'blog_post' => 'unk',
			'loaded_file' => $my_file->getFileCount()
		);
		return $stats;
	}
	...
	/**
	 * save a new photo for the user
	 */
	function savePhotoData($id_user, $file_descriptor, $max_width, $max_height) {

		if(!$this->deletePhotoData($id_user)) return false;
		...
		$savefile = $id_user.'p_'.mt_rand(0,100).'_'.time().'_'.$file_descriptor['name'];
		if(file_exists($GLOBALS['where_files_relative'].$this->getPAPath().$savefile)) return false;
		...
		if(!$this->acl_man->updateUser( $id_user,
			...
			sl_unlink($this->getPASrc().$savefile);
			return false;
		}
		return true;
	}
	...
	/**
	 * delete the current user photo
	 */
	function deletePhotoData($id_user) {
		...
		if($user_info[ACL_INFO_PHOTO] != '') {
			...
			if(!sl_unlink(substr($this->getPAPath(), 1).$user_info[ACL_INFO_PHOTO])) return false;
			if(!$this->acl_man->updateUser( $id_user,
											false,
											false,
											false,
	...
	function getUserProfileViewList($id_user, $limit) {
		...
		LIMIT 0,".$limit;
		if(!$re_query = mysql_query($query)) return $user_list;
	...
	function loadTeacherData($id_user) {
		...
		list($this->_teacher_data[$id_user]['curriculum'], $this->_teacher_data[$id_user]['pubblications']) = mysql_fetch_row($re);
	}
	...
	function saveTeacherCurriculumAndPublication($id_user, $curriculum = false, $publications = false) {
		...
		WHERE id_user = '".$id_user."'";
		$re = $this->_query($query);
		list($num_of) = mysql_fetch_row($re);
		if($num_of) {
	...
	function getUserCourseStat($id_user) {
		...
		$c_lang 	=& DoceboLanguage::CreateInstance('admin_course_managment', 'lms');
		...
		while(list($id_c, $code, $name, $status,
				$status_user, $date_inscr, $date_first_access, $date_complete) =  mysql_fetch_row($re_course_user) ) {
			...
			$course_status = array(
				CST_PREPARATION => $c_lang->def('_CST_PREPARATION', 'admin_course_managment', 'lms'),
				CST_AVAILABLE 	=> $c_lang->def('_CST_AVAILABLE', 'admin_course_managment', 'lms'),
				CST_EFFECTIVE 	=> $c_lang->def('_CST_CONFIRMED', 'admin_course_managment', 'lms'),
				CST_CONCLUDED 	=> $c_lang->def('_CST_CONCLUDED', 'admin_course_managment', 'lms'),
				CST_CANCELLED 	=> $c_lang->def('_CST_CANCELLED', 'admin_course_managment', 'lms')
			);
		...
		while(list($id_c, $session_num, $time_num, $last_num) = mysql_fetch_row($re_time)) {
    ...
    foreach ($comp as $key=>$value) {
     if ($value['id_competence']!='') {
      ...
      $temp=array();
    	...
    	$temp['required'] = ($value['idst']!='' ? 'yes' : 'no');
    	$temp['score_init'] = $value['score_init'];
    	$temp['score_got'] = $value['score_got'];
    	$temp['score_total'] = $value['score_init']+$value['score_got'];
    	$output[$cat]['competences'][]=$temp;
     }
    }
    
    foreach ($output as $key=>$value) {
      $catdat = $_man->GetCompetenceCategory($key);
      $output[$key]['category'] = $catdat['name'];
    }
