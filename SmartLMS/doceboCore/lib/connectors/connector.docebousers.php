...
class DoceboConnectorDoceboUsers extends DoceboConnector {
	...
	**/
	...
	function connect() {
		...
		$this->directory =& new Module_Directory();
		$this->tree_view =& $this->directory->getTreeView_OrgView();
		$this->people_view =& $this->directory->getPeopleView(); 
		$this->groupFilter_idst = $this->tree_view->aclManager->getGroupST($this->groupFilter);
		$this->people_view->data->setGroupFilter($this->groupFilter_idst, FALSE );
		...
		$tmp = $this->tree_view->aclManager->getGroup( false, '/oc_0' );
		$arr_idst[] = $tmp[0];
		$this->tree_oc = $tmp[0];
		$tmp = $this->tree_view->aclManager->getGroup( false, '/ocd_0' );
		$arr_idst[] = $tmp[0];
		$this->tree_ocd = $tmp[0];
		...
		if( $rs === FALSE ) {
			$this->last_error = $this->tree_view->lang->def('_ORG_IMPORT_ERR_ERRORONQUERY').$query.' ['.mysql_error($this->dbconn).']';
			return FALSE;
		}
		...
		$this->org_chart_group = $this->tree_view->aclManager->getBasePathGroupST('/oc');

		$query = " SELECT id_dir, translation "
				." FROM ".$GLOBALS['prefix_fw']."_org_chart"
				." WHERE lang_code = '".getLanguage()."'";
		$result = mysql_query($query);
		while(list($id_dir, $dir_name) = mysql_fetch_row($result)) {
			...
			$re = mysql_query($query_idstMember);
			while(list($idstMember) = mysql_fetch_row($re)) {
	...
	function get_next_row() {
		...
		$row = mysql_fetch_row($this->data);print_r($row);
		if($row == false) {
			$this->eof = true;
			return false;
		}
	...
	**/
	...
	function add_row( $row, $pk ) {
		...
		if( $arr_user === FALSE ) {
			if( $firstname === NULL ) 	$firstname 	= $this->default_cols['firstname'];
			if( $lastname === NULL ) 	$lastname 	= $this->default_cols['lastname'];
			if( $pass === NULL ) 		$pass 		= $this->default_cols['pass'];
			if( $email === NULL ) 		$email 		= $this->default_cols['email'];
			
			$idst = $this->tree_view->aclManager->registerUser( $userid, $firstname, $lastname,
																$pass, $email, '', '',
																'');
		} else {
			$idst = $arr_user['idst'];
			...
			$result = $this->tree_view->aclManager->updateUser( $idst, $userid, $firstname, $lastname,
		...
		if($idst !== false) {
			...
			if($tree_code) {
				...
				foreach($to_add as $id_dir) {
					...
					$this->tree_view->aclManager->addToGroup($idst_oc_folder, $idst);
					$this->tree_view->aclManager->addToGroup($idst_ocd_folder, $idst);
				}
				foreach($to_del as $id_dir) {
					...
					$this->tree_view->aclManager->removeFromGroup($idst_oc_folder, $idst);
					$this->tree_view->aclManager->removeFromGroup($idst_ocd_folder, $idst);
				}
			}
			//  -------------------------------------------------------------------
			$result = TRUE;
			$this->tree_view->aclManager->addToGroup($this->tree_oc, $idst);
			$this->tree_view->aclManager->addToGroup($this->tree_ocd, $idst);
			
			if ($this->tree != $this->tree_oc) {
				$this->tree_view->aclManager->addToGroup($this->tree,$idst );
				$this->tree_view->aclManager->addToGroup($this->tree_desc,$idst );
			}
			
			// add to group level
			$userlevel = $this->tree_view->aclManager->getGroupST(ADMIN_GROUP_USER);
			$this->tree_view->aclManager->addToGroup($userlevel,$idst );
	...
	function delete_bypk( $pk ) {
		...
		if( $arr_people === FALSE )
			...
			if( $this->canceled == '1' )
				$this->tree_view->aclManager->suspendUser($arr_people['idst']);
			else
				$this->tree_view->aclManager->deleteUser($arr_people['idst']);
		}
	}
	...
	function delete_all_filtered( $arr_pk ) {
		...
		foreach( $arr_idst_todelete as $id_st ) {
			if( $this->canceled == '1' )
				$this->tree_view->aclManager->suspendUser($id_st);
			else
				$this->tree_view->aclManager->deleteUser($id_st);			
		}
	}
	...
	function delete_all_notinserted() {
		...
		$counter = 0;
		if( $idst_rs !== FALSE )
			while(list($id_st) = mysql_fetch_row($idst_rs)) {
				if( $this->canceled == '1' )
					$this->tree_view->aclManager->suspendUser($id_st);
				else
					$this->tree_view->aclManager->deleteUser($id_st);
