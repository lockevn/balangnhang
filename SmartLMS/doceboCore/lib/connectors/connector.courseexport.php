...
class DoceboConnectorCourseExport extends DoceboConnector {
	...
	**/
	function connect()
	{
		...
		list($number_of_course) = mysql_fetch_row(mysql_query($query));
		
		$this->tot_row = $number_of_course;
		
		$query = 	"SELECT `code`, `name`, `description`, `lang_code`, `status`, `subscribe_method`, `mediumTime`, `permCloseLO`, `difficult`, `show_progress`, `show_time`, `show_extra_info`, `show_rules`, `date_begin`, `date_end`, `valid_time`, `max_num_subscribe`, `min_num_subscribe`, `selling`, `prize`"
					." FROM ".$GLOBALS['prefix_lms']."_course"
					." ORDER BY name";
