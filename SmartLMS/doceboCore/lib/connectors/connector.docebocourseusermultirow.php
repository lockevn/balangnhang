	...
	**/
	...
	function get_row_bypk($pk) {
		...
		if($this->course_cache === false) {
			...
			while(list($id_course, $code, $name) = mysql_fetch_row($re_course)) {
	...
	function add_row( $row, $pk ) {
        		...
        		&& ($row['status'] == 'c' || $row['status'] == 'p'))
		{
            ...
            list($status, $date_inscr, $date_first_access, $date_complete) = mysql_fetch_row(mysql_query($query));
	...
	function delete_all_notinserted() {
		...
		FROM ".$GLOBALS['prefix_lms']."_course
		WHERE 1";
		$re_course = mysql_query($search_course);
		while(list($id_course, $name) = mysql_fetch_row($re_course)) {
		...
		$re_courseuser = mysql_query($search_query);
		if(!$re_courseuser) return 0;
		$counter = 0;
		while(list($id_course, $id_user) = mysql_fetch_row($re_courseuser)) {
...
class DoceboConnectorUI_DoceboCourseUserMultiRowUI extends DoceboConnectorUI {
	...
	function _step0() {
		...
		$out .= $this->form->getRadioSet( 	$this->lang->def('_CANCELED_COURSEUSER'), 
											...
											array( 	$this->lang->def('_UNACTIVATE')  => '1', 
