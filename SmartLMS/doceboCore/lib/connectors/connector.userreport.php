...
class DoceboConnectorUserReport extends DoceboConnector {
	...
	**/
	...
	function connect() {
		...
		$this->_query_result = mysql_query($query_course_user);
	...
	function _converStatusCourse($status) {
		switch($status) {
			...
			case "2" : return $this->lang->def('_DEACTIVE');
		}
	}
	...
	function get_first_row() {
		...
		$result = mysql_fetch_row($this->_query_result);
		if(!$result) {
			$this->_readed_end = true;
			return array();
		}
	...
	function get_next_row() {
		...
		$row = array();
		if(!$this->_query_result) return false;
		if(!$result = mysql_fetch_row($this->_query_result)) {
			$this->_readed_end = true;
			return FALSE;
		}
		$this->row_index++;
