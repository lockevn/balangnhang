...
class DoceboCal_lms extends DoceboCal_core{
	...
	function getEvents($year=0,$month=0,$day=0,$start_date="",$end_date="") {
		...
		$query="SELECT a.* FROM ".$GLOBALS['prefix_fw']."_calendar AS a,".$GLOBALS['prefix_lms']."_calendar AS b WHERE a.id=b.id AND (a.private<>'on' OR (a.private='on' AND a._owner='".$GLOBALS['current_user']->getIdSt()."')) AND b.idCourse='".$_SESSION['idCourse']."' AND ".$where." ORDER BY start_date";
