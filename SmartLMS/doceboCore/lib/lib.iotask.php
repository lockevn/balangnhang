...
class DoceboConnectionManager {
	...
	function get_first_connection() {
		...
		$this->rs_connection = mysql_query($query);
		if( $this->rs_connection === FALSE )
			return FALSE;
		if( mysql_num_rows($this->rs_connection) == 0 )
			return FALSE;
		
		return mysql_fetch_row($this->rs_connection);
	}
	...
	function get_all_connections_name() {
		...
		$rs_connection = mysql_query($query);
		if( $rs_connection === FALSE )
			return FALSE;
		if( mysql_num_rows($rs_connection) == 0 )
			return array();
		$arr_name = array();
		while( list($name) = mysql_fetch_row($rs_connection) ) 
			$arr_name[] = $name;
		return $arr_name;
	}
	...
	function get_connection_byname( $name ) {
		...
		return mysql_fetch_row($rs_connection);
	}
	...
	function delete_connection_byname( $name ) {
		...
		if( mysql_query($query) ) 
			...
			$this->last_error = $lang->def('_ERROR_DB').mysql_error();
			return FALSE;
		}
	}
	...
	function save_connection( $old_name, $connection ) {
		...
		if( strlen(trim($name)) == 0 ) {
			$this->last_error = $lang->def('_ERROR_SPECIFY_NAME');
			return FALSE;
		}
	...
	function get_first_connector() {
		...
		$this->rs_connector = mysql_query($query);
		if( $this->rs_connector === FALSE )
			return FALSE;
		if( mysql_num_rows($this->rs_connector) == 0 )
			return FALSE;
		
		return mysql_fetch_row($this->rs_connector);	
	}
	...
	function get_connector_byfile($file ) {
		$query = "SELECT type, file, class FROM ".$GLOBALS['prefix_fw']."_connector"
				." WHERE file = '".$file."'" ;
		$rs_connector = mysql_query($query);
		if( $rs_connector === FALSE )
			return FALSE;
		if( mysql_num_rows($rs_connector) == 0 )
			return FALSE;
		
		return mysql_fetch_row($rs_connector);
	}
	...
	function get_first_task() {
		...
		ORDER BY sequence ";
		$this->rs_task = mysql_query($query);
		if( $this->rs_task === FALSE )
			return FALSE;
		if( mysql_num_rows($this->rs_task) == 0 )
			return FALSE;
		
		$arr_result = mysql_fetch_row($this->rs_task);
		if( $arr_result === FALSE )
			return FALSE;
	...
	function get_task_byname($task_name) {
		...
		$rs_task = mysql_query($query);
		if( $rs_task === FALSE )
			return FALSE;
		if( mysql_num_rows($rs_task) == 0 )
			return FALSE;
		
		$arr_result = mysql_fetch_row($rs_task);
...
class DoceboImport {
	...
	**/
	function getUIMap() {
		...
		$form = new Form();
		$table = new TypeOne($GLOBALS['visuItem'], $lang->def('_IMPORT_MAP'), $lang->def('_IMPORT_MAP'));
