...
function sl_mkdir( $path, $mode ) {
	...
	if( $uploadType == "ftp" ) {
		...
		doDebug(($result ? '\tTRUE' : '\tFALSE')." mkdir( ".$GLOBALS['where_files_relative'].$path.", ".$mode." )");
		return $result;
	}	
}
...
function sl_fopen( $filename, $mode ) {
	$uploadType = $GLOBALS['uploadType'];
	doDebug("sl_fopen( $filename, $mode )");
	$mfirst = $mode{0};
...
function sl_upload_fs( $srcFile, $dstFile ) {
	...
	if(!$re) doDebug("Error on move_uploaded_file from: $srcFile to $dstFile");
	return $re;
}
