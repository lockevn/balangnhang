...
class TreeDb_OrgDb extends TreeDb {
	...
	function getFolderTranslations( $idFolder ) {
		...
		while( list( $lang_code, $translation ) = mysql_fetch_row( $rs ) ) {
	...
	function getFolderIdByTranslations( $translation ) {
		...
		list( $id_dir ) = mysql_fetch_row( $rs );
		return $id_dir;
	}
	...
	function getFoldersCurrTranslation( $arr_groupsid ) {
		...
		$rs = mysql_query( $query );
		$arrTranslations = array();
		while( list( $id_dir, $translation ) = mysql_fetch_row( $rs ) ) {
...
class TreeView_OrgView extends TreeView {
	...
	function  TreeView_OrgView($tdb, $id, $rootname = 'root') {
		...
		$this->multi_choice = $GLOBALS['use_org_chart_multiple_choice'] == '1'; 
	}
	...
	function _getLabelAssignField() 	{ return $this->lang->def('_ORGCHART_ASSIGN_FIELD_ALT'); }
	...
	function _getImgAssignUser()		{ return getPathImage('fw').'org_chart/assign_user.gif';}
	function _getLabelAssignUser() 		{ return $this->lang->def('_ORGCHART_ASSIGN_USER_ALT'); }
	
	function _getOpCreateUser() 		{ return 'createUser';}
	function _getImgCreateUser()		{ return getPathImage('fw').'standard/user.gif';}
	function _getLabelCreateUser() 		{ return $this->lang->def('_ORGCHART_CREATE_USER_ALT'); }	
	
	function _getOpWaitingUser() 		{ return 'waitingUser';}
	function _getImgWaitingUser()		{ return getPathImage('fw').'org_chart/waiting_user.gif';}
	function _getLabelWaitingUser() 	{ return $this->lang->def('_ORGCHART_WAITING_USER'); }
	...
	function _getAddAlt() 		{ return $this->lang->def('_ORGCHART_ADDNODE_ALT'); }
	...
	function load() {
		...
		if( $this->deselect_all ) {
			...
			$this->itemSelectedMultiAlt = array();
		}
			...
			if( $this->isFolderAccessible() && checkPerm('createuser_org_chart', true, $to_check, 'framework') ) {
				return array(	
					$this->id.$this->_getOpCreateUser() => array(	$this->id .'['.$this->_getOpCreateUser().']', 
																	$this->lang->def('_ORGCHART_CREATE_USER'), 
	...
	function getFolderPrintName( &$folder ) {
		$print_name = "";
		if($folder->level == 0)
			$print_name = $GLOBALS['title_organigram_chart'];
		else {
			if( $this->filter_nodes === FALSE )
				...
				$print_name = $this->lang->def('_ORGCHAR_HIDDENNODE');
		}
		return $print_name;
	}
	...
	function extendedParsing( $arrayState, $arrayExpand, $arrayCompress ) {
		if( isset($arrayState['editpersonsave']) ) {
			...
			if( $idst !== '' ) {
				...
				if( $userid != '' ) {
					...
					if(($this->aclManager->absoluteId($userid) != $info[ACL_INFO_USERID]) || ($this->aclManager->encrypt($pass) != $info[ACL_INFO_PASS])) {
						...
						$msg_composer->setSubjectLangText('sms', '_MODIFIED_USER_SBJ_SMS', false);
					...
					$GLOBALS['page']->add(getResultUi($this->lang->def('_MODIFIED_USER') ));
				...
				if( $this->aclManager->getUserST($userid) !== FALSE ) {
					...
					if( $arr_idst_groups != FALSE && $userid != '') {
						...
						if($GLOBALS['current_user']->getUserLevelId() != ADMIN_GROUP_GODADMIN) {
							...
							$limit_insert = $GLOBALS['current_user']->preference->getPreference('admin_rules.limit_user_insert');
							$max_insert = $GLOBALS['current_user']->preference->getPreference('admin_rules.max_user_insert');
							$direct_insert = $GLOBALS['current_user']->preference->getPreference('admin_rules.direct_user_insert');
							
							if( ($limit_insert == 'off') || ($limit_insert == 'on' && $max_insert > 0)) {
								
								if($direct_insert == 'on') {
									
									$GLOBALS['current_user']->preference->setPreference('admin_rules.max_user_insert', $max_insert -1 );
									...
									$msg_composer->setSubjectLangText('sms', '_WAITING_USER_SBJ_SMS', false);
			...
			if( $idst !== '' ) {
				...
				$GLOBALS['page']->add(getResultUi($this->lang->def('_DELETED_USER') ));
			}
		}
				...
				if( !isset( $action[$mand_lang] ) || $action[$mand_lang] == '' ) { 
					...
					//if( $filled === true ) {
						...
						if($this->getSelectedFolderId() == '0') {
							...
							if(mysql_query($query_root_name)) 
								$GLOBALS['framework']['title_organigram_chart'] = $folderName;
						...
						$this->refresh = TRUE;
						if($this->getSelectedFolderId() == '0') jumpTo('index.php?modname=directory&op=org_chart');
					/*} else {
	...
	function canAdd() {
		if( is_array( $this->filter_nodes ) ) {
			...
			if($GLOBALS['current_user']->getUserLevelId() != ADMIN_GROUP_GODADMIN) return false;
		}
		return $this->isFolderAccessible() && ($GLOBALS['use_org_chart'] == '1');
	}
	...
	function printElement(&$stack, $level) {
		...
		if( !$this->selector_mode ) {
			...
			if( $this->isFolderAccessible($stack[$level]['folder']) && $GLOBALS['use_user_fields'] == '1' )
			...
			if($level != 0 ) {
				if( $this->isFolderAccessible($stack[$level]['folder']) ) {
					...
					if( $GLOBALS['use_org_chart_field'] == '1'
						&& $GLOBALS['current_user']->getUserLevelId() == ADMIN_GROUP_GODADMIN )
						$tree .= '<input type="image" class="tree_view_image" ' 
			...
			/* -- custom: checkbox per Seleziona Ramo
			$tree .= Form::getLabel( 	DIRECTORY_ID.DIRECTORY_OP_SELECTFOLD.'_FOLD_'.$idst, 
										$this->lang->def('_ORG_CHART_FORFOLDER'), 
										'label_bold tree_view_image' );
			$tree .= '<input type="checkbox" class="tree_view_image"'
					.' id="'.DIRECTORY_ID.DIRECTORY_OP_SELECTFOLD.'_FOLD_'.$idst.'" '
					.' name="'.$check_name.'" '
					.' value=""'
					.$disabled;
			if( array_search( $idst, $this->itemSelectedMulti_alt ) !== FALSE ) {	
				$tree .= ' checked="checked" ';
			} 				
	...
	function loadNewFolder() {
		...
		$tree = $form->getFormHeader($this->lang->def('_ORG_CHART_NEW_FOLDER'));
		...
		foreach($array_lang as $k => $lang_code ) {			
			$tree .= $form->getTextfield(( ($mand_lang == $lang_code) ? '<span class="mandatory">*</span>' : '' ).$lang_code, 
									...
									255, 
									'',
									$lang_code.' '.$this->lang->def('_ORG_CHART_NEW_FOLDER') );
		}
	...
	function loadRenameFolder() {
		...
		$tree = $form->getFormHeader($this->lang->def('_ORG_CHART_RENAME_FOLDER'));
		$tree .= $form->openElementSpace();
		$tree .= $this->printState();
		...
		if($this->getSelectedFolderId() == '0') {
			...
			$tree .= $form->getTextfield($this->lang->def('_ROOT_RENAME'), 
										...
										255, 
										$GLOBALS['framework']['title_organigram_chart'],
										$GLOBALS['framework']['title_organigram_chart']
											.' '.$this->lang->def('_ORG_CHART_RENAME_FOLDER') );
		} else {
			...
			foreach($array_lang as $k => $lang_code ) {			
				$tree .= $form->getTextfield(( ($mand_lang == $lang_code) ? '<span class="mandatory">*</span>' : '' ).$lang_code, 
										...
										$lang_code.' '.$this->lang->def('_ORG_CHART_RENAME_FOLDER') );
			}
		}
		$tree .= $form->closeElementSpace();
	...
	function loadFolderField2() {
		...
		require_once($GLOBALS['where_framework'].'/lib/lib.form.php');
		$form = new Form();
		$tree = $form->getFormHeader($this->lang->def('_ORG_CHART_LIST_FIELDS_MANDATORY') . " : " .$id_folder_desc );
		$tree .= $form->openElementSpace();
		$tree .= $this->printState();
	...
	function loadAssignField() {
		...
		foreach($arr_all_fields as $field ) {
			...
			if( $def_value == ORG_CHART_FIELD_INHERIT ) {
				...
				if( $gid == '/ocd_0' ) 
					$text = $GLOBALS['title_organigram_chart'];
				else {
	...
	function loadAssignField2() {
		...
		require_once($GLOBALS['where_framework'].'/lib/lib.form.php');
		$form = new Form();
		$tree = $form->getFormHeader($this->lang->def('_ORG_CHART_LIST_FIELDS_MANDATORY'));
		$tree .= $form->openElementSpace();
		$tree .= $this->printState();
		...
		$tree .= '<div class="label_head">'.$this->lang->def('_ORG_CHART_FIELD_MANDATORY').'</div>';
		$tree .= '<div class="label_head">'.$this->lang->def('_ORG_CHART_FIELD_WRITE').'</div>';
		$tree .= $form->closeFormLine();
		*/
		$tb = new TypeOne(0, 
			$this->lang->def('_ORG_CHART_FIELD_CAPTION'), 
			$this->lang->def('_ORG_CHART_FIELD_SUMMARY'));
		$tb->setTableStyle('tree_org_table_field');
		$tb->addHeadCustom('<tr class="first_intest">'
			.'<th scope="col" abbr="'.$this->lang->def('_NAME').'">'
				.$this->lang->def('_ORG_CHART_FIELD_NAME').'</th>'
			.'<th scope="col" abbr="'.$this->lang->def('_ORG_CHART_FIELD_MANDATORY_ABBR').'">'
				.$this->lang->def('_ORG_CHART_FIELD_MANDATORY').'</th>'
		...
		foreach( $arr_fields as $id_filed => $status ) {
			if( $status == ORG_CHART_FIELD_NORMAL || $status == ORG_CHART_FIELD_DESCEND ) {
				...
				if( isset( $arr_fields_normal[$id_filed] ) && $arr_fields_normal[$id_filed][FIELD_INFO_MANDATORY] == 'true' )
					$tree .= ' checked="checked"';
				$tree .= ' />';
				$tree .= $form->getLabel( $this->id.'_'.$id_filed.'_mandatory', $this->lang->def('_ORG_CHART_FIELD_MANDATORY'), 'label_bold access-only' );
				...
				$input_manadatory .= ' />'
					.$form->getLabel( $this->id.'_'.$id_filed.'_mandatory', $this->lang->def('_ORG_CHART_FIELD_MANDATORY'), 'label_bold access-only' );
	...
	function loadImportUsers2() {
		...
		if($_FILES['file_import']['name'] == '') {
			...
			jumpTo( $back_url.'&import_result=-1' );
		} else {
			$path = '/doceboCore/';
			...
			if(!file_exists( $GLOBALS['where_files_relative'].$path.$savefile )) {
				...
				$_SESSION['last_error'] = def('_ERRORUPLOAD');
				jumpTo( $back_url.'&create_result=-1' );
			}
		}
	...
	function loadImportUsers3() {
		...
		$tree .= getResultUi(str_replace("%count%", $result[0], $this->lang->def('_ORG_CHART_IMPORT_USER_COUNT')) );
		
		if( count($result) > 1 ) {
			require_once($GLOBALS['where_framework'].'/lib/lib.newtypeone.php');
			$tree .= str_replace("%count%", count($result)-1, $this->lang->def('_ORG_CHART_IMPORT_USER_ERR_COUNT'));
			$table = new TypeOne($GLOBALS['visuItem'], $this->lang->def('_ORG_CHART_IMPORT_USER_ERR_TABLE'), $this->lang->def('_ORG_CHART_IMPORT_USER_ERR_TABLE'));
			$table->setColsStyle(array('',''));
			$table->addHead(array(	$this->lang->def('_ORG_CHART_IMPORT_USER_ERR_ROW'),
									$this->lang->def('_ORG_CHART_IMPORT_USER_ERR_TEXT')
							));
