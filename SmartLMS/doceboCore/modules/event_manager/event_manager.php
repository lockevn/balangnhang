...
function event_manager_view($op) {
	...
	if( $op == 'save' ) {
		...
		foreach( $arr_permission as $key => $permission) {
			$result = mysql_query(	"UPDATE ".$GLOBALS['prefix_fw']."_event_manager "
	...
	$rs = mysql_query( "SELECT ec.idClass, class, platform, description, idEventMgr, permission, channel, recipients"
						." FROM ".$GLOBALS['prefix_fw']."_event_class as ec"
						." JOIN ".$GLOBALS['prefix_fw']."_event_manager as em"
						." WHERE ec.idClass = em.idClass"
						." ORDER BY idEventMgr" );

	while( list($idClass,$class,$platform,$description,$idEventMgr,$permission,$channel,$recipients) = mysql_fetch_row($rs) ) {
		$cont = array();
...
function event_special_view($op) {
	...
	if( $op == 'special_save' ) {
		...
		foreach( $arr_platform as $key => $platform) {
			if( $key == 0 ) {
				if( $platform != '' ) {
					$result1 = mysql_query(	"INSERT INTO ".$GLOBALS['prefix_fw']."_event_class "
					...
					$result1 = mysql_query(	"INSERT INTO ".$GLOBALS['prefix_fw']."_event_manager "
											." (idClass,recipients,show_level) VALUES "
											." ('".$idClass."','".$arr_recipients[$key]."','".$arr_show_level[$key]."') ");
				}
			} else {
				$result1 = mysql_query(	"UPDATE ".$GLOBALS['prefix_fw']."_event_class "
		...
		foreach( $arr_recipients as $key => $recipients ) {
			if( $key == 0 ) {
				...
				$result2 = mysql_query(	"UPDATE ".$GLOBALS['prefix_fw']."_event_manager "
	...
	$tb_event_classes = new TypeOne(400, $lang->def('_EVENT_SETTINGS'), $lang->def('_EVENT_SETTINGS'));
	...
	$rs = mysql_query( "SELECT ec.idClass, class, platform, description, idEventMgr, recipients, show_level"
						." FROM ".$GLOBALS['prefix_fw']."_event_class as ec"
						." JOIN ".$GLOBALS['prefix_fw']."_event_manager as em"
						." WHERE ec.idClass = em.idClass"
						." ORDER BY ec.idClass" );

	while( list($idClass,$class,$platform,$description,$idEventMgr,$recipients,$show_level) = mysql_fetch_row($rs) ) {
		$cont = array();
