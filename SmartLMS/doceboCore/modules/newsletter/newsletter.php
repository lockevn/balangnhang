...
function newsletter() {
	...
	$user_info=$acl_manager->getUser($GLOBALS["current_user"]->getIdSt(), false);
	...
	//array_unshift($lang_list, $lang->def("_DEFAULT"), $lang->def("_ANY"));
	$lang_list =array(_ANY_LANG_CODE=>$lang->def("_ANY"))+$lang_list;
...
function nl_pause() {
	...
	$out->add("<br />".$lang->def("_SENDING_MSG_FROM")." ".($cycle*$ipc)." ".$lang->def("_SENDING_MSG_TO")." ".($cycle*$ipc+$ipc)."<br />\n");
...
function nl_sendcomplete() {
	...
	$out->add("<br /><b>".$lang->def("_SEND_COMPLETE")."</b><br /><br />\n");
...
function init_send() {
	...
	$savefile = '';
	$max_file = get_req('file_number', DOTY_INT, 0);
...
// http://127.0.0.1:82/claudio/lms_cvs/doceboCore/index.php?modname=newsletter&op=selsendto&id_send=1
	...
	if( isset($_POST['okselector']) ) {
		...
		foreach($arr_selection as $idstMember) {
			$arr=$mdir->aclManager->getGroupAllUser($idstMember);
		...
		if ($lang != _ANY_LANG_CODE) {
			$tot =count($mdir->aclManager->getUsersIdstByLanguage($lang, $send_to_idst));
		}
		else {
	...
	else if( isset($_POST['cancelselector']) )
	{
		...
		jumpTo('index.php?modname=newsletter&op=newsletter');
	}
	else {
		...
		$mdir->loadSelector($url,
			$lang->def( '_NEWSLETTER_SENDTO' ), "", TRUE);
	}
...
function newsletterSummary($id_send) {
	...
	$out->add($form->getTextBox(getInfoUi($txt)));
