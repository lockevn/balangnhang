...
function adminManager_list() {
	...
	$contentH = array( 	$lang->def( '_USERNAME' ),
						...
						'<img src="'.getPathImage().'admin_manager/man_menu.gif" alt="'.$lang->def( '_ADMIN_MENU_TITLE' ).'" '
							.'title="'.$lang->def( '_ADMIN_MENU_TITLE' ).'" />',

						'<img src="'.getPathImage().'directory/tree.gif" alt="'.$lang->def( '_ADMIN_ASSIGNTREE_TITLE' ).'" '
							.'title="'.$lang->def( '_ADMIN_ASSIGNTREE_TITLE' ).'" />',

						'<img src="'.getPathImage().'admin_manager/lang_lang.gif" alt="'.$lang->def( '_ADMIN_ASSIGN_LANG_TITLE' ).'" '
							.'title="'.$lang->def( '_ADMIN_ASSIGN_LANG_TITLE' ).'" />'	);
	$typeH = array( '', '', 'image', 'image', 'image', 'image');
	if ($lms_is_active) {

		$contentH[] = '<img src="'.getPathImage().'admin_manager/man_course.gif" alt="'.$lang->def( '_ADMIN_ASSIGNCOURSE_TITLE' ).'" '
							.'title="'.$lang->def( '_ADMIN_ASSIGNCOURSE_TITLE' ).'" />';
		$typeH[] = 'image';
	}
	if ($cms_is_active) {
...
function adminManager_assign_tree( $adminidst ) {
	...
	if( isset($_POST['okselector']) ) {
		...
		jumpTo( 'index.php?modname=admin_manager&amp;op=view' );
...
function adminManager_lang_course() {
	...
	if(isset($_POST['save'])) {
		...
		jumpTo('index.php?modname=admin_manager&op=view&result='.($re ? 1 : 0 ));
	}
...
function adminManager_edit_course() {
	...
	if(isset($_POST['save_selection'])) {
		...
		jumpTo('index.php?modname=admin_manager&amp;op=view&amp;result='.( $re ? 'ok' : 'err' ));
	}
