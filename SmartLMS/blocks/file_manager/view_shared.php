...
*/
	...
	/*if ($SESSION->fromdiscussion != $id){
		if (! $course = get_record('course', 'id', $SESSION->fromdiscussion) ) {
        error("That's an invalid course id", "view.php?id={$id}&amp;rootdir={$rootdir}");
		}
	} else*/if (! $course = get_record('course', 'id', $id) ) {
        error("That's an invalid course id", "view.php?id={$id}&amp;rootdir={$rootdir}");
    }
	...
	if (!isset($catlinkid) && !isset($foldlinkid)) {
		...
		*/
		if (!function_exists('build_navigation')){
    		...
    		$strtitle = strip_tags(get_string('othersharedfiles', 'block_file_manager'));
	        $navigation = build_navigation(array(array('name'=>$strtitle, 'link'=>" $courselink", 'type'=>'misc')));
		...
		*/
		if (!function_exists('build_navigation')){
		    ...
		    // new method path level 2 @ romain
    		$strtitle = strip_tags(get_string('othersharedfiles', 'block_file_manager'));
	        $navigation = build_navigation(array(array('name'=>$strtitle, 'link'=>"", 'type'=>'misc')));
    	    print_header($strtitle, $course->fullname, $navigation, '', '', true, '', ''); 
    	}
	}
