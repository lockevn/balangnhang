...
*/
	...
	*/
    ...
    if (!function_exists('build_navigation')){
        ...
        $navigation = build_navigation(array(array('name'=>$strtitle, 'link'=>" $courselink "."<a href=\"view.php?id={$id}&amp;groupid={$groupid}&amp;rootdir={$rootdir}\">".get_string("filemanager", 'block_file_manager').
		"</a> -> ".get_string('folders', 'block_file_manager'), 'type'=>'misc')));
        print_header($strtitle, $course->fullname, $navigation, '', '', true, '', ''); 
    }
		...
		if (isset($foldid)) {
			...
			if ($foldername) {
				if (count_records('fmanager_folders','name', $foldername, 'path', $rootdir) != count_records('fmanager_folders', 'name', $foldername, 'id', $foldid)) {
					...
					if ($groupid == 0){
						// MODIF JF
						if (isset($oldpath) && isset($oldpath->path) && isset($oldpath->name) ) {
							$newpath .= "/".fm_get_user_dir_space()."$oldpath->path$oldpath->name/$foldername";
						}
						else{
							$newpath .= "/".fm_get_user_dir_space()."/$foldername";;
						}
					} else {
						// MODIF JF
						if (isset($oldpath) && isset($oldpath->path) && isset($oldpath->name)) {
							...
							$newpath .= "/".fm_get_group_dir_space($groupid)."/$foldername";
						}
					}
					if (file_exists($newpath)) {
						print_simple_box(get_string('msgduplicate', 'block_file_manager'), 'center', '', '#FFFFFF');
