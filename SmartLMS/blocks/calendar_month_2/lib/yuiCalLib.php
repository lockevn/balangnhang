 ...
 */
function calendar_get_link_previous_yui($text, $linkbase, $d, $m, $y, $accesshide=false, $defaultURL) {
    $href = calendar_get_link_href($linkbase, $d, $m, $y);
    $defaulthref = calendar_get_link_href($defaultURL, $d, $m, $y);
    if(empty($href)) return $text;
    return link_arrow_left_yui($text, $href, $accesshide, 'previous', $defaulthref);
}
 ...
 * @return string HTML string.
 */
function calendar_get_link_next_yui($text, $linkbase, $d, $m, $y, $accesshide=false, $defaultURL) {
    $href = calendar_get_link_href($linkbase, $d, $m, $y);
    $defaulthref = calendar_get_link_href($defaultURL, $d, $m, $y);
    if(empty($href)) return $text;
    return link_arrow_right_yui($text, $href, $accesshide, 'next', $defaulthref);
}
...
function calendar_top_controls_yui($type, $data) {
    ...
    switch($type) {
        case 'frontpage':
            ...
            $nextlink = calendar_get_link_next_yui(get_string('monthnext', 'access'), $CFG->wwwroot.'/blocks/calendar_month_2/mini.php?course='.$data['id'].'&', 0, $nextmonth, $nextyear, $accesshide=true, 'index.php?');
            $prevlink = calendar_get_link_previous_yui(get_string('monthprev', 'access'), $CFG->wwwroot.'/blocks/calendar_month_2/mini.php?course='.$data['id'].'&', 0, $prevmonth, $prevyear, true, 'index.php?');
 ...
 * @return string HTML string.
 */
function link_arrow_right_yui($text, $url='', $accesshide=false, $addclass='', $default) {
    global $THEME;
    check_theme_arrows();
    $arrowclass = 'arrow ';
    if (! $url) {
    ...
    if ($url) {
        ...
        return '<a'.$class.' href="'.$default.'" onclick="nextAJAX('.$url.'); return false;" title="'.preg_replace('/<.*?>/','',$text).'">'.$htmltext.$arrow.'</a>';
    }
 ...
 * @return string HTML string.
 */
function link_arrow_left_yui($text, $url='', $accesshide=false, $addclass='', $default) {
    global $THEME;
    check_theme_arrows();
    $arrowclass = 'arrow ';
    if (! $url) {
    ...
    if ($url) {
        ...
        return '<a'.$class.' href="'.$default.'" onclick="prevAJAX('.$url.'); return false;" title="'.preg_replace('/<.*?>/','',$text).'">'.$arrow.$htmltext.'</a>';
    }
...
function calendar_get_mini_yui($courses, $groups, $users, $cal_month = false, $cal_year = false) {
    ...
    while($weekCount<5){$content .= '<tr><td>&nbsp;</td></tr>';$weekCount++;}
