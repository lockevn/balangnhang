...
class PayAccount_WireTransfer extends PayAccount {

	/**
	 * class constructor
	 */
	function PayAccount_WireTransfer() {
	...
	function getFormDetails() {
		...
		$lang =& DoceboLanguage::createInstance('payaccount_wire_transfer', 'ecom');
	...
	function saveDetails($data_source) {
		...
		foreach ($data_source as $key => $value) {
			if($key!='save' && $key!='account_name'){
				...
				WHERE account_name='wire_transfer' and  param_name ='".$key."'";
