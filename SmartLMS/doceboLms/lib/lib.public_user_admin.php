...
class PeopleListView extends ListView {
		...
		if( !$this->selector_mode ) {
			...
			if($this->del_perm) {
				$colInfos[] = array( 	'hLabel' => '<img src="'.$this->_getDeleteImage().'" '
										...
										.'title="'.$this->lang->def( '_DIRECTORY_DELPERSON' ).'" />',
									'hClass' => "image",
									'fieldClass' => "image",
									'data' => 'delete',
									'toDisplay' => true,
	...
	function userExtraData($id_user) {
		...
		$profile->init('profile', 'lms', 'modname=public_user_admin&op=org_manageuser&id_user='.$id_user, 'ap');
	...
	function printOut() {
		...
		$profile->init('profile', 'lms', 'modname=public_user_admin&op=org_chart', 'ap');
