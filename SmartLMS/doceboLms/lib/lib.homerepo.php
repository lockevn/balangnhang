...
class HomeRepo_DataRetriever extends DataRetriever {
	...
	function deleteItem( $idItem ) {
		...
		list( $idResource, $objectType ) = mysql_fetch_row( $rs );
...
class HomeRepo_ListView extends ListView {
	...
	function _getDeleteConfirmLabel() { return _DELETEITEMCONFIRM; }
	...
	function _getMoveImage() { return getPathImage().'standard/move.gif'; }
...
class Homerepo_TreeView extends TreeView {
	...
	function _getAddLabel() { return _NEWHOMEFOLDER; }
	function _getAddAlt() { return _NEWHOMEFOLDER; }
	function _getCreateLabel() { return _CREATEHOMEFOLDER; }
	function _getCreateAlt() { return _CREATEHOMEFOLDER; }
...
function manHomerepo_showerror( &$treeView ) {
	...
	if( $treeView->error == TVERR_MOVEONDESCENDANT )
		echo _ERROR_TVERR_MOVEONDESCENDANT;
...
function &manHomerepo_CreateTreeView( $withContents = TRUE, $multiSelect = FALSE, $withActions = FALSE, $lotypes = NULL ) {
		...
		if( $withContents ) {
			...
			$listView = new HomeRepo_ListView( '', $dataRetriever, $typeOneRenderer, 'homerepo');
...
function manHomeRepoSave( &$lo, $withForm = FALSE, $withContents = TRUE, $treeView = NULL, $idLO = NULL ) {
		...
		if( $treeView->cancel ) {
			if( isset($_POST['idLO']) ) {
				jumpTo( 'index.php?modname=homerepo&op=homerepo');
			} else {
				jumpTo( ''.$lo->getBackUrl());
			}			
		}
...
function loadFields( $arrayData, &$lo, $idLO ) {
	...
	echo '<span class="mainTitle">'._CATINFO. ' ' . $title .'</span><br /><br />';
	...
	while(list($idCat, $catTitle) = mysql_fetch_row($reCategory)) {
	...
	if( isset($arrayData['difficult']) ) {
		...
		switch($selDiff) {
			case '_VERYEASY': $selDiff = "1"; break;
			case '_EASY': $selDiff = "2"; break;
			case '_MEDIUM': $selDiff = "3"; break;
			case '_DIFFICULT2': $selDiff = "4"; break;
		...
		echo '<option value="1" '.(($selDiff=="1")?'selected':'').' >'._VERYEASY.'</option>'
			...
			.'<option value="4" '.(($selDiff=="4")?'selected':'').' >'._DIFFICULT2.'</option>'
...
function manHomeRepo_ShowItem( $itemId ) {
	...
	echo '<div class="ObjectForm">';
	echo '<span class="mainTitle">'._CATINFO. ' ' . $arrayData['title'] .'</span><br /><br />';
