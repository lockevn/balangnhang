...
class RepoTreeView extends TreeView {
	...
	function _getAddImage() { return getPathImage().'treeview/newfolder.gif'; }
	function _getAddLabel() { return def('_REPOADDFOLDER', 'storage', 'lms'); }
	function _getAddAlt() { return def('_REPOADDFOLDER', 'storage', 'lms'); }
	
	function _getFolderNameLabel() { return def('_NAME', 'storage', 'lms'); }
	function _getCreateLabel() { return def('_REPOCREATEFOLDER', 'storage', 'lms'); }
	function _getCreateAlt() { return def('_REPOCREATEFOLDER', 'storage', 'lms'); }
	function _getCreateImage() { return getPathImage().'treeview/newfolder.gif'; }
	
	function _getCreateLOLabel() { return def('_REPOCREATELO', 'storage', 'lms'); }
	function _getCreateLOAlt() { return def('_REPOCREATELO', 'storage', 'lms'); }
	function _getCreateLOImage() { return getPathImage().'standard/add.gif'; }
	
	function _getOpPlayTitle() { return def('_REPOPLAYTITLE', 'storage', 'lms'); }
	function _getOpEditTitle() { return def('_REPOEDITLO', 'storage', 'lms'); }
	function _getOpCopyTitle() { return def('_REPOCOPYLO', 'storage', 'lms'); }
	function _getCopyImage()   { return getPathImage().'standard/editcopy.gif'; }
	...
	*/
	function _getOpPropertiesTitle() { return def('_REPOPROPERTIESTITLE'); }
	...
	function _getOtherActions() {
		if( $this->isFolderSelected() ) {
			...
			if( !$isFolder ) 
				return array();
				/* array(	array($this->_getOpEditLO(), def('_REPOEDITLO', 'storage', 'lms'), getPathImage().'standard/mod.gif' ),
								array($this->_getOpCopyLO(), def('_REPOCOPYLO', 'storage', 'lms'), getPathImage().'standard/editcopy.gif' )
							);*/
		}
		return array( array($this->_getOpCreateLO(), def('_REPOCREATELO', 'storage', 'lms'), getPathImage().'standard/add.gif' )	);
	}
	...
	function LOSelector( $module, $back_url ) {
		...
		$out .= '<div class="title">'
			.def('_SELECTLO', 'storage', 'lms')
			.'</div><br />';
		$out .=  getBackUi(ereg_replace('&', '&amp;', $back_url), def('_BACK') );
		$out .=  '<form id="LOSelector" method="post" action="index.php?modname='.$module.'&amp;op=display&amp;'.$this->_getOpCreateLOSel().'=1" >';
		$first = TRUE;
		while( list($objectType) = mysql_fetch_row($rs) ) {
			...
			$out .=  '<label for="'.$objectType.'">'.def('_LONAME_'.$objectType).'</label>'
				.'<br />';
			$first = FALSE;
		}
		$out .=  $this->printState();
		$out .=  '<br /><input type="submit" class="button" value="'.def('_REPOCREATELO')
	...
	function _getDeleteLabel() { 
		if( $this->isFolderSelected() ) {
			...
			if( !isset($stackData['folder']->otherValues) ) return def('_DEL');
			
			$arrData = $stackData['folder']->otherValues;
			$isFolder = ($arrData[REPOFIELDOBJECTTYPE] === '');
			if( !$isFolder ) return def('_DEL');
		}
		return def('_DELETEFOLDER');
	}
