...
class Man_Reservation {
	...
	function getCategoryName($id_category)
	{
		...
		list($name_category) = mysql_fetch_row($result);
	...
	function getSubscribedUserIdst($id_event)
	{
		...
		while(list($subscribed) = mysql_fetch_row($result))
		{
			$re[] = $subscribed;
		}
	...
	function getSubscribedUser($id_event)
	{
		...
		list($subscribed) = mysql_fetch_row($result);
	...
	function getCategory($id_course = false)
	{
		...
		if($num_rows)
		{
			while(list($id_category, $name) = mysql_fetch_row($result))
				$re[$id_category] = $name;
		}
		return $re;
	}
	...
	function getEventCategory($id_event)
	{
		...
		list($id_category) = mysql_fetch_row(mysql_query($query));
	...
	function getLaboratories()
	{
		...
		if($num_rows)
		{
			while(list($id_classroom, $name) = mysql_fetch_row($result))
	...
	function getEventInfo($id_event)
	{
		...
		if ($num_rows)
		{
			...
			list($id_event, $id_course, $id_laboratory, $id_category, $title, $description, $date, $max_user, $deadline, $from_time, $to_time) = mysql_fetch_row($result);
	...
	function getEventDropDown($id_course, $id_category, $id_user)
	{
		...
		while (list($id_event, $title) = mysql_fetch_row($result))
			$re[$id_event] = $title;
	...
	/*function getLaboratoryInfo($id_laboratory)
	{
		...
		if ($num_rows)
		{
			...
			list($id_laboratory, $name, $location, $description) = mysql_fetch_row($result);
	...
	function getMaxSubscriptionForCategory($id_category)
	{
		...
		list($re) = mysql_fetch_row(mysql_query($query));
	...
	function controlMaxSubscriptionForCategory($id_category, $id_user)
	{
		...
		list($max_subscription) = mysql_fetch_row(mysql_query($query));
		...
		list($subscription) = mysql_fetch_row(mysql_query($query_control));
	...
	function controlUserSubscription($id_user, $id_event)
	{
		...
		list($is_subscribed) = mysql_fetch_row($result);
	...
	function controlSwitchPossibility($id_course, $id_category, $id_user)
	{
		...
		$result = mysql_query($query);
		list($number_of_events) = mysql_fetch_row($result);
	...
	/*function viewLaboratories($id_course = false)
	{
		...
		if ($num_rows)
		{
			while (list($id_laboratory, $name, $location, $description, $id_course_2) = mysql_fetch_row($result))
			{
	...
	function delEvent($id_event)
	{
		...
		if ($result)
		{
			...
			return $result_2 = mysql_query($query_2);
		}
		else
			return false; 
	}
	...
	function viewEvents($id_course = false, $order_by = false)
	{
		...
		if ($num_rows)
		{
			...
			while (list($id_event, $id_course, $id_laboratory, $id_category, $title, $description, $date, $max_user, $deadline, $from_time, $to_time, $category_name, $laboratory_name) = mysql_fetch_row($result))
			{
	...
	function viewCategory($id_course = false)
	{
		...
		if($num_rows)
		{
			while(list($id_category, $name, $id_course_2, $max_subscription) = mysql_fetch_row($result))
			{
	...
	function getCategoryMaxSubscription($id_category)
	{
		...
		list($re) = mysql_fetch_row($result);
