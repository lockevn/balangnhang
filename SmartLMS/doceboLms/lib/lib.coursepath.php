...
class Selector_CoursePath {
	...
	function loadCoursepathSelector() {
		...
		if($this->show_filter === true) {
			$form = new Form();
			$GLOBALS['page']->add(
				$form->getOpenFieldset($lang->def('_COURSEPATH_FILTER'))
		...
		$tb = new TypeOne($GLOBALS['visuItem'], $lang->def('_COURSE_PATH_CAPTION'), $lang->def('_COURSE_PATH_SUMMARY'));
		...
		if($GLOBALS['current_user']->getUserLevelId() != ADMIN_GROUP_GODADMIN) {
			...
			if(empty($all_path)) {
				...
				$query_coursepath .= " AND id_path IN (".implode(',', $all_path).") ";
			}
		}
		if($this->filter['coursepath_name'] != '') {
			$query_coursepath .= " AND path_name LIKE '%".$this->filter['coursepath_name']."%'";
		}
		list($tot_coursepath) = mysql_fetch_row(mysql_query("SELECT COUNT(*) ".$query_coursepath));

		$query_coursepath .= "
		ORDER BY path_name
		LIMIT ".$ini.",".(int)$GLOBALS['visuItem'];

		$re_coursepath = mysql_query($select.$query_coursepath);

		$type_h = array('image', '', '', '');
		$cont_h = array(
			'<span class="access-only">'.$lang->def('_COURSE_PATH_SELECTION').'</span>',
			$lang->def('_COURSE_PATH_NAME'),
			$lang->def('_COURSE_PATH_DESC')
		);
		$tb->setColsStyle($type_h);
		$tb->addHead($cont_h);
		while(list($id_path, $name, $descr) = mysql_fetch_row($re_coursepath)) {
...
class CoursePath_Manager {
	...
	function getCoursepathList($ini, $result_number) {
		...
		while($row = mysql_fetch_row($repath)) {
	...
	function getCoursepathAllInfo($arr_id) {
		...
		while($row = mysql_fetch_row($re_select)) {
	...
	function getPathStructure($coursepaths) {
		...
		$re_courses = $this->_query($query);
		while(list($id_path, $in_slot, $id_item) = mysql_fetch_row($re_courses)) {
	...
	function moveUp($id_path, $id_slot, $id_course) {
		...
		list($seq) = mysql_fetch_row(mysql_query($query));
	...
	function getPathSlot($arr_path) {
		...
		while(list($id_path, $id_slot, $min_selection, $max_selection) = mysql_fetch_row($repath_elem)) {
	...
	function createSlot($id_path, $min_selection, $max_selection) {
		...
		list($sequence) = mysql_fetch_row($this->_query("
	...
	function getPathElem($id_path) {
		...
		$info = array();
		while(list($id_item, $in_slot, $prerequisites) = mysql_fetch_row($repath_elem)) {
	...
	function getSlotElem($id_path, $id_slot) {
		...
		$repath_elem = $this->_query($query_pathelem);
		$info = array();
		while(list($id_item) = mysql_fetch_row($repath_elem)) {
	...
	function getSubscribed($id_path) {
		...
		while(list($id) = mysql_fetch_row($re_users)) {
	...
	function getUserSubscriptionsInfo($id_user, $exclude_waiting = false) {
		...
		$re_users = $this->_query($query);
		while(list($id_path, $wait) = mysql_fetch_row($re_users)) {
