...
class LightRepoManager {
	...
	function getRepoDetails($id_repo) {
		...
		$re_query = mysql_query($query); 
		return mysql_fetch_row($re_query);
	}
	...
	function deleteRepo($id_repo) {
		...
		$re = mysql_query($query);
		while($old_file = mysql_fetch_row($re)) {
	...
	function getUserLastEnterInRepo($id_repo) {
		...
		{
			list($last_enter) = mysql_fetch_row(mysql_query($query));
			doDebug($query);
			return $last_enter;
		}
		return false;
	...
	function getNumberOfFileInReport ($id_repo, $from_date = false)
	{
		...
		list($new_file) = mysql_fetch_row(mysql_query($query));
	...
	function saveFile($id_file, $file_info) {
		...
		if($id_file != '0') { 
			$old_file = mysql_fetch_row($this->getFileInfo($id_file));
