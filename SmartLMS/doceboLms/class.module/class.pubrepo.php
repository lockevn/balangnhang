...
class Module_Pubrepo extends LmsModule {
	...
	//class constructor
	function Module_Pubrepo($module_name = '') {
	...
	**/
	function initialize() {
		...
		if( !$ready ) {
			...
			$this->treeView = new RepoTreeView($this->repoDb, 'pubrepo', def('_PUBREPOROOTNAME', 'storage', 'lms'));
		...
		switch($this->treeView->op) {
			...
			default:
				$GLOBALS['page']->addStart(
					getTitleArea(def('_PUBREPO', 'pubrepo', 'lms'), 'pubrepo')
