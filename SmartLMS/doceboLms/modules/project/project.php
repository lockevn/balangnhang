...
/************************************************************************/
/*          Modulo Gestione Progetti (c) 2005 Giovanni Derks            */
/************************************************************************/
...
define("_FPATH_INTERNAL", "/doceboLms/".$GLOBALS['lms']['pathprj']);
...
function project() {
	...
	$myprj = userProjectsList($GLOBALS["current_user"]->getIdSt());
	...
	$tab = new typeOne(0, $lang->def("_MYPRJ"), $lang->def("_PROJECT_SUMMARY"));

	$content_h = array( $lang->def("_TITLE"), $lang->def("_PROGRESS"), $lang->def("_PROGRESS_PERC") );
	$type_h = array( '', '', 'image' );
	if($mod_perm) {
		$content_h[] = '<img src="'.getPathImage().'standard/moduser.gif" '.
			'alt="'.$lang->def("_MODPROJECTADMIN").'" title="'.$lang->def("_MODPROJECTADMIN").'" />';
		$type_h[] = 'image' ;
		$content_h[] = '<img src="'.getPathImage().'standard/mod.gif" '.
			'alt="'.$lang->def("_MOD").'" title="'.$lang->def("_MOD").'" />';
		$type_h[] = 'image' ;
	}
	if($del_perm) {
		$content_h[] = '<img src="'.getPathImage().'standard/rem.gif" '.
	...
	foreach ($myprj as $key=>$row) {
		...
		$content = array(
			...
			/* '<img class="'.$class.'" src="'.getPathImage().'prjman/'.$img.'.gif" width="'.( $progtot*2 ).'" alt="'.$lang->def("_PROGRESS_ALT").'" />' */
		...
		if ($mod_perm) {
			if ($row["flag"] == 2) {
				$content[] = '<a href="index.php?modname=project&amp;op=manprjadmin&amp;id='.$val.'">'
					.'<img src="'.getPathImage().'standard/moduser.gif" '
	...
	if(checkPerm('add', true)) {
		$tab->addActionAdd('<a href="index.php?modname=project&amp;op=addprj" title="'.$lang->def("_ADDPROJECT").'">'
			.'<img src="'.getPathImage().'standard/add.gif" alt="'.$lang->def("_ADDPROJECT_ALT").'" /> '.$lang->def("_ADDPROJECT").'</a>');
	}
...
/*	$res=array();
	...
	$query=mysql_query("SELECT * FROM ".$GLOBALS["prefix_lms"]."_prj WHERE cid='".$_SESSION["idCourse"]."' ORDER BY ptitle;");

	if (($query) && (mysql_num_rows($query) > 0)) {
		while ($row=mysql_fetch_array($query)) {
			$grpqry=mysql_query("SELECT * FROM ".$GLOBALS["prefix_lms"]."_coursegroupuser WHERE (idGroup='".$row["pgroup"]."' AND idUser='$userid');");
...
function addprj() {
	...
	$out->add($form->getButton('save', 'save', $lang->def('_CREATEPRJ')));
	...
	$out->add("<input class=\"button\" type=\"submit\" value=\"".$lang->def("_CREATEPRJ")."\" />\n");
	$out->add("</form>\n");
	*/
	$out->add("</div>");
...
function addprj_now() {
	...
	if (!in_group($GLOBALS["current_user"]->getIdSt(), $pgroup))
	...
	$goonlink="<br /><br /><a href=\"index.php?modname=project&amp;op=project\">".$lang->def("_PRJGOON")." &gt;&gt;</a>\n";

	if ($err == "") {
		$query=mysql_query("INSERT INTO ".$GLOBALS["prefix_lms"]."_prj (ptitle,pgroup,psfiles,pstasks,psnews,pstodo,psmsg,cid) VALUES('$ptitle','$pgroup','$psfiles','$pstasks','$psnews','$pstodo','$psmsg','$idCourse');");

		if ($query) {
			$query=mysql_query("SELECT * FROM ".$GLOBALS["prefix_lms"]."_prj ORDER BY id DESC;");
			$row=mysql_fetch_array($query);
			$id=$row["id"];

			$query=mysql_query("INSERT INTO ".$GLOBALS["prefix_lms"]."_prj_users (pid,userid,flag) VALUES('$id','".$GLOBALS["current_user"]->getIdSt()."','2');");
			$out->add( mysql_error());

			//$out->add(getResultUi($lang->def("_PRJINSOK")).$goonlink);
			jumpTo("index.php?modname=project&op=project");
		}
		else
			$out->add(getErrorUi($lang->def("_PRJERR").": ".mysql_error()).$backlink);
	}
	else
		$out->add(getErrorUi($lang->def("_PRJERR").": ".$err).$backlink);
...
function show_task( $id, $row, $modimg ) {
	...
	$out->add('<div class="titleBlu">'.$lang->def("_PRJTASKS").'</div>'."\n"
		.'<div class="container_elem">');

	$query = mysql_query("SELECT * FROM ".$GLOBALS["prefix_lms"]."_prj WHERE id='$id' LIMIT 1;");
	$data = mysql_fetch_array($query);
	$progtot = $row["pprog"];

	$out->add("<table width=\"100%\">\n");
	$out->add("<tr><td>");
	$out->add("<b>".$lang->def("_PRJPROGTOT").":</b></td><td class=\"progress_td\">\n");
	if ($progtot < 100) {
		$class="prjprogbar_g";
		$img="progbar_g";
	}
	...
	/*$out->add("<img class=\"$class\" src=\"".getPathImage()."prjman/$img.gif\" width=\"".($progtot*1.6)."\" alt=\"".$lang->def("_PROGRESS_ALT")."\" />\n");*/
	$out->add(renderProgress($progtot, 0, 100, false));
	$out->add("</td><td class=\"align_right\">$progtot%</td>\n");

	$readlink = $modlink=""; $dellink="";
	if ((is_owner($GLOBALS["current_user"]->getIdSt(), $id)) || (is_admin($GLOBALS["current_user"]->getIdSt(), $id))) {
		$modlink="<a href=\"index.php?modname=project&amp;op=editprogtot&amp;id=".$id."\"><img src=\"".getPathImage()."standard/mod.gif\" alt=\"".$lang->def("_MOD")."\" /></a>";
		$dellink="<img src=\"".getPathImage()."standard/rem.gif\" alt=\"".$lang->def("_DEL")."\" />";
	}
	$out->add("<td class=\"image_td\">$modlink</td><td class=\"image_td\">&nbsp;</td>\n");
	$out->add("</tr>\n");

	$query=mysql_query("SELECT * FROM ".$GLOBALS["prefix_lms"]."_prj_tasks WHERE pid='$id' ORDER BY tname;");
	if (($query) && (mysql_num_rows($query) > 0)) {
		while ($data=mysql_fetch_array($query)) {
			...
			$readlink = $modlink=""; $dellink="";
			if ((is_owner($GLOBALS["current_user"]->getIdSt(), $id)) || (is_admin($GLOBALS["current_user"]->getIdSt(), $id))) {
				$modlink="<a href=\"index.php?modname=project&amp;op=prjedititem&amp;type=task&amp;id=$id&amp;itemid=".$data["id"]."\"><img src=\"".getPathImage()."standard/mod.gif\" alt=\"\" /></a>";
				$dellink="<a href=\"index.php?modname=project&amp;op=prjdelitem&amp;type=task&amp;id=$id&amp;itemid=".$data["id"]."\"><img src=\"".getPathImage()."standard/rem.gif\" alt=\"\" /></a>";
			}
			$out->add("<td class=\"image_td\">$modlink</td><td class=\"image_td\">$dellink</td>\n");
			$out->add("</tr>\n");
		}
	}
	$out->add("</table>\n");
	if ((is_owner($GLOBALS["current_user"]->getIdSt(), $id)) || (is_admin($GLOBALS["current_user"]->getIdSt(), $id))) {
		$out->add('<div class="prjaddrow">'
			.'<a href="index.php?modname=project&amp;op=prjadditem&amp;type=task&amp;id='.$id.'">'
			.'<img src="'.getPathImage().'standard/add.gif" alt="'.$lang->def("_ADDTASK").'" /> '.$lang->def("_ADDTASK").'</a>'
			.'</div>'."\n");
...
function show_news( $id, $row , $modimg ) {
	...
	$out->add('<div class="titleBlu">'.$lang->def("_PRJNEWS").'</div>'."\n"
		.'<div class="container_elem">');

	$query = mysql_query("SELECT * FROM ".$GLOBALS["prefix_lms"]."_prj_news WHERE pid='$id' ORDER BY ndate DESC;");
	if (($query) && (mysql_num_rows($query) > 0)) {
		$out->add("<table width=\"100%\">\n");
		while ($data=mysql_fetch_array($query)) {
			$ndate=$GLOBALS["regset"]->databaseToRegional($data["ndate"], "date");
			$modlink=""; $dellink="";
			...
			if ((is_owner($GLOBALS["current_user"]->getIdSt(), $id)) || (is_admin($GLOBALS["current_user"]->getIdSt(), $id))) {
				$modlink="<a href=\"index.php?modname=project&amp;op=prjedititem&amp;type=news&amp;id=$id&amp;itemid=".$data["id"]."\"><img src=\"".getPathImage()."standard/mod.gif\" alt=\"".$lang->def("_MOD")."\" /></a>";
				$dellink="<a href=\"index.php?modname=project&amp;op=prjdelitem&amp;type=news&amp;id=$id&amp;itemid=".$data["id"]."\"><img src=\"".getPathImage()."standard/rem.gif\" alt=\"".$lang->def("_DEL")."\" /></a>";
			}
			$out->add("<td class=\"image_td\">$modlink</td><td class=\"image_td\">$dellink\n");
			$out->add("</td></tr>\n");
		}
		$out->add("</table>\n");
	}
	if ((is_owner($GLOBALS["current_user"]->getIdSt(), $id)) || (is_admin($GLOBALS["current_user"]->getIdSt(), $id))) {
		$out->add('<div class="prjaddrow">'
			...
			.'<img src="'.getPathImage().'standard/add.gif" alt="'.$lang->def("_ADDNEWS").'" /> '.$lang->def("_ADDNEWS").'</a>'
			.'</div>'."\n");
	}
	$out->add('</div>');
}
...
function show_files( $id, $row , $modimg ) {
	...
	$out->add('<div class="titleBlu">'.$lang->def("_PRJFILES").'</div>'."\n"
		.'<div class="container_elem">');

	$query = mysql_query("SELECT * FROM ".$GLOBALS["prefix_lms"]."_prj_files WHERE pid='$id' ORDER BY ftitle;");
	if (($query) && (mysql_num_rows($query) > 0)) {
		$out->add('<table width="100%">'."\n");
		while($data = mysql_fetch_array($query)) {
			$fname = $data["fname"];
			...
			$out->add('<tr><td class="image_td">'."\n"
				.'<a href="index.php?modname=project&amp;op=download&amp;type=file&amp;id='.$data['id'].'">'.$img.'</a>'
				.'</td><td>'."\n"
				.$readlink);
			if (!empty($data["fver"]))
				$out->add( " ".$lang->def("_FILEVER")." ".$data["fver"]);
			$out->add( "</td>"
				//.
				."\n");
			if ((is_owner($GLOBALS["current_user"]->getIdSt(), $id)) || (is_admin($GLOBALS["current_user"]->getIdSt(), $id))) {
				$modlink = "<a href=\"index.php?modname=project&amp;op=prjedititem&amp;type=file&amp;id=$id&amp;itemid=".$data["id"]."\"><img src=\"".getPathImage()."standard/mod.gif\" alt=\"".$lang->def("_MOD")."\" /></a>";
				$dellink="<a href=\"index.php?modname=project&amp;op=prjdelitem&amp;type=file&amp;id=$id&amp;itemid=".$data["id"]."\"><img src=\"".getPathImage()."standard/rem.gif\" alt=\"".$lang->def("_DEL")."\" /></a>";
	...
	if ((is_owner($GLOBALS["current_user"]->getIdSt(), $id)) || (is_admin($GLOBALS["current_user"]->getIdSt(), $id))) {
		$out->add( '<div class="prjaddrow">'
			...
			.'<img src="'.getPathImage().'standard/add.gif" alt="'.$lang->def("_ADDFILE").'" /> '.$lang->def("_ADDFILE").'</a>'
			.'</div>'."\n");
	}
	$out->add( '</div>');
}
...
function show_todo( $id, $row , $modimg ) {
	...
	$out->add( '<div class="titleBlu">'.$lang->def("_PRJTODO").'</div>'."\n"
			.'<div class="container_elem">');

		$query=mysql_query("SELECT * FROM ".$GLOBALS["prefix_lms"]."_prj_todo WHERE pid='$id' ORDER BY id DESC;");
		if (($query) && (mysql_num_rows($query) > 0)) {
			$out->add("<table width=\"100%\">\n");
			while ($data=mysql_fetch_array($query)) {
				...
				$modlink=""; $dellink="";
				if ((is_owner($GLOBALS["current_user"]->getIdSt(), $id)) || (is_admin($GLOBALS["current_user"]->getIdSt(), $id))) {
					$modlink="<a href=\"index.php?modname=project&amp;op=prjedititem&amp;type=todo&amp;id=$id&amp;itemid=".$data["id"]."\"><img src=\"".getPathImage()."standard/mod.gif\" alt=\"".$lang->def("_MOD")."\" /></a>";
					$dellink="<a href=\"index.php?modname=project&amp;op=prjdelitem&amp;type=todo&amp;id=$id&amp;itemid=".$data["id"]."\"><img src=\"".getPathImage()."standard/rem.gif\" alt=\"".$lang->def("_DEL")."\" /></a>";
				}
				$out->add('<td class="image_td">'.$modlink.'</td>'
					.'<td class="image_td">'.$dellink.'</td>'."\n");
				$out->add("</tr>\n");
			}
			$out->add("</table>\n");
		}
		if ((is_owner($GLOBALS["current_user"]->getIdSt(), $id)) || (is_admin($GLOBALS["current_user"]->getIdSt(), $id))) {
			$out->add( '<div class="prjaddrow">'
				...
				.'<img src="'.getPathImage().'standard/add.gif" alt="'.$lang->def("_ADDTODO").'" /> '.$lang->def("_ADDTODO").'</a>'
				.'</div>'."\n");
		}
		$out->add( '</div>');
}
...
function show_prj() {
	...
	$id = $_GET["id"];
	$myprj = user_projects($GLOBALS["current_user"]->getIdSt());

	if( !in_array($id, $myprj) )
		die("You can't access");

	$modimg = '<img src="'.getPathImage().'standard/mod.gif" alt="'.$lang->def("_MOD").'" />';

	$query = mysql_query("SELECT * FROM ".$GLOBALS["prefix_lms"]."_prj WHERE id='$id';");
	if (($query) && (mysql_num_rows($query) > 0)) {
		$row = mysql_fetch_array($query);
	}
...
/*	$out->add('<div class="alignRight">'
	...
	if ($row["psmsg"]) {
		...
		$out->add('<h3 class="prjmsg_title">'.$lang->def("_PRJMSG").'</h3>'."\n");
		...
		$sf->can_write 		= true;
		$sf->can_moderate 	= (bool)((is_owner($GLOBALS["current_user"]->getIdSt(), $id)) || (is_admin($GLOBALS["current_user"]->getIdSt(), $id)));
		$sf->can_upload 	= true;
...
function manprjadmin() {
	...
	if(($view_perm) && (in_array($id, $myprj)) && (is_owner($GLOBALS["current_user"]->getIdSt(), $id))) {
		...
		if (isset($_POST["cancelselector"])) {
			...
			WHERE cid='".$_SESSION["idCourse"]."'
				AND id = '".$id."'";
			list($group) = mysql_fetch_array(mysql_query($query));
			if($group == $group_all) {
...
function edit_news($mode="edit") {
	...
	if(($view_perm) && (in_array($id, $myprj)) && ( (is_owner($GLOBALS["current_user"]->getIdSt(), $id)) || (is_admin($GLOBALS["current_user"]->getIdSt(), $id)) ) ) {
		...
		if ( isset($_POST["applychanges"]) ) {
			$ndate=$GLOBALS["regset"]->regionalToDatabase($_POST["ndate"], "date");
			$ntitle=$_POST["ntitle"];
			$ntxt=$_POST["ntxt"];

			if ($mode == "new") {
				$query=mysql_query("INSERT INTO ".$GLOBALS["prefix_lms"]."_prj_news (pid,ntitle,ntxt,ndate) VALUES('$id','$ntitle','$ntxt','$ndate');");
			}
			if ($mode == "edit") {
				$query=mysql_query("UPDATE ".$GLOBALS["prefix_lms"]."_prj_news SET ndate='$ndate',ntitle='$ntitle',ntxt='$ntxt' WHERE id='$itemid' LIMIT 1;");
			}
			jumpTo(" index.php?modname=project&op=showprj&id=$id");
		}
		...
		if ($mode == "edit") {
			$query=mysql_query("SELECT * FROM ".$GLOBALS["prefix_lms"]."_prj_news WHERE pid='$id' AND id='$itemid';");
			if (($query) && (mysql_num_rows($query) > 0)) {
				$row=mysql_fetch_array($query);
				$ndate=$GLOBALS["regset"]->databaseToRegional($row["ndate"], "date");
			}
		...
		if (empty($ndate))
			$ndate=$GLOBALS["regset"]->databaseToRegional(date("Y-m-d"), "date");
		...
		if (($query) && (mysql_num_rows($query) > 0) || ($mode == "new")) {
			...
			if ($mode == "edit") {
				...
				$ndate=$GLOBALS["regset"]->databaseToRegional($row["ndate"]);
				//$datearr[2]."-".$datearr[1]."-".$datearr[0];
			}
			if ($mode == "new")
				$ndate=$GLOBALS["regset"]->databaseToRegional(date("Y-m-d H:i:s"));
...
function edit_todo($mode="edit") {
	...
	if(($view_perm) && (in_array($id, $myprj)) && ( (is_owner($GLOBALS["current_user"]->getIdSt(), $id)) || (is_admin($GLOBALS["current_user"]->getIdSt(), $id)) ) ) {
		...
		if (isset($_POST["applychanges"])) {
			...
			if ($mode == "new") {
				$query=mysql_query("INSERT INTO ".$GLOBALS["prefix_lms"]."_prj_todo (pid,ttitle,ttxt) VALUES('$id','$ttitle','$ttxt');");
			}
			if ($mode == "edit") {
				$query=mysql_query("UPDATE ".$GLOBALS["prefix_lms"]."_prj_todo SET ttitle='$ttitle',ttxt='$ttxt' WHERE id='$itemid' LIMIT 1;");
			}
			@jumpTo(" index.php?modname=project&op=showprj&id=$id");
		}
		...
		if ($mode == "edit") {
			$query=mysql_query("SELECT * FROM ".$GLOBALS["prefix_lms"]."_prj_todo WHERE pid='$id' AND id='$itemid';");
			if (($query) && (mysql_num_rows($query) > 0)) {
				$row=mysql_fetch_array($query);
			}
			$label=$lang->def("_SAVE");
...
function edit_tasks($mode="edit") {
	...
	if(($view_perm) && (in_array($id, $myprj)) && ( (is_owner($GLOBALS["current_user"]->getIdSt(), $id)) || (is_admin($GLOBALS["current_user"]->getIdSt(), $id)) ) ) {
		...
		if (isset($_POST["applychanges"])) {
			...
			if ($mode == "new") {
				$query=mysql_query("INSERT INTO ".$GLOBALS["prefix_lms"]."_prj_tasks (pid,tname,tprog,tdesc) VALUES('$id','$tname','$tprog','$tdesc');");
			}
			if ($mode == "edit") {
				$query=mysql_query("UPDATE ".$GLOBALS["prefix_lms"]."_prj_tasks SET tprog='$tprog',tname='$tname',tdesc='$tdesc' WHERE id='$itemid' LIMIT 1;");
			}
			@jumpTo(" index.php?modname=project&op=showprj&id=$id");
		}
		...
		if ($mode == "edit") {
			$query=mysql_query("SELECT * FROM ".$GLOBALS["prefix_lms"]."_prj_tasks WHERE pid='$id' AND id='$itemid';");
			if (($query) && (mysql_num_rows($query) > 0)) {
				$row=mysql_fetch_array($query);
			}
			$label=$lang->def("_SAVE");
	...
	if(($view_perm) && (in_array($id, $myprj)) && ( (is_owner($GLOBALS["current_user"]->getIdSt(), $id)) || (is_admin($GLOBALS["current_user"]->getIdSt(), $id)) ) ) {
		...
		if ( isset($_POST["applychanges"]) ) {
			...
			if ($mode == "new") {
				...
				if (($ok) && ($savefile != ""))
					$query=mysql_query("INSERT INTO ".$GLOBALS["prefix_lms"]."_prj_files (pid,fname,ftitle,fver,fdesc) VALUES('$id','".( get_magic_quotes_gpc() ? $savefile : mysql_escape_string($savefile) )."','$ftitle','$fver','$fdesc');");
			}
			if ($mode == "edit") {
				$query=mysql_query("UPDATE ".$GLOBALS["prefix_lms"]."_prj_files SET ftitle='$ftitle',fver='$fver',fdesc='$fdesc' WHERE id='$itemid' LIMIT 1;");
			}
		...
		if ($mode == "edit") {
			$query=mysql_query("SELECT * FROM ".$GLOBALS["prefix_lms"]."_prj_files WHERE pid='$id' AND id='$itemid';");
			if (($query) && (mysql_num_rows($query) > 0)) {
				$row=mysql_fetch_array($query);
			}
			$label=$lang->def("_SAVE");
		...
		$out->add($form->getTextfield($lang->def("_FILEVERSION"), "fver", "fver", 255, $fver));
		...
		if (($query) && (mysql_num_rows($query) > 0) || ($mode == "new")) {
			...
			$out->add("</tr><tr><td>".$lang->def("_FILEVER")."\n ");
	...
	if((//-TP// funAccess("project","OP",true)) && (in_array($id, $myprj))) {
		...
		if ( isset($_POST["addmsg"]) ) {
			...
			if($_FILES['attach']['name'] == '') $savefile = '';
				...
				if(!file_exists ($pathprj.$savefile)) {
					if(!sl_upload($_FILES['attach']['tmp_name'], $pathprj.$savefile))
					{
						sl_close_fileoperations();
						$savefile = '';
						$out->add( '<div class="errorBlock">'.$lang->def("_ERRORUPLOAD").'</div><br />');
						$ok=0;
					}
				}
				else {
			...
			if ($ok) {
				...
				$mtxt=$_POST["mtxt"];
				$mid=$_POST["mid"];
				$query=mysql_query("INSERT INTO ".$GLOBALS["prefix_lms"]."_prj_msg (pid,mid,mfrom,mdate,msub,mfile,mtxt) VALUES('$id','$mid','$mfrom','$mdate','$msub','$savefile','$mtxt');");
				jumpTo(" index.php?modname=project&op=showprj&id=$id");
			}
		}
		...
		if ( isset($_POST['replyto']) && ($_POST["replyto"] > 0)) {
			$query=mysql_query("SELECT * FROM ".$GLOBALS["prefix_lms"]."_prj_msg WHERE id='".$_POST["replyto"]."' AND pid='$id';");
			if (($query) && (mysql_num_rows($query) > 0)) {
					$row=mysql_fetch_array($query);
					$title=$row["msub"];
		...
		if ($mid == "") {
			$mid=$GLOBALS["current_user"]->getIdSt()."-".time().chr(rand(65,90)).chr(rand(65,90));
		}
		...
		$out->add("<input type=\"hidden\" id=\"mfrom\" name=\"mfrom\" value=\"".$GLOBALS["current_user"]->getIdSt()."\" />\n");
...
function read_msg() { 	/*
	...
	if((//-TP// funAccess("project","OP", true)) && (in_array($id, $myprj))) {
		...
		$query=mysql_query("SELECT * FROM ".$GLOBALS["prefix_lms"]."_prj_msg WHERE id='".$_GET["msgid"]."' AND pid='$id';");
		if (($query) && (mysql_num_rows($query) > 0)) {
				$row=mysql_fetch_array($query);
				$title=$row["msub"];
				$text=$row["mtxt"];
				$user_query=mysql_query("SELECT * FROM ".$GLOBALS["prefix_lms"]."_user WHERE idUser='".$row["mfrom"]."';");
				...
				if (!in_array($GLOBALS["current_user"]->getIdSt(), $readarr)) {
					array_push($readarr, $GLOBALS["current_user"]->getIdSt());
					$mread=implode("-", $readarr);
					$query=mysql_query("UPDATE ".$GLOBALS["prefix_lms"]."_prj_msg SET mread='$mread' WHERE  id='".$_GET["msgid"]."' AND pid='$id';");
				}
...
function sel_prj($goto) { /*
	...
	$out->add( '<div class="titleSection">'
		.'<img src="'.getPathImage().'title_section/progetti.gif" alt="'.$lang->def("_IMGSECTION").'" />'
		._SECTIONNAME
	.'</div>');
...
/*
	...
	foreach ($myprj as $key=>$val) {
		$query=mysql_query("SELECT * FROM ".$GLOBALS["prefix_lms"]."_prj WHERE id='$val';");
		$row=mysql_fetch_array($query);
		if (is_owner($GLOBALS["current_user"]->getIdSt(), $val))
...
function del_prj_now($id) {
	...
	// -------------------------------------- Cancello i file:
	$query=mysql_query("SELECT * FROM ".$GLOBALS["prefix_lms"]."_prj_files WHERE pid='$id';");
	if (($query) && (mysql_num_rows($query) > 0)) { // cancello allgeati
		while($row=mysql_fetch_array($query)) {
			@sl_unlink(_FPATH_INTERNAL.$row["fname"]);
		}
	} // Cancello le righe dal database:
	$query=mysql_query("DELETE FROM ".$GLOBALS["prefix_lms"]."_prj_files WHERE pid='$id';");
	// ------------------------------------------------------------

	// -------------------------------------- Cancello i tasks:
	// Cancello le righe dal database:
	$query=mysql_query("DELETE FROM ".$GLOBALS["prefix_lms"]."_prj_tasks WHERE pid='$id';");
	// ------------------------------------------------------------

	// -------------------------------------- Cancello le news:
	// Cancello le righe dal database:
	$query=mysql_query("DELETE FROM ".$GLOBALS["prefix_lms"]."_prj_news WHERE pid='$id';");
	// ------------------------------------------------------------

	// -------------------------------------- Cancello le cose da fare:
	// Cancello le righe dal database:
	$query=mysql_query("DELETE FROM ".$GLOBALS["prefix_lms"]."_prj_todo WHERE pid='$id';");
	// ------------------------------------------------------------

	// -------------------------------------- Cancello i flag utente:
	// Cancello le righe dal database:
	$query=mysql_query("DELETE FROM ".$GLOBALS["prefix_lms"]."_prj_users WHERE pid='$id';");
	// ------------------------------------------------------------

	// -------------------------------------- Cancello le info del progetto:
	// Cancello le righe dal database:
	$query=mysql_query("DELETE FROM ".$GLOBALS["prefix_lms"]."_prj WHERE id='$id' LIMIT 1;");
	// ------------------------------------------------------------
...
function del_item() {
	...
	if(($view_perm) && (in_array($id, $myprj)) && ( (is_owner($GLOBALS["current_user"]->getIdSt(), $id)) || (is_admin($GLOBALS["current_user"]->getIdSt(), $id)) ) ) {
		...
		if (isset($_POST["undo"])) {
			...
			$out->add(getDeleteUi(
			$lang->def('_AREYOUSURE_DEL_'.strtoupper($type)),
				'<span class="text_bold">'.$lang->def('_TITLE').' :</span> '.$title.'<br />',
				false,
				'conf_del',
				'undo'));
...
function del_item_now($id, $itemid, $type) {
	...
	switch($type) {
		case "news": {
				$query=mysql_query("DELETE FROM ".$GLOBALS["prefix_lms"]."_prj_news WHERE id='".$itemid."' AND pid='".$id."' LIMIT 1;");
		} break;
		case "todo": {
			$query=mysql_query("DELETE FROM ".$GLOBALS["prefix_lms"]."_prj_todo WHERE id='".$itemid."' AND pid='".$id."' LIMIT 1;");
		} break;
		case "task": {
			$query=mysql_query("DELETE FROM ".$GLOBALS["prefix_lms"]."_prj_tasks WHERE id='".$itemid."' AND pid='".$id."' LIMIT 1;");
		} break;
		case "file": {
			$query=mysql_query("SELECT * FROM ".$GLOBALS["prefix_lms"]."_prj_files WHERE id='".$itemid."' AND pid='".$id."'");
			$row=@mysql_fetch_array($query);
			@sl_unlink(_FPATH_INTERNAL.$row["fname"]);
			$query=mysql_query("DELETE FROM ".$GLOBALS["prefix_lms"]."_prj_files WHERE id='".$itemid."' and pid='".$id."' LIMIT 1;");
		} break;
	}
...
function read_item() {
	...
	if(($view_perm) && (in_array($id, $myprj))) {
		...
		$query=mysql_query("SELECT * FROM ".$GLOBALS["prefix_lms"].$table." WHERE pid='$id' AND id='".(int)$_GET["itemid"]."' LIMIT 1;");
		if (($query) && (mysql_num_rows($query) > 0)) {
			$row=mysql_fetch_array($query);
		...
		if ($_GET["type"] == "file") {
			...
			$out->add('<a href="'.$url.'">'.$img." ".$lang->def("_DOWNLOAD_FILE")."</a>\n");
		}
...
function edit_progtot() {
	...
	if(($view_perm) && (in_array($id, $myprj)) && ( (is_owner($GLOBALS["current_user"]->getIdSt(), $id)) || (is_admin($GLOBALS["current_user"]->getIdSt(), $id)) ) ) {
		...
		if ( isset($_POST["applychanges"]) ) {
			...
			$query=mysql_query("UPDATE ".$GLOBALS["prefix_lms"]."_prj SET pprog='$progtot' WHERE id='$id' LIMIT 1;");
			@jumpTo(" index.php?modname=project&op=showprj&id=$id");
		}
...
function projectDispatch($op) {
	...
	switch($op) {
		...
		case "prjadditem": {
			if (isset($_POST["undo"])) {
				jumpTo("index.php?modname=project&op=showprj&id=".$_GET["id"]);
			}
			else {
		...
		case "download" : {
			...
			switch($type) {
				case "file" : {
					$query=mysql_query("SELECT * FROM ".$GLOBALS["prefix_lms"]."_prj_msg WHERE pid='$id' $filter ORDER BY $oby $ord, id DESC;");
					list( $pid, $fname, $ftitle ) = mysql_fetch_row(mysql_query("
