...
class Choice_Question extends Question {
	...
	function create( $idTest, $back_test ) {
		...
		if(isset($_POST['add_question'])) {
			...
			list($idQuestBank) = mysql_fetch_row(mysql_query("SELECT LAST_INSERT_ID()"));
			if(!$idQuestBank) {
		...
		$GLOBALS['page']->add('<table class="test_answer" cellspacing="0" summary="'.$lang->def('_TEST_ANSWER_SUMMARY').'">'."\n"
	...
	function displayUserResult( $id_track, $num_quest, $show_solution ) {
		...
		WHERE idQuest = '".(int)$this->id."' AND 
			idTrack = '".(int)$id_track."'";
		list($id_answer_do) = mysql_fetch_row(mysql_query($recover_answer));
	...
	function importFromRaw($raw_quest, $id_test = false) {
		...
		//find id of auto_increment colum
		list($new_id_quest_bank) = mysql_fetch_row(mysql_query("SELECT LAST_INSERT_ID()"));
		if(!$new_id_quest_bank) return false;

		if(!is_array($raw_quest->answers)) return $new_id_quest_bank;

		if($id_test != 0)
		{
			//insert question vao test
			$ins_query = "
				INSERT INTO ".$GLOBALS['prefix_lms']."_testquest 
				( idQuest, idTest, idCategory, type_quest, title_quest, difficult, time_assigned, sequence, page ) VALUES 
				( 	NULL,
