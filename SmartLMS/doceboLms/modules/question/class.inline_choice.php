...
class InlineChoice_Question extends Question {
	...
	function edit( $back_test ) {
		...
		if(isset($_POST['add_question'])) {
			//update question
			if(substr_count($_POST['title_quest'], '[answer]') != 1) {
				$GLOBALS['page']->add(getErrorUi($lang->def('_TEST_ERR_BAD_ANSWER_TAG')
					.getBackUi('index.php?modname=question&amp;op=create&amp;type_quest='
	...
	function play( $num_quest, $shuffle_answer = false, $id_track = 0, $freeze = false ) {
		...
		$select = '<select class="test_inline_select" id="quest_'.$id_quest.'" name="quest['.$id_quest.']"'
				...
				.($GLOBALS['lms']['no_answer_in_test'] == 'on' ? '<option value="0"'.( $find_prev ? '' : ' selected="selected"' ).'>'.$lang->def('_QUEST_IDONTWANTTO').'</option>' : '');
	...
	function displayUserResult( $id_track, $num_quest, $show_solution ) {
		...
		$select = ($GLOBALS['lms']['no_answer_in_test'] == 'on' ? '<span class="text_bold">'.$lang->def('_QUEST_IDONTWANTTO').'</span>' : '');
		while(list($id_answer, $answer, $is_correct, $com) = mysql_fetch_row($re_answer)){
