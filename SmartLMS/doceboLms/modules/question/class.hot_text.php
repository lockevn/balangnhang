...
class HotText_Question extends Question {
	...
	function create( $idTest, $back_test ) {
		...
		$GLOBALS['page']->add(getTitleArea($lang->def('_TEST_SECTION'), 'test')
			...
			.Form::getTextfield( $lang->def('_TEST_QUEST_HT_TITLE'), 'title_info', 'title_info', 300, 
			( isset($_POST['title_info']) ? stripslashes($_POST['title_info']) : '' ), $lang->def('_TEST_QUEST_HT_TITLE') )
	...
	function edit( $back_test ) {
		...
		if(!isset($_POST['answer_id'])) {
			list($title_info) = mysql_fetch_row(mysql_query("
			SELECT extra_info 
			FROM ".$GLOBALS['prefix_lms']."_testquest_extra 
			WHERE idQuest = '".(int)$this->id."' AND idAnswer = '0'"));
			
			list($sel_cat, $quest, $sel_diff, $sel_time) = mysql_fetch_row(mysql_query("
		...
		$GLOBALS['page']->add(getTitleArea($lang->def('_TEST_SECTION'), 'test')
			...
			.Form::getTextfield( $lang->def('_TEST_QUEST_HT_TITLE'), 'title_info', 'title_info', 300, 
			( isset($_POST['title_info']) ? stripslashes($_POST['title_info']) : $title_info ), $lang->def('_TEST_QUEST_HT_TITLE') )
	...
	function play( $num_quest, $shuffle_answer = false, $id_track = 0, $freeze = false ) {
		...
		WHERE idQuest = '".(int)$this->id."'
		ORDER BY sequence");
		list($id_none, $none) = mysql_fetch_row($re_answer);
	...
	function displayUserResult( $id_track, $num_quest, $show_solution ) {
		...
		WHERE idQuest = '".(int)$this->id."'
		ORDER BY sequence");
		list($id_none, $none, $is_correct_none, $comm_none) = mysql_fetch_row($re_answer);
