...
class Question {
	...
	function copy( $new_id_test, $back_test = NULL, $isInBank=false ) {
		...
		FROM ".$GLOBALS['prefix_lms'].$tableNamePrefix."quest_extra 
		WHERE idQuest = '".(int)$this->id."'");
	...
	function exportToRaw($isInBank = false) {
		...
		FROM ".$GLOBALS['prefix_lms'].$tablePrefix."quest 
		WHERE idQuest = '".(int)$this->id."'")); 
		...
		FROM ".$GLOBALS['prefix_lms'].$tablePrefix."questanswer 
		WHERE idQuest = '".(int)$this->id."'
		ORDER BY idAnswer");
		...
		$re_extra = mysql_query("
		SELECT idAnswer, extra_info 
		FROM ".$GLOBALS['prefix_lms'].$tablePrefix."quest_extra 
		WHERE idQuest = '".(int)$this->id."'");
