...
function myfriends(&$url) {
	...
	while(list(, $ui) = each($users_info)) {
		...
		$profile->init('profile', 'lms', 'modname=myfriends&op=searchuser', 'ap');
		...
		'<p class="mf_action_line">'
			...
			.'<a href="'.$url->getUrl('op=delfriend&id_friend='.$ui['id']).'" '
				.'title="'.( $ui['waiting'] == MF_WAITING ? $lang->def('_DEL_WAITING_FRIEND').' : '.$profile->resolveUsername() : $lang->def('_TITLE_DEL_FRIEND').' : '.$profile->resolveUsername()).'">'
				//.'title="'.$lang->def('_TITLE_DEL_FRIEND').' : '.$profile->resolveUsername().'">'
				.'<img src="'.getPathImage().'standard/remfriend.gif" alt="'.( $ui['waiting'] == MF_WAITING ? $lang->def('_DEL_WAITING_FRIEND').' : '.$profile->resolveUsername() : $lang->def('_TITLE_DEL_FRIEND').' : '.$profile->resolveUsername()).'" />'
				//.'<img src="'.getPathImage().'standard/remfriend.gif" alt="'.$lang->def('_TITLE_DEL_FRIEND').' : '.$profile->resolveUsername().'" />'
			.'</a>'
		.'</p>', 'content');
	}
...
function approveuser(&$url) {
	...
	while(list(, $ui) = each($users_info)) {
		...
		$profile = new UserProfile( $ui['id'] );
		$profile->init('profile', 'lms', 'modname=myfriends&op=approveuser', 'ap');
