		...
		while(list($idF, $title, $descr, $num_thread, $num_post, $locked, $emoticons) = mysql_fetch_row($re_forum) ) {
			...
			if($mod_perm) {
				...
				$content[] = '<a href="index.php?modname=forum&amp;op=delforum&amp;idForum='.$idF.'" ' .
						'title="'.$lang->def('_DELFORUM').' : '.strip_tags($title).'">
			...
			$GLOBALS['page']->add(
				...
				.'<tr>'
					...
					.'<td class="image" nowrap="nowrap">'.$num_thread
						.( $thread_notread ? '<div class="forum_notread">'.$thread_notread.' '.$lang->def('_UNREADED').'</div>' : '' )
					.'</td>'
					.'<td class="image" nowrap="nowrap">'.$num_post
						.( $mess_notread ? '<div class="forum_notread">'.$mess_notread.' '.$lang->def('_UNREADED').'</div>' : '' )
					.'</td>'
				.'</tr>'
				.'<tr>'
			...
			if($mod_perm) {
				...
				$GLOBALS['page']->add(
					...
					.'<li><a href="index.php?modname=forum&amp;op=delforum&amp;idForum='.$idF.'"' .
							' title="'.$lang->def('_DELFORUM').' : '.strip_tags($title).'">
						<img src="'.getPathImage().'standard/rem.gif" alt="'.$lang->def('_DEL').'" /></a></li>'
				, 'content');
...
function addforum() {
	...
	$default = 'blank.gif';
	$onchange="onchange=\"if(document.images) document.images['forum_icon'].src='".getPathImage('fw')."emoticons/"."'+this.options[this.selectedIndex].value;\"";
	...
	$GLOBALS['page']->add(
		'</select> <img src="'.getPathImage('fw')."emoticons/blank.gif".'" id="forum_icon" name="forum_icon" />'
		.Form::closeFormLine()
...
function modforum() {
	...
	closedir($templ->handle);
	$GLOBALS['page']->add(
		'</select> <img src="'.getPathImage('fw')."emoticons/".$emoticons.'" id="forum_icon" name="forum_icon" />'
		.Form::closeFormLine()
		.Form::closeElementSpace()
		.Form::openButtonSpace()
...
function thread() {
	...
	while(list($idT, $t_author, $posted, $title, $num_post, $num_view, $locked, $erased, $important) = mysql_fetch_row($re_thread)) {
		...
		$content_temp = ( $erased && !$mod_perm ?
						...
						($important ? '<img src="'.getPathImage().'forum/important.gif" alt="'.$lang->def('_IMPORTANT').'" />' : '').' <a'.$c_css.' href="index.php?modname=forum&amp;op=message&amp;idThread='.$idT.'">'.$title.'</a>');
		...
		if($mod_perm) {
			...
			$content[] = '<a href="index.php?modname=forum&amp;op=movethread&amp;id_forum='.$id_forum.'&amp;id_thread='.$idT.'"><img src="'.getPathImage().'standard/move.gif" alt="'.$lang->def('_ALT_MOVE').'" title="'.$lang->def('_MOVE_THREAD').'" /></a>';
...
function modthread() {
	...
	FROM ".$GLOBALS['prefix_lms']."_forumthread
	WHERE idThread = '".$id_thread."'";
	list($id_forum, $is_important) = mysql_fetch_row(mysql_query($thread_query));
	$forum_query = "
	SELECT title, locked
	FROM ".$GLOBALS['prefix_lms']."_forum
	WHERE idForum = '".$id_forum."'";
...
function delthread() {
	...
	if(isset($_POST['undo'])) {
		if($_GET['search'] == 1) jumpTo('index.php?modname=forum&op=search&amp;ini='.$ini);
...
function message() {
	...
	$profile_man->init('profile', 'lms', 'index.php?modname=forum&op=forum');
	...
	FROM ".$GLOBALS['prefix_lms']."_forumthread
	WHERE idThread = '".$id_thread."'";
	list($id_forum, $thread_title, $tot_message, $locked_t, $erased_t, $is_important) = mysql_fetch_row(mysql_query($thread_query));
	...
	while(list($id_message, $message_info) = each($messages)) {
		...
		$sender .= $author['actions']
			...
			.'<a href="index.php?modname=forum&amp;op=viewprofile&amp;idMessage='.$id_message.'&amp;ini='.$ini_page.'&amp;idThread='.$id_thread.'">'.$lang->def('_VIEWPROFILE').'</a>';
	...
	if ($is_important)
		if (checkPerm('moderate', true) || checkPerm('mod', true))
			$GLOBALS['page']->add('<div><p align="right"><a href="index.php?modname=forum&op=message&amp;idThread='.$id_thread.'&amp;ini='.$ini_page.'&amp;important=2">'.$lang->def('_SET_NOT_IMPORTANT_THREAD').'</a></p>', 'content');
	else
		if (checkPerm('moderate', true) || checkPerm('mod', true))
			$GLOBALS['page']->add('<div><p align="right"><a href="index.php?modname=forum&op=message&amp;idThread='.$id_thread.'&amp;ini='.$ini_page.'&amp;important=1">'.$lang->def('_SET_IMPORTANT_THREAD').'</a></p>', 'content');
...
function viewprofile() {
	...
	$profile->init('profile', 'lms', 'modname=forum&op=viewprofile&idMessage='.$id_message.'&ini='.$ini, 'ap');
...
function savesema() {
	...
	jumpTo('index.php?modname=forum&op=message&idThread='.$id_thread.'&amp;result='.( $re ? 'ok' : 'err_sema' ));
}
...
function moveThread($id_thread, $id_forum)
{
	...
	if(isset($_GET['confirm']))
	{
		...
		jumpTo('index.php?modname=forum&amp;op=thread&idForum='.$id_forum);
	}
...
function addUnreadNotice($id_forum) {
	...
	FROM ".$GLOBALS['prefix_lms']."_forum_access 
	WHERE idForum = '".$id_forum."'";
	$re_view = mysql_query($query_view_forum);
	if(mysql_num_rows($re_view)) {
	
		$members = array();
		while(list($idst) = mysql_fetch_row($re_view)) {
			$members[] = $idst; 
		}
