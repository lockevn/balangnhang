            ...
            $tadd = ": ".$this->itemtrack->getTrackingScore($report['idscorm_tracking'])."/".$itemInfo['adlcp_masteryscore'];
				...
				if( $this->showit ) {
					if( $this->linkCustomCallBack !== FALSE ) {
						...
						$out .= "<div class=\"report_on_tree $class\">".$func(def($text, 'standard', 'framework').$tadd,$itemInfo['uniqueid'])."</div>";
					} else {
						$out .= "<div class=\"report_on_tree $class\">".def($text, 'standard', 'framework').$tadd."</div>";
					}
				}
			} else {
