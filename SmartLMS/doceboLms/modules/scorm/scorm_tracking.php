...
class Scorm_Tracking {
	...
	function Scorm_Tracking($idUser, $idRefernce, $id, $idscorm_package, $connection, $createonfail = true, $id_isitemid = false)
	{
		...
		if( $idUser === null ) {
			...
			list($this->idUser, $this->idscorm_item) = mysql_fetch_row($rs);
			$this->idtrack = $id;
	...
	function getXmlDoc()
	{
		...
		$rs = mysql_query("SELECT xmldata FROM $this->tracktable WHERE idscorm_tracking = $this->idtrack", $this->dbconn);
		if ($rs === false) {
