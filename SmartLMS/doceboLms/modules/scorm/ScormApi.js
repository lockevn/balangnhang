 ...
 */
  ...
  */
 function ScormApi(host, baseurl, serviceid, idUser, idReference, idscorm_organization, scormVersion) {
 	this.id = "";
	this.idscorm_item = "";
	this.idUser = idUser || "";
...
*/
	...
	if( ajxreq.transport.status == 200 ) {
		try {
			...
			w.document.close();
   			alert( ex );
			alert( ajxreq.transport.responseText );
		}
	} else {
		alert( ajxreq.transport.status );
		this.setError("101");
...
ScormApi.prototype.CreateXmlHttpRequest = function() {
 ...
 ScormApi.prototype.errorTable["201"] = "";
...
function ScormApiUI( host, baseurl, serviceid, idUser, idReference, idscorm_organization, scormVersion ) {
	this.base = ScormApi;
	this.base(host, baseurl, serviceid, idUser, idReference, idscorm_organization, scormVersion);
	this.slEventCap = 0;
