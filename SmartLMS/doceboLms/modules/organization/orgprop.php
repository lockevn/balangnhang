...
function organization_property(&$treeView, $idItem) {
	...
	$tv->addTab( new TabElemDefault( 'prereqisites', $lang->def('_ORGPREREQUISITES'), getPathImage().'organizations/prerequisites.gif' ) );
	$tv->addTab( new TabElemDefault( 'settings', $lang->def('_ORGPROPERTIESTITLE'), getPathImage().'organizations/settings1.gif' ) );
	$tv->addTab( new TabElemDefault( 'catalogation', $lang->def('_CATINFO'), getPathImage().'standard/mod.gif' ) );
...
function organization_property_prereqisites(&$treeView, $idItem, &$form, &$lang) {
	...
	$GLOBALS['page']->add( $form->getOpenFieldset($lang->def('_ORGPREREQUISITES')));
...
function organization_property_settings(&$treeView, $idItem, &$form, &$lang) {
	...
	$GLOBALS['page']->add( 
		$form->getRadioSet( $lang->def( '_ORGVISIBILITY' ), 
							'visibility', 
							'visibility', 
	...
	if( !$isFolder ) {
		...
		$orgselfprerequisites = array( $lang->def( '_ORGPRETOCOMPLETE' ) => "incomplete",
											$lang->def( '_ORGPREINFINITE' ) => "*",
											$lang->def( '_ORGPREONLYONE' ) => "NULL" );
		
		$GLOBALS['page']->add($form->getOpenCombo( $lang->def( '_ORGSELFPREREQUISITES' ) ) );
		...
		/*$GLOBALS['page']->add( 
				$form->getRadioSet( $lang->def( '_ORGSELFPREREQUISITES' ), 
									'selfPrerequisites', 
									'selfPrerequisites', 
									array( $lang->def( '_ORGPRETOCOMPLETE' ) => "incomplete",
											$lang->def( '_ORGPREINFINITE' ) => "*",
											$lang->def( '_ORGPREONLYONE' ) => "NULL"	 ),
		...
		if( $lo->canBeMilestone() ) {
			...
			if(  $startFolder !== FALSE && $startFolder->id != $folder->id ) {
				...
				$jsOut .= 'if( milestone_value == "start" )'
							.'if( !window.confirm("'
							.unhtmlentities(addslashes(str_replace( '%LOTitle%', 
										$startFolder->otherValues[REPOFIELDTITLE], 
...
function organization_property_catalogation(&$treeView, $idItem, &$form, &$lang) {
	...
	$arr_diff = array( 	'1'	=>	$lang->def( '_VERYEASY' ),
						'2'	=>	$lang->def( '_EASY' ),
						'3'	=>	$lang->def( '_MEDIUM' ),
						'4'	=>	$lang->def( '_DIFFICULT2'),
						'5'	=>	$lang->def( '_VERYDIFFICULT')   
					);
	$selDiff = isset($values['difficult'])?$values['difficult']:"";
	
	$GLOBALS['page']->add( $form->getDropdown(	$lang->def( '_DIFFICULT' ), 
												"difficult", 
												"difficult", 
												$arr_diff , 
												$selDiff 
	...
	$langArray = $GLOBALS['globLangManager']->getAllLangCode();
...
function organization_property_common(&$treeView, $idItem, &$form, &$lang, &$folder) {
	...
	if( isset($_POST['stay_on_properties']) ) {
		...
		$values['publish_from'] = $GLOBALS['regset']->databaseToRegional($data[ORGFIELD_PUBLISHFROM], 'date');	
		$values['publish_to'] = $GLOBALS['regset']->databaseToRegional($data[ORGFIELD_PUBLISHTO], 'date');
	}
...
function organization_access( &$treeView, $idItem ) {
	...
	if( isset($_POST['okselector']) ) {
		...
		$treeView->tdb->_setAccessUG($idItem, ACLKINDUSER, $arr_selected);
		jumpTo( 'index.php?modname=storage' );
	} elseif( isset($_POST['cancelselector']) ) {
		jumpTo( 'index.php?modname=storage' );
	} else {
		if( !isset($_GET['stayon']) ) {
			$directory->resetSelection($treeView->tdb->getAccessUsers($idItem));
		}
		$directory->show_user_selector = TRUE;
		$directory->show_group_selector = TRUE;
		$directory->show_orgchart_selector = FALSE;
		...
		$directory->setUserFilter('group',$arr_idstGroup);
		$directory->setGroupFilter('path', '/lms/course/'.$_SESSION['idCourse'].'/group');
		
		$directory->multi_choice = TRUE;
		$directory->loadSelector('index.php?modname=storage&amp;org_access=1&amp;idItem='.$idItem.'&amp;stayon=1',
						$lang->def( '_ORG_ACCESS' ),
						$lang->def( '_ORG_ACCESSDESCR' ).' : '
							.'<span class="text_bold">'.$folder->otherValues[REPOFIELDTITLE].'</span>',
						TRUE);
	}
