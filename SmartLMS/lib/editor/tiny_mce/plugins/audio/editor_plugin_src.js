...
tinyMCE.importPluginLanguagePack('audio');

var Smartweb_AudioPlugin = {
	getInfo : function() {
		return {
			longname : 'Audio',
			author : 'Moxiecode Systems AB',
			authorurl : 'http://tinymce.moxiecode.com',
			infourl : 'http://wiki.moxiecode.com/index.php/TinyMCE:Plugins/audio',
			version : tinyMCE.majorVersion + "." + tinyMCE.minorVersion
		};
	},

	initInstance : function(inst) {
		// Warn if user has flash plugin and audio plugin at the same time
		if (inst.hasPlugin('flash') && !tinyMCE.flashWarn) {
			alert('Flash plugin is deprecated and should not be used together with the audio plugin.');
			tinyMCE.flashWarn = true;
		}

		if (!tinyMCE.settings['audio_skip_plugin_css'])
			tinyMCE.importCSS(inst.getDoc(), tinyMCE.baseURL + "/plugins/audio/css/content.css");
	},

	getControlHTML : function(cn) {
		switch (cn) {
			case "audio":
				return tinyMCE.getButtonHTML(cn, 'lang_audio_desc', '{$pluginurl}/images/audio.gif', 'mceAudiossss');
		}
	...
	execCommand : function(editor_id, element, command, user_interface, value) {
		...
		switch (command) {
			case "mceAudio":
				tinyMCE.openWindow({
						file : '../../plugins/audio/audio.htm',
						width : 430 + tinyMCE.getLang('lang_audio_delta_width', 0),
						height : 470 + tinyMCE.getLang('lang_audio_delta_height', 0)
					}, {
						editor_id : editor_id,
						inline : "yes"
				});
	...
	cleanup : function(type, content, inst) {
		...
		switch (type) {
			case "insert_to_editor":
				...
				content = content.replace(/<script[^>]*>\s*write(Flash|ShockWave|Windowsaudio|QuickTime|Realaudio)\(\{([^\)]*)\}\);\s*<\/script>/gi, '<img class="mceItem$1" title="$2" src="' + img + '" />');
			...
			case "insert_to_editor_dom":
				...
				for (i=0; i<nl.length; i++) {
					if (/mceItem(Flash|ShockWave|Windowsaudio|QuickTime|Realaudio)/.test(nl[i].className)) {
				...
				for (i=0; i<nl.length; i++) {
					...
					switch (ci) {
						case 'clsid:d27cdb6e-ae6d-11cf-96b8-444553540000':
							nl[i].parentNode.replaceChild(Smartweb_AudioPlugin._createImg('mceItemFlash', d, nl[i]), nl[i]);
							break;

						case 'clsid:166b1bca-3f9c-11cf-8075-444553540000':
							nl[i].parentNode.replaceChild(Smartweb_AudioPlugin._createImg('mceItemShockWave', d, nl[i]), nl[i]);
							break;

						case 'clsid:6bf52a52-394a-11d3-b153-00c04f79faa6':
						case 'clsid:22d6f312-b0f6-11d0-94ab-0080c74c7e95':
						case 'clsid:05589fa1-c356-11ce-bf01-00aa0055595a':
							nl[i].parentNode.replaceChild(Smartweb_AudioPlugin._createImg('mceItemWindowsaudio', d, nl[i]), nl[i]);
							break;

						case 'clsid:02bf25d5-8c17-4b23-bc80-d3488abddc6b':
							nl[i].parentNode.replaceChild(Smartweb_AudioPlugin._createImg('mceItemQuickTime', d, nl[i]), nl[i]);
							break;

						case 'clsid:cfcdaa03-8be4-11cf-b84b-0020afbbccfa':
							nl[i].parentNode.replaceChild(Smartweb_AudioPlugin._createImg('mceItemRealaudio', d, nl[i]), nl[i]);
							break;
					}
				}
				...
				for (i=0; i<nl.length; i++) {
					switch (tinyMCE.getAttrib(nl[i], 'type')) {
						case 'application/x-shockwave-flash':
							Smartweb_AudioPlugin._createImgFromEmbed(nl[i], d, 'mceItemFlash');
							break;

						case 'application/x-director':
							Smartweb_AudioPlugin._createImgFromEmbed(nl[i], d, 'mceItemShockWave');
							break;

						case 'application/x-mplayer2':
							Smartweb_AudioPlugin._createImgFromEmbed(nl[i], d, 'mceItemWindowsaudio');
							break;

						case 'video/quicktime':
							Smartweb_AudioPlugin._createImgFromEmbed(nl[i], d, 'mceItemQuickTime');
							break;

						case 'audio/x-pn-realaudio-plugin':
							Smartweb_AudioPlugin._createImgFromEmbed(nl[i], d, 'mceItemRealaudio');
							break;
					}
				}
				break;
			...
			case "get_from_editor":
				...
				while ((startPos = content.indexOf('<img', startPos+1)) != -1) {
					...
					attribs = Smartweb_AudioPlugin._parseAttributes(content.substring(startPos + 4, endPos));

					// Is not flash, skip it
					if (!/mceItem(Flash|ShockWave|Windowsaudio|QuickTime|Realaudio)/.test(attribs['class']))
						continue;
					...
					// Use object/embed
					if (!tinyMCE.getParam('audio_use_script', false)) {
						switch (attribs['class']) {
							case 'mceItemFlash':
								ci = 'd27cdb6e-ae6d-11cf-96b8-444553540000';
								cb = 'http://download.macroaudio.com/pub/shockwave/cabs/flash/swflash.cab#version=6,0,40,0';
								mt = 'application/x-shockwave-flash';
								break;

							case 'mceItemShockWave':
								ci = '166B1BCA-3F9C-11CF-8075-444553540000';
								cb = 'http://download.macroaudio.com/pub/shockwave/cabs/director/sw.cab#version=8,5,1,0';
								mt = 'application/x-director';
								break;

							case 'mceItemWindowsaudio':
								ci = tinyMCE.getParam('audio_wmp6_compatible') ? '05589FA1-C356-11CE-BF01-00AA0055595A' : '6BF52A52-394A-11D3-B153-00C04F79FAA6';
						...
						embedHTML = Smartweb_AudioPlugin._getEmbed(ci, cb, mt, pl, attribs);
					} else {
						// Use script version
						switch (attribs['class']) {
							case 'mceItemFlash':
	...
	handleNodeChange : function(editor_id, node, undo_index, undo_levels, visual_aid, any_selection) {
		...
		do {
			if (node.nodeName == "IMG" && /mceItem(Flash|ShockWave|Windowsaudio|QuickTime|Realaudio)/.test(tinyMCE.getAttrib(node, 'class'))) {
				tinyMCE.switchClass(editor_id + '_audio', 'mceButtonSelected');
				return true;
			}
		} while ((node = node.parentNode));

		tinyMCE.switchClass(editor_id + '_audio', 'mceButtonNormal');
...
tinyMCE.addPlugin("audio", Smartweb_AudioPlugin);
