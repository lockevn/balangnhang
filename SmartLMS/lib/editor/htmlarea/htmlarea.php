...
HTMLArea.indent = function(s, sindentChar) {
	...
	s = s.replace(c[4], function(strn) { //skip pre and script tags
	  strn = strn.replace(c[6], function(st,$1,$2,$3) { //exclude comments		
		// HACK: LockeVN: string = $3.replace(/[\n\r]/gi, " ").replace(/\s+/gi," ").replace(c[0], function(str) {
		// LockeVN: I do remove the .replace(/\s+/gi," ")  command, it will fix the bug for smartcom, but I dont know it side effect on indent feature of HTMLArea
		string = $3.replace(/[\n\r]/gi, " ").replace(c[0], function(str) {
		// ENDHACK: LockeVN        
			if (str.match(c[2])) {
