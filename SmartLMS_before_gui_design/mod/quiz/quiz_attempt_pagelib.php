...
define('PAGE_QUIZ_ATTEMPT',   'mod-quiz-attempt');


page_map_class(PAGE_QUIZ_ATTEMPT, 'page_quiz_attempt');

$DEFINEDPAGES = array(PAGE_QUIZ_ATTEMPT);
...
class page_quiz_attempt extends page_generic_activity {

    function init_quick($data) {
        if(empty($data->pageid)) {
            error('Cannot quickly initialize page: empty course id');
        }
        $this->activityname = 'quiz_attempt';
        parent::init_quick($data);
    }
  
    function get_type() {
        return PAGE_QUIZ_ATTEMPT;
    }
    ...
    /*danhut added to add mod-quiz-attempt option in admin/stickyblocks.php*/
	...
	function url_get_path() {
        ...
        if ($id == PAGE_QUIZ_ATTEMPT) {
            return $CFG->wwwroot.'/mod/quiz/attempt.php';
	...
	function print_header($title) {
        ...
        $replacements = array(
                              '%fullname%' => get_string('stickyblocksquizattempt','admin')
        );
        ...
        $nav = get_string('stickyblocksquizattempt','admin');
