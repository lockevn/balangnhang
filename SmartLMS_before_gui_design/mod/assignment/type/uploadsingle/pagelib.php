...
define('PAGE_UPLOAD_SINGLE_VIEW',   'mod-assignment-view');


page_map_class(PAGE_UPLOAD_SINGLE_VIEW, 'page_upload_single');

$DEFINEDPAGES = array(PAGE_UPLOAD_SINGLE_VIEW);
...
class page_upload_single extends page_generic_activity {

    function init_quick($data) {
        if(empty($data->pageid)) {
            error('Cannot quickly initialize page: empty course id');
        }
        $this->activityname = 'uploadsingle';
        parent::init_quick($data);
    }
  
    function get_type() {
        return PAGE_UPLOAD_SINGLE_VIEW;
    }
	...
	function url_get_path() {
        ...
        if ($id == PAGE_UPLOAD_SINGLE_VIEW) {
            return $CFG->wwwroot.'/mod/assignment/view.php';
	...
	function print_header($title) {
        ...
        $replacements = array(
                              '%fullname%' => get_string('stickyblockssingleuploadview','admin')
        );
        ...
        $nav = get_string('stickyblockssingleuploadview','admin');
