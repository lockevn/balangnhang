		...
		foreach ($userarr as $ua) {
			foreach ($ua as $u) {
				if ($u->ownertype == $ownerisuser) {
					...
					if ($sitewide == 1) {
						$entry .= "  (".get_string("categories", 'block_file_manager').": $course->shortname)";
					}
					if ($u->viewed == 0) {
						$entry .= " <img border=\"0\" src=\"../file_manager/pix/new.gif\" />";
					}
...
function fm_print_share_course_members($id, $linkid=NULL, $type=0, $groupid=0, $systemcontext=NULL) {
	global $CFG, $USER;
	...
	if ($id == 1) {
		...
		//$sqltoexecute = "SELECT {$CFG->prefix}user.firstname, {$CFG->prefix}user.lastname, {$CFG->prefix}user.id,  {$CFG->prefix}role.name FROM {$CFG->prefix}user, {$CFG->prefix}course_display, {$CFG->prefix}role_assignments, {$CFG->prefix}role  WHERE (course = $id AND {$CFG->prefix}course_display.userid = {$CFG->prefix}user.id AND {$CFG->prefix}user.id = {$CFG->prefix}role_assignments.userid AND {$CFG->prefix}role_assignments.roleid = {$CFG->prefix}role.id)";
		...
		// MODIF JF :: ERREUR si $roleid indefini
		if (!isset($roleid)){
			$roleid= get_records('role', 'id');
		}
		
		if (!$allpersons = get_role_users($roleid, $systemcontext, false, 'u.id, u.firstname, u.lastname, u.email, u.username, ra.hidden')) {
			$table->data[] = array("","<center><i><b>".get_string("msgnocourseusers", "block_file_manager")."</b></i></center>");
		} else {
			...
			if ($allpersons) {
				foreach($allpersons as $key => $au) {
					...
					$tmp = get_record("role", "id", $au->roleid);
				//	$allusers[$au->id]->role = $au->name;
					$allusers[$au->id]->role = $tmp->name;
