...
*/
	...
	/**
	$courselink = '';
	if ($course->id != SITEID) {		// To display properly at front page
		$courselink = "<a href=\"{$SESSION->fromdiscussion}\">{$course->shortname}</a> -> ";
	}

	print_header(
		strip_tags(get_string('addlink', 'block_file_manager')),
		"$course->fullname",
		" $courselink <a href=\"view.php?id={$id}&groupid={$groupid}\">".get_string('filemanager', 'block_file_manager').'</a> -> '.get_string('addlink', 'block_file_manager'),
		'',
		'',
		true,
		'',
