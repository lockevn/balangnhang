...
*/
    ...
    */
	...
	$systemcontext = get_context_instance(CONTEXT_COURSE, $id);
	$editingteacher = has_capability('moodle/legacy:editingteacher', $systemcontext);
	$coursecreator = has_capability('moodle/legacy:coursecreator', $systemcontext);
	...
	*/
	...
	if ($groupid != 0){
		...
		switch ($groupmode){
			...
			case VISIBLEGROUPS :
			case SEPARATEGROUPS : 
				if ($isadmin || $editingteacher || $coursecreator){
					//Super rights
				} elseif (! groups_is_member($groupid)){ // Must check if the user is member of that group
	...
	*/
	...
	if (!function_exists('build_navigation')){
		// before 1.9
        print_header(strip_tags(get_string('addlink', 'block_file_manager')), "$course->fullname", 
        " $courselink <a href=\"view.php?id={$id}&amp;groupid={$groupid}&amp;rootdir={$rootdir}\">".get_string("filemanager", 'block_file_manager')."</a> -> ".get_string("btnfilesharing", 'block_file_manager'), 
		'', // focus
		$meta, 
		true, '', navmenu($course, null, 1),'','');
    } else {
	    // new method path level 2 @ romain
    	$strtitle = strip_tags(get_string('addlink', 'block_file_manager'));
        $navigation = build_navigation(array(array('name'=>$strtitle, 'link'=> " $courselink <a href=\"view.php?id={$id}&amp;groupid={$groupid}&amp;rootdir={$rootdir}\">".get_string("filemanager", 'block_file_manager')."</a> -> ".get_string("btnfilesharing", 'block_file_manager'), 'type'=>'misc')));
        print_header($strtitle, $course->fullname, $navigation, 		
		'', // focus
		$meta, 
		true, '', ''); 
    }
	...
	print_table(fm_print_share_course_members($course->id, $linkid, $type, $groupid, $systemcontext));
	// Stores $cb id's
	if ($cb != NULL) {
		foreach($cb as $c) {
